...
static int exec_mmap(void)
{
	...
	if (current->mm->count > 1) {
		...
		*mm = *current->mm;
		...
		mm->mm_mach_task = current->osfmach3.task;
#else	/* CONFIG_OSFMACH3 */
...
int do_execve(char * filename, char ** argv, char ** envp, struct pt_regs * regs)
{
	...
	int i;
#ifdef	CONFIG_OSFMACH3
	unsigned short old_euid = current->euid;
#endif	/* CONFIG_OSFMACH3 */
...
#ifdef	CONFIG_OSFMACH3
	if (old_euid != current->euid) {
		/* identity has changed */
		extern void osfmach3_changed_identity(struct task_struct *tsk);
		osfmach3_changed_identity(current);
	}
#endif	/* CONFIG_OSFMACH3 */
	if(retval>=0)
		/* execve success */
