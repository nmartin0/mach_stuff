	...
	struct task_struct * p, struct pt_regs * regs)
{
	...
	struct i386_saved_state child_state;
	boolean_t inherit;
	...
	osfmach3_trap_setup_task(p, EXCEPTION_STATE, i386_SAVED_STATE);
	...
	if (clone_flags & CLONING_INIT) {
		...
		osfmach3_ptregs_to_state(regs, &child_state, NULL);
		child_state.gs = regs->gs;
		child_state.fs = regs->fs;
		child_state.es = regs->es;
		child_state.ds = regs->ds;
		child_state.trapno = 0;
		child_state.err = 0;
		child_state.cs = regs->cs;
		child_state.efl = regs->eflags & 0xffffcfff;	/* iopl is always 0 for a new process */
		child_state.esp = regs->esp;
		child_state.ss = regs->ss;
		/* XXX rest of the state ? */

		child_state.eax = 0;
		child_state.uesp = esp;

		kr = thread_set_state(child_thread, i386_SAVED_STATE,
