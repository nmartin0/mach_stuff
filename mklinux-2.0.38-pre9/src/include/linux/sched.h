...
#ifdef	CONFIG_OSFMACH3
#define CLONING_INIT	0x00100000	/* creating an empty task for "init" */
#define CLONING_KERNEL	0x00200000	/* creating a kernel "thread" */
#endif	/* CONFIG_OSFMACH3 */
...
struct mm_struct {
	int count;
#ifdef	CONFIG_OSFMACH3
	struct osfmach3_mach_task_struct *mm_mach_task;
...
#endif	
#ifdef	CONFIG_OSFMACH3
	struct osfmach3_task_struct osfmach3;	/* Mach info */
#endif	/* CONFIG_OSFMACH3 */
};
...
extern inline void __add_wait_queue(struct wait_queue ** p, struct wait_queue * wait)
{
	...
	*p = wait;
	wait->next = next;
#if	defined(__powerpc__) || defined(CONFIG_OSFMACH3)
	wait->task->tss.wchan = (unsigned long)p;
#endif	
}
