		...
		struct symbol_table *symtab)
{
	...
	GET_USE_COUNT(mp) += 1;
#ifdef __powerpc__
	/* The PowerPC instruction cache DOES NOT SNOOP from the */
	/* data cache.  This flush will make it happy! */
#ifdef	CONFIG_OSFMACH3
	flush_instruction_cache_range(&init_mm, 
				      (unsigned long) ((char *)mp->addr),
				      (unsigned long) (((char *)mp->addr) +
						       (mp->size * PAGE_SIZE)));
