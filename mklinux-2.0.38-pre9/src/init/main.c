...
#ifdef	CONFIG_OSFMACH3
extern void osfmach3_start_init(char **argv_init, char **envp_init);
...
static void parse_options(char *line)
{
	...
	while ((line = next) != NULL) {
		...
		if (checksetup(line))
			continue;
#ifdef	CONFIG_OSFMACH3
		if (!strcmp(line, "mach_console")) {
			extern int osfmach3_use_mach_console;
			osfmach3_use_mach_console = 1;
			continue;
		}
#ifdef	CONFIG_PMAC_CONSOLE
		if (!strcmp(line, "regular_vc_colors")) {
			extern int regular_vc_colors;
			regular_vc_colors = 1;
			continue;
		}
...
#if defined(CONFIG_OSFMACH3) && defined(CONFIG_MODULES)
extern void export_osfmach3_symbols(void);
#endif
