	...
	void	*arg)
{
	...
	for (;;) {
		...
		for (i = 0; i < count; i += sizeof (kd_event)) {
			ev = (kd_event *) &inbuf[i];
			if (ev->type != KEYBD_EVENT) {
				printk("keyboard_input_thread: "
				       "non-keyboard event 0x%x\n",
				       ev->type);
				continue;
			}
			keyboard_interrupt((int) ev->value.sc, NULL, NULL);
			kbd_bh();
		}
	}
	/*NOTREACHED*/
#else	/* KEYBD_EVENT */
	panic("keyboard_input_thread: where am I, who are you, what's a keyboard ?");
	keyboard_interrupt(0, NULL, NULL);
#endif	/* KEYBD_EVENT */
}
#endif	/* CONFIG_OSFMACH3 */
...
int kbd_init(void)
{
	...
	if (kr != KERN_SUCCESS) {
		MACH3_DEBUG(0, kr,
			    ("kbd_init: device_open(\"kbd\""));
		printk("kbd_init: can't open kbd. Using dumb keyboard.\n");
		return 0;
	}

/* XXX the following #define should be exported by the microkernel XXX */
#define KB_EVENT		1
#define OSFMACH3_KDSKBDMODE	0x80044b01
	count = sizeof keyboard_status / sizeof (int);
	keyboard_status = KB_EVENT;
	...
	if (kr != KERN_SUCCESS) {
		MACH3_DEBUG(0, kr,
			    ("kbd_init: "
			     "device_set_state(KBSKBDMODE, KB_EVENT)"));
		panic("kbd_init: couldn't set keyboard in event mode");
	}
#endif	/* CONFIG_OSFMACH3 */
...
#ifdef	CONFIG_OSFMACH3
	/*
	 * Lauch a thread to read from the Mach device and simulate
	 * keyboard interrupts.
	 */
