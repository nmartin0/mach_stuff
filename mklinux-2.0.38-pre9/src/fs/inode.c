...
repeat:
	...
	if (!inode->i_nlink) {
		...
		/* inode was deleted: uncache its memory object */
		imo = inode->i_mem_object;
		if (imo && imo->imo_cacheable) {
...
#ifdef	CONFIG_OSFMACH3
	if (!best) {
		/* try to recover some unused but cached inodes */
		inode = first_inode;
		for (i = nr_inodes; i > 0; i--) {
			if (inode->i_count == 1 &&
			    inode->i_mem_object &&
			    inode->i_mem_object->imo_cacheable) {
				inode_pager_uncache_now(inode);
				/* let the pager release the inode ... */
				osfmach3_yield();
				/* ... and try again */
				goto repeat;
			}
		...
		/* no inode could be uncached, we're really out of inodes ... */
		if (nr_free_inodes > 0) {
			goto repeat;
		}
	}
