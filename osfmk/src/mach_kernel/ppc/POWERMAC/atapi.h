...
#define AR_DATA         0x00             /* RW - data register (16 bits) */
#define AR_ERROR        0x10             /*  R - error register */
#define AR_FEATURES     0x10             /*  W - features */
#define AR_IREASON      0x20             /* RW - interrupt reason */
#define AR_TAG          0x30             /*    - reserved for SAM TAG byte */
#define AR_CNTLO        0x40             /* RW - byte count, low byte */
#define AR_CNTHI        0x50             /* RW - byte count, high byte */
#define AR_DRIVE        0x60             /* RW - drive select */
#define AR_COMMAND      0x70             /*  W - command register */
#define AR_STATUS       0x70             /*  R - immediate status */
#else
#define AR_ISPB(y,n) ((powermac_info.class == POWERMAC_CLASS_POWERBOOK) ? y : n)
#define AR_DATA       AR_ISPB(0x00,0x00)   /* RW - data register (16 bits) */
#define AR_ERROR      AR_ISPB(0x04,0x10)   /*  R - error register */
#define AR_FEATURES   AR_ISPB(0x04,0x10)   /*  W - features */
#define AR_IREASON    AR_ISPB(0x08,0x20)   /* RW - interrupt reason */
#define AR_TAG        AR_ISPB(0x0c,0x30)   /*    - reserved for SAM TAG byte */
#define AR_CNTLO      AR_ISPB(0x10,0x40)   /* RW - byte count, low byte */
#define AR_CNTHI      AR_ISPB(0x14,0x50)   /* RW - byte count, high byte */
#define AR_DRIVE      AR_ISPB(0x18,0x60)   /* RW - drive select */
#define AR_COMMAND    AR_ISPB(0x1c,0x70)   /*  W - command register */
#define AR_STATUS     AR_ISPB(0x1c,0x70)   /*  R - immediate status */
#endif
#endif
...
struct atapicmd {                       /* ATAPI command block */
	...
	decl_simple_lock_data(,sleep_lock);
	int              busy;          /* busy flag */
...
struct atapi {                          /* ATAPI controller data */
	unsigned long	port;          /* i/o port base */
...
extern struct atapi atapitab[];         /* the table of atapi controllers */
