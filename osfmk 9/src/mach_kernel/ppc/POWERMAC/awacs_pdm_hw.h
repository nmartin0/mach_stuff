 ...
 *   Notes:
 *
 *  1.  The hardware uses a serial-bus time multiplexed scheme to communicate
 ...
 *  2.  The CodecControl is used to as a pointer/data reg to access four 12-bit
 ...
 *  3.  The SoundControl is used to set the data rate and indicates the input,
 *      output and status subframes. (See Note 2).
 *  4.  The CodecStatus indicate when there was an error, or a change of input
 *      or output sources, i.e. a mic, line, or headphones were plugged in.
 *  5.  The ClippingCount  is incremented on a left/right channel overflow. It
...
struct _awacs_regmap_t
{
	...
	uchar_t					pad1;
	volatile uchar_t			BufferSize[2];
	ushort_t				pad2;
	volatile uchar_t			Phase[4];
...
enum AWACsCODEC_ControlRegisterGeneralConstants 
{
	...
	kCodecCtlBusy		=	0x00800000, 		/*AWACS busy bit */    
...
enum AWACsCODEC_ControlRegister0Constants 
{
	...
	kLineInput			=	0x00000800,	/*Bit 11 = audio line input*/
...
enum AWACsBufferSizeRegisterConstants
{
	kBufferSizeMask			=	0x7FC,	/* Buffer size data bits */
	kDefaultBufferSize		=	0x400,	/* Got this value from MacOS :P */
	kMaxBufferSize			=	kBufferSizeMask
};

enum AWACSDMAOutControlRegisterConstants
{
	DMA_IF1	= 0x80,		/* Buf1 interrupt status, set to clear */
	DMA_IF0	= 0x40,		/* Buf0 interrupt status, set to clear */
	DMA_ERR	= 0x20,		/* Buffer overrun? set to clear */
	DMA_STAT= 0x10,		/* ?? */
	DMA_IE0	= 0x08,		/* HW buf0 interrupt enable bit */
	DMA_IE1 = 0x04,		/* HW buf1 interrupt enable bit */
	DMA_IER = 0x02,		/* ?? */
	DMA_LSB = 0x01		/* DMAOut LSB mask.  Not used */
};
