...
/*
** Define the Type-dependent Register Set for the Viper memory controller.
** This structure maps over iomod's `priv_trs' array (see "iomod.h").
**
** First we set up all the sub-structures, followed by the `vi_trs'.
*/

extern	struct iomod *viper;	/* Viper memory controller (from "busconf.c") */

#define	VI_TRS	((struct vi_trs *)viper->priv_trs)
 ...
 */
struct vi_ctrl {		/* (WO) */
	vu_int	vsc_tout:13,	/* VSC clocks to wait before buserr timeout */
			: 1,
...
struct vi_stat {		/* (RO) */
	vu_int	hw_rev	:24,	/* Viper hardware revision (24 bits!) */
 ...
 */
struct vi_trs {
	vu_int		vi_control;	/* PAGE0->pz_Pdep.pd_Viper.v_Ctrlcpy */
 ...
 */
struct vnu_ctl {
	vu_int	sdt_msk	:16,	/* screen door transparancy mask */
			: 6,
...
union vnu_prec {		/* 16 or 24 bit precision */
	struct {
		vu_int	zero1;		/* must be zero */
		vu_int	intg	:16,	/* integer part (16 bits) */
			frac	:12,	/* fractional part (12 bits) */
			zero2	: 4;	/* must be zero */
	} prec16;
	struct {
		vu_int	frac_lo	: 4,	/* fractional part (lower 4 bits) */
			zero1	:28;	/* must be zero */
		vu_int	intg	:24,	/* integer part (24 bits) */
...
struct vn_user {
	...
	vu_int		vnu_spancnt;	/* span count (13 bits, signed) */
	vu_int		vnu_graddr;	/* graphics address (24 bits: 6-29) */
	vu_int		vnu_resv2;
	union vnu_prec	vnu_zslope;	/* Z Slope */
	union vnu_prec	vnu_z;		/* Z */
	vu_int		vnu_resv3[8];
	vu_int		vnu_bslope;	/* Blue Slope (12-19:int, 20-31:fra) */
	vu_int		vnu_bcolor;	/* Blue Color (12-19:int, 20-31:fra) */
	vu_int		vnu_resv4[2];
	vu_int		vnu_rslope;	/* Red Slope (12-19:int, 20-31:fra) */
	vu_int		vnu_rcolor;	/* Red Color (12-19:int, 20-31:fra) */
	vu_int		vnu_resv5[2];
	vu_int		vnu_gslope;	/* Green Slope (12-19:int, 20-31:fra) */
	vu_int		vnu_gcolor;	/* Green Color (12-19:int, 20-31:fra) */
};
...
struct vn_supr {
	...
	vu_int		vns_zaddr;	/* Z Buffer Address (RO) */
};
