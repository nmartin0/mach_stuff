...
/* XXX - temporary */
boolean_t vm_allow_clustered_pagein = TRUE;
...
#define ALIGNED(x) (((x) & (PAGE_SIZE - 1)) == 0)
					 ...
					 * page is provided */
{
	...
	vm_offset_t	cluster_offset;
	vm_offset_t	cluster_start, cluster_end, paging_offset;
	vm_offset_t	align_offset;
	...
	"vm_f_page: access wait acc_req %d, obj 0x%X, offset 0x%X, page 0x%X\n",
			...
			if (object != first_object &&
				...
				if (m->unlock_request != VM_PROT_NONE) {
					...
					XPR(XPR_VM_FAULT,
		  "vm_f_page: CALL unl wait obj 0x%X, offset 0x%X, page 0x%X\n",
						(integer_t)object, offset,
						(integer_t)m, 0, 0);
					/* take an extra ref */
					assert(object->ref_count > 0);
				...
				if (m->lock_supplied &&
					...
					XPR(XPR_VM_FAULT,
		"vm_f_page: call retry_on_w obj 0x%X, offset 0x%X, page 0x%X\n",
						(integer_t)object, offset,
						(integer_t)m, 0, 0);
					return(VM_FAULT_RETRY);
				}
		...
		if (look_for_page && !must_be_resident) {
			...
			align_offset = paging_offset&(PAGE_SIZE-1);
			if (align_offset != 0) {
				cluster_offset = trunc_page(cluster_offset);
			}

#define SPANS_CLUSTER(x) ((((x) - align_offset) & (cluster_size - 1)) == 0)
			...
			if (((vm_default_behind != 0 && 
				...
				if ((cluster_bot < offset) &&
					...
					cluster_bot += PAGE_SIZE;
				}
			    }
			    if (behavior == VM_BEHAVIOR_DEFAULT) {
				vm_offset_t bot = vm_default_behind*PAGE_SIZE;
			    ...
			    for ( cluster_start = offset - PAGE_SIZE;
				 ...
				 (cluster_start != (align_offset - PAGE_SIZE));
...
retry_cluster_backw:
			...
			if ((behavior == VM_BEHAVIOR_DEFAULT && 
			    ...
			    cluster_top = (offset + cluster_size) - 
					  cluster_offset;
			    if (align_offset != 0) {
				if ((cluster_top > (offset + PAGE_SIZE)) &&
				    SPANS_CLUSTER(cluster_top)) {
					cluster_top -= PAGE_SIZE;
				}
			    }
			    if (behavior == VM_BEHAVIOR_DEFAULT) {
				vm_offset_t top = (vm_default_ahead*PAGE_SIZE)+
							PAGE_SIZE;
	...
	if (object->copy_strategy == MEMORY_OBJECT_COPY_CALL) {
		...
		if ((fault_type & VM_PROT_WRITE) == 0) {
			*protection &= ~VM_PROT_WRITE;
			...
			vm_object_unlock(object);
			XPR(XPR_VM_FAULT,
		"vm_f_page: CALL unlock obj 0x%X, offset 0x%X, page 0x%X, unl_req %d\n",
				(integer_t)object, offset,
				(integer_t)m, new_unlock_request, 0);
	...
	*top_page = first_m;
	...
	if (vm_page_deactivate_behind) {
		if (offset && /* don't underflow */
			(object->last_alloc == (offset - PAGE_SIZE))) {
	...
	boolean_t	interruptible)
{
	 ...
	 */
	vm_offset_t		src_lo_offset = trunc_page(src_offset);
	vm_offset_t		dst_lo_offset = trunc_page(dst_offset);
	vm_offset_t		src_hi_offset = round_page(src_offset + *src_size);
	vm_offset_t		dst_hi_offset = round_page(dst_offset + *src_size);
