...
/* SYSCALL structures are marked by setting a reserved bit in the MSR,
 * this bit should never leak into executing code, it'll be masked off
 * beforehand. We don't use bit 0, since it leads to sign problems.
 */

#define MSR_SYSCALL_MASK		MASK(MSR_RES1)
#define MSR_MARK_SYSCALL(msr) 		(msr | (MSR_SYSCALL_MASK))
#define MSR_WAS_SYSCALL(msr) 		(msr & MSR_SYSCALL_MASK)
#define MSR_REMOVE_SYSCALL_MARK(msr) 	(msr & ~(MSR_SYSCALL_MASK))
...
/* This segment register is used for copyin/copyout+remapping and must be
 * saved and restored in the thread context.
 */
#define SR_COPYIN	12
#define SR_COPYIN_NAME	sr12

/* This segment register is unused by the kernel, and thus contains
...
#define PROCESSOR_VERSION (mfpvr() >> 16)
...
/* TODO NMGS 601v 603ev etc. etc. as and when needed */
...
#define mtsr(SR, REG)							     \
	__asm__ volatile("sync;mtsr %0, %1; isync" : : "i" (SR), "r" (REG)); \
...
{
        __asm__ volatile("sync;mtsrin %0, %1;isync" : : "r" (val), "r" (reg));
        return;
}
...
{
        int result;
        __asm__ volatile("isync;mfdec %0" : "=r" (result));
        return result;
}
