...
/*
 * POWERMAC I/O various BUS extension boards addresses:
 *	Each range of 8 consecutive address is mapped into an unique 4K page
 */
#ifdef notdef	/* not yet included */
...
MACRO_BEGIN								\
	if (IOBUS_TYPE_FLAGS(iotype) & IOBUS_TYPE_FLAGS_NOTCOHERENT)	\
	    flush_dcache((vm_offset_t)(addr), (vm_size_t)(len), FALSE);	\
MACRO_END

#define	IOBUS_PURGE_DCACHE(iotype, addr, len)				\
MACRO_BEGIN								\
	if (IOBUS_TYPE_FLAGS(iotype) & IOBUS_TYPE_FLAGS_NOTCOHERENT)	\
	    invalidate_dcache((vm_offset_t)(addr), (vm_size_t)(len), FALSE);\
MACRO_END
