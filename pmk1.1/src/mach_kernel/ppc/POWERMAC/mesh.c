...
#define	STATE_REBUILD_SG	0x100	/* have to rebuild sglist.. */
...
#define MESH_DBDMA_BUFFERS	MESH_MAX_SG_SEGS+1
...
void mesh_construct_entry(mesh_state_t, vm_offset_t, long, long);
	...
	void		*ptr)
{
	...
	sc->max_dma_segs = MESH_MAX_SG_SEGS;

	mesh->m_current = NULL;
	mesh->m_dbdma_buffer = dbdma_alloc(MESH_DBDMA_BUFFERS);
...
{
	vm_offset_t	address;
	unsigned long	count, seg_count, i;
	unsigned char 	cmd;
	...
	if (state->s_flags & STATE_REBUILD_SG) {
		mesh_get_io_segment(mesh, state, &sgp, &seg_count,
					&address, &count);

		for (i = 0; i < seg_count; i++, dmap++, sgp++) {
			if (i) {
				count = sgp->sg_count;
			...
			if (isread)
				invalidate_cache_for_io(address, count, TRUE);
			else
				flush_dcache(address, count, TRUE);

			if (i == seg_count-1) {
				DBDMA_BUILD(dmap,
					isread ?  DBDMA_CMD_IN_LAST : DBDMA_CMD_OUT_LAST,
					0, count, address, DBDMA_INT_NEVER, DBDMA_BRANCH_NEVER,
					DBDMA_WAIT_NEVER);
			} else {
				DBDMA_BUILD(dmap,
		...
		state->s_flags &= ~STATE_REBUILD_SG;
	...
	count = state->s_residual;

	MESH_TRACE2("data xfer %s %d", isread ? "read" : "write",
			count);

	MESH_SET_XFER(regs, count);
...
mesh_end_transfer(mesh_softc_t mesh, mesh_regmap_t regs, mesh_state_t state)
{
	...
	if (isread) {
		if (regs->r_fifo_cnt)  {
			mesh_DumpLog("FIFO");
			printf("** FIFO RESIDUAL COUNT %d\n", regs->r_fifo_cnt);
		}

		/* Check to see if there is any bytes left over.. */
...
void __inline__ 
mesh_construct_entry(mesh_state_t st,
...
mesh_build_sglist(mesh_softc_t mesh, mesh_state_t st, target_info_t *tgt)
{
	...
	if ((st->s_flags & STATE_CHAINED_IO) == 0) 
		...
		for (ior = tgt->ior; ior; ior = ior->io_link) {
			...
			mesh_construct_entry(st, (vm_offset_t) ior->io_data, count, offset);
			offset += count;
		}
	}

	
	for (i = 0; i < st->s_segment_count; i++, dmap++, sgp++) {
		DBDMA_BUILD(dmap,
			isread ?  DBDMA_CMD_IN_MORE : DBDMA_CMD_OUT_MORE,
			0, sgp->sg_count, sgp->sg_physaddr,
		...
		unsigned long *count)
{
	...
	*count = st->s_residual;
	...
	if (offset > sgp->sg_offset) {
		...
		if (st->s_index >= st->s_segment_count)  {
#if DEBUG
			mesh_DumpLog("SCSI DMA error");
#endif
			panic("SCSI DMA error : s_index > sg_segment_count");
		}
