...
/* CMU_HIST */
/*
 * Revision 2.14.2.1  92/03/28  10:14:58  jeffreyh
 * 	Pick up changes from MK71
 * 	[92/03/20  13:31:03  jeffreyh]
 * 
 * Revision 2.15  92/02/23  22:43:55  elf
 ...
 * 
 * Revision 2.14  91/10/09  16:15:50  af
 ...
 * 	[91/09/18            rpd]
 * 
 * Revision 2.13  91/08/24  12:26:00  af
 ...
 * 
 * Revision 2.12  91/06/25  20:55:51  rpd
 * 	Tweaks to make gcc happy.
 * 
 * Revision 2.11  91/06/19  16:29:16  rvb
 * 	File moved here from mips/PMAX since it is now "MI" code, also
 * 	used by Vax3100 and soon -- the omron luna88k.
 * 	[91/06/04            rvb]
 * 
 * Revision 2.10  91/05/14  17:29:24  mrt
 * 	Correcting copyright
 * 
 * Revision 2.9  91/05/13  06:05:20  af
 ...
 * 
 * Revision 2.7.2.3  91/04/05  13:22:43  af
 * 	Removed restriction on max read/write xfers, with noticeable
 * 	performance benefits.
 * 	Handle odd-byte-boundary conditions, unified dma_out routine,
 * 	removed useless counters but kept a watchout on sii_wait,
 ...
 * 	the fifo, but how can I clean it ?].
 * 
 * Revision 2.7.2.2  91/03/30  12:29:26  af
 * 	Fixed one more race in sii_dosynch().
 * 	[91/03/30            af]
 * 
 * Revision 2.7.2.1  91/03/29  16:56:47  af
 * 	Added disconnect-reconnect capability.  Tapes work.
 * 	Also:  added watchdog() to cope with hitachi disk problems,
 * 	rid of thread_block()ing and do proper scsi bus allocation
 * 	among multiple targets in FIFO order, fixed the way we do
	...
	struct bus_ctlr	*ui;
{
	...
	for (target_id = 0, dma_ptr = (char*)sii->buff;
		...
		dsr = sii_wait(&regs->sii_data_csr, SCSI_PHASE_STATUS);
		csr = regs->sii_conn_csr;
		SII_ACK(regs,csr,dsr,0);
		...
		dsr = sii_wait(&regs->sii_data_csr, SCSI_PHASE_MSG_IN);
		csr = regs->sii_conn_csr;
	...
	register sii_softc_t	sii;
{
	...
	ds = sii_wait(&regs->sii_data_csr, SII_DTR_IBF);
	...
	ds = sii_wait(&regs->sii_data_csr, SII_DTR_DONE);
	regs->sii_data_csr = SII_DTR_DONE;
...

{
	...
	/* tk50s are slow */
	if ((cs & SII_CON_CON) == 0)
		cs = sii_wait(&regs->sii_conn_csr, SII_CON_CON);
	...
	(void) sii_wait(&regs->sii_data_csr, SII_DTR_DONE);
	regs->sii_data_csr = SII_DTR_DONE;
	...
	register sii_softc_t	sii;
{
	...
	ds = sii_wait(&regs->sii_data_csr, SII_DTR_DI);
	...
	ds = sii_wait(&regs->sii_data_csr, SCSI_PHASE_MSG_IN);
...
msgin:
	...
	ds = sii_wait(&regs->sii_data_csr, SII_DTR_MIS);
