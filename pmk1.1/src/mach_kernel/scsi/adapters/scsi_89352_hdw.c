...
/* CMU_HIST */
/*
 * Revision 2.4.2.1  92/03/28  10:15:08  jeffreyh
 * 	Pick up changes from MK71
 * 	[92/03/20  13:31:16  jeffreyh]
 * 
 * Revision 2.6  92/02/25  14:19:31  elf
 * 	Corrected typo.
 * 	[92/02/24            danner]
 * 
 * Revision 2.5  92/02/23  22:44:03  elf
 ...
 * 
 * Revision 2.4  91/08/24  12:27:10  af
 * 	Reflected changes in MI code
 * 	[af]
 * 	Changed hardware header from sd.h to scsi.h
 * 	[91/07/13            danner]
 * 
 * Revision 2.3.3.1  91/08/19  13:48:07  danner
 * 	Changed hardware header from sd.h to scsi.h
 * 	[91/07/13            danner]
 * 
 * Revision 2.3  91/07/11  11:01:31  danner
 * 	Copyright Fixes
 * 
 * Revision 2.2  91/07/09  23:22:16  danner
...
int	spc_probe(), scsi_slave();
void	spc_intr();
void    scsi_attach(), spc_go();

caddr_t	spc_std[NSCSI] = { SPC_DEFAULT_ADDRESS };
	...
	struct bus_ctlr	*ui;
{
	...
	simple_lock_init(&spc->chiplock, ETAP_IO_CHIP);
 ...
 */
void spc_go(tgt, cmd_count, in_count, cmd_only)
