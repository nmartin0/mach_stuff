...
#define FRAME_PCB_TO_STACK(thread)					;\
	...
	cmpl	kernel_pmap, %ecx	/* If kernel loaded task */	;\
...
Entry(mach_rpc_return_wrapper)
	...
	movl	%gs:(%eax),%eax			/ get active thread
...
3:
	decl	%edx
	je	4f			/ ...till max reached
	cmpl	$0,%eax
	jne	2b			/ ...or till a NUL found
4:
 ...
 */
#if	NCPUS > 1 && defined(CBUS) && MACH_ASSERT
#define	ILL_ON_SLAVE			\
	CPU_NUMBER(%eax)	;	\
	cmpl	$0, %eax	;	\
	je	0f		;	\
...
0:
#else	/* NCPUS > 1 && defined(CBUS) */
#define	ILL_ON_SLAVE
#endif	/* NCPUS > 1 && defined(CBUS) && MACH_ASSERT */
