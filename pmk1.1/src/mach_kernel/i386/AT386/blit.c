...
/* CMU_HIST */
/*
 * Revision 2.8  91/05/14  16:19:57  mrt
 * 	Correcting copyright
 * 
 * Revision 2.7  91/03/16  14:45:38  rpd
 * 	Updated for new kmem_alloc interface.
 * 	[91/03/03            rpd]
 * 
 * Revision 2.6  91/02/05  17:16:04  mrt
 * 	Changed to new Mach copyright
 * 	[91/02/01  17:41:44  mrt]
 * 
 * Revision 2.5  90/12/20  16:37:26  jeffreyh
 * 	Changes for __STDC__
 * 	[90/12/07            jeffreyh]
 * 
 * Revision 2.4  90/11/26  14:49:00  rvb
 * 	jsb bet me to XMK34, sigh ...
 * 	[90/11/26            rvb]
 * 	Synched 2.5 & 3.0 at I386q (r1.8.1.5) & XMK35 (r2.4)
 * 	[90/11/15            rvb]
 * 
 * Revision 2.3  90/08/27  21:58:46  dbg
 * 	Fix Intel Copyright as per B. Davies authorization.
 * 	[90/08/14            dbg]
 * 
 * Revision 1.8.1.4  90/06/07  08:05:24  rvb
 * 	Fix to prevent obnoxious flickering.	[kupfer]
 * 
 * Revision 2.2  90/05/03  15:40:41  dbg
 * 	Convert for pure kernel.
 * 	[90/04/27            dbg]
 * 
 * Revision 1.8.1.3  90/02/28  15:48:54  rvb
 * 	Fix numerous typo's in Olivetti disclaimer.
 * 	[90/02/28            rvb]
 * 
 * Revision 1.8.1.2  90/01/08  13:32:23  rvb
 * 	Add Intel copyright.
 * 	Add Olivetti copyright.
 * 	[90/01/08            rvb]
 * 
 * Revision 1.8.1.1  89/10/22  11:33:45  rvb
 * 	Received from Intel October 13, 1989.
 * 
 * 	Revision 1.14  89/09/27  16:02:37  kupfer
 * 	Add support for resetting display before reboot.
 * 	[89/10/13            rvb]
 * 
 * Revision 1.13  89/09/21  12:05:32  lance
 * X108 checkin
 * 
 * Revision 1.8  89/09/20  17:27:14  rvb
 * 	Revision 1.12  89/07/26  22:38:08  kupfer
 * 	Merge in support for user programs that want to use the 786.
 * 
 * 	Revision 1.11  89/07/10  15:52:10  kupfer
 * 	Fold in the branch with code to better track boot status.
 * 
 * Revision 1.10  89/07/07  17:56:06  kupfer
 * X79 merge, 2nd attempt.
 * 
 * Revision 1.5  89/04/05  13:00:37  rvb
 * 	Forward Declarations for gcc.
 * 	[89/03/21            rvb]
 * 
 * Revision 1.4  89/03/09  20:04:46  rpd
 * 	More cleanup.
 * 
 * Revision 1.3  89/02/26  12:40:54  gm0w
 * 	Changes for cleanup.
 * 
 */
/* CMU_ENDHIST */
...
/* file in a tile descriptor */
extern void		tiledesc(
				volatile TILEDESC	* tile,
				int			x,
				int			y,
				int			w,
				int			ww,
				addr786_t		adx,
				int			bpp);
/* load Display Processor Control Registers */
extern void		loadall(
				struct blitdev	* blt,
...
#if	AUTOINIT
/* initialize 786 */
extern int		blitattach(
				struct mb_device	* md);
/* probe for existence of board */
extern int		blitprobe(
				caddr_t		reg,
				int		unit);
#endif
/* do ioctl operation */
extern int		blitioctl(
				dev_t		dev,
				int		cmd,
				caddr_t		data,
				int		flag);
/* prepare for reboot */
extern void		blitreboot(void);
/* set status indication */
extern void		setstatus(
				int		val);
/* allocate blit memory */
extern void		blit_memory_init(
				struct blitsoft	* bs);
/* reset board */
extern void		blit_reset_board(void);
/* initialize BIU */
extern void		init_biu(
				struct blitdev	* blt);
/* initialize Display Processor */
extern void		init_dp(
				struct blitsoft	* bs);
/* map board into kernel VM */
extern struct blitdev *	mapblit(
				caddr_t		physaddr,
				int		length);
/* free board memory */
extern void		freeblit(
				vm_offset_t	va,
				int		length);
/* blit rom existence check */
extern boolean_t	blit_rom_ok(
				struct blitdev	* blt);
/* get font info */
extern void		getfontinfo(
				struct blitdev	* blt);
/* pick cursor size */
extern short		pick_cursor_height(void);
/* clear screen */
extern void		clear_blit(
				struct blitdev	* blt);
...
void
tiledesc(
	volatile TILEDESC *tile,	/* pointer to tile descriptor */
