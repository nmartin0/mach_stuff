...
#if	MACH_HOST
extern __inline__ boolean_t
check_processor_set(thread_t thread)
{
	boolean_t	retval;

	mp_disable_preemption();
	retval = current_processor()->processor_set == (thread)->processor_set;
...
#if	NCPUS > 1
extern __inline__ boolean_t
check_bound_processor(thread_t thread)
{
	boolean_t 	retval;

	mp_disable_preemption();
	retval = ((thread)->bound_processor == PROCESSOR_NULL ||
...
thread_handoff(
	...
	register void		(*continuation)(void),
	register thread_t	new)
{
	 ...
	 *	of the new thread, to ensure we can handoff.
	 *	This check uses current_processor()->processor_set,
	 *	which we can read without locking.
	 *
	 *	Because the stack-privileged threads (pageout, swapin,
	 *	idle, and io-done) never call thread_handoff,
	 *	we don't have to check stack_privilege.
	 */
