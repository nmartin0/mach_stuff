...
const struct seg_desc Gdt[NGDTENT] = {
	...
	DESC(STACKBASE, LIM16, DATA, SEG32, BYTES), /* 0x30 : stack segment */
	DESC(STACKBASE, LIM16, DATA, SEG16, BYTES)  /* 0x38 : stack segment */
};

/* Gdt is dynamically loaded by a pseudo-descriptor */
struct pseudo_desc {
	int	limit:16;
	int	base_low:16;
	int	base_high:16;
...
#define GDTLIMIT	(NGDTENT* SEG_DESC_SIZE) /* -1 to be correct */


struct pseudo_desc Gdtr = { GDTLIMIT-1, 0, 0 };

/*
 * IDT pseudo-desc is reloaded in the LINUX boot code at
 * address 0 (start address of the real-mode interrupt vectors).
 */
struct pseudo_desc Idtr = {0xffff, 0, 0};
