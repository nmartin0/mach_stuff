 ...
 */
/*
 * Stand-alone BEXT2FS file reading package.
 */

#include <bext2fs/bext2fs.h>
#include "externs.h"
...
struct fs_ops bext2fs_ops = {
	bext2fs_open_file,
	bext2fs_close_file,
	bext2fs_read_file,
	bext2fs_file_size,
	bext2fs_file_is_directory,
	bext2fs_file_is_executable
};
 ...
 */
static void
free_file_buffers(register struct bext2fs_file *fp)
{
	register int level;
 ...
 */
static int
read_inode(ino_t inumber, register struct bext2fs_file *fp)
{
	vm_offset_t		buf;
...
#endif 
	fs = fp->f_fs;
	disk_block = bext2_ino2blk(fs, fp->f_gd, inumber);

	rc = device_read(fp->f_dev.dev_port,
			 0,
			 (recnum_t) dbtorec(&fp->f_dev,
					    bext2_fsbtodb(fp->f_fs, disk_block)),
			 (int) BEXT2_BLOCK_SIZE(fs),
			 (char **)&buf,
			 &buf_size);
	if (rc != KERN_SUCCESS)
	    return (rc);
	...
	{
	    register struct bext2_inode *dp;

	    dp = (struct bext2_inode *)buf;
	    dp += bext2_itoo(fs, inumber);
	    fp->i_ic = *dp;
	}
	...
	daddr_t			*disk_block_p)	/* out */
{
	...
	for (; level >= 0; level--) {
		...
		rc = device_read(fp->f_dev.dev_port,
				 0,
				 (recnum_t) dbtorec(&fp->f_dev,
					    bext2_fsbtodb(fp->f_fs, ind_block_num)),
				 BEXT2_BLOCK_SIZE(fp->f_fs),
				 (char **)&data,
				 &size);
		if (rc != KERN_SUCCESS)
		    return (rc);
...
static int
buf_read_file(
	register struct bext2fs_file	*fp,
	vm_offset_t			offset,
	...
	vm_size_t			*size_p)	/* out */
{
	...
	off = bext2_blkoff(fs, offset);
	file_block = bext2_lblkno(fs, offset);
	block_size = bext2_blksize(fs, fp, file_block);
	...
	ino_t *inumber_p)
{
	...
	vm_size_t	buf_size;
	vm_offset_t	offset;
	register struct bext2_dir_entry *dp;
	int		length;
	kern_return_t	rc;
	char		tmp_name[256];
...
read_fs(
	struct device *dev,
	struct bext2_super_block **fsp,
	struct bext2_group_desc  **gdp,
	vm_size_t		*gd_size_p)
{
	register
	struct bext2_super_block *fs;
	 ...
	 */
	fs = (struct bext2_super_block *)buf;
	if (fs->s_magic != BEXT2_SUPER_MAGIC) {
		(void) vm_deallocate(mach_task_self(), buf, buf_size);
	...
	*fsp = fs;
	...
	gd_blocks = (gd_count + BEXT2_DESC_PER_BLOCK(fs) - 1) /
		    BEXT2_DESC_PER_BLOCK(fs);
	gd_size = gd_blocks * BEXT2_BLOCK_SIZE(fs);
	gd_location = fs->s_first_data_block + 1;
	gd_sector = (gd_location * BEXT2_BLOCK_SIZE(fs)) / DEV_BSIZE;
	...
	*gdp = (struct bext2_group_desc *) buf2;
	*gd_size_p = gd_size;
...
static int
mount_fs(register struct bext2fs_file	*fp)
{
	register struct bext2_super_block *fs;
	int error;
...
static void
unmount_fs(register struct bext2fs_file *fp)
{
...
#define	RETURN(code)	{ rc = (code); goto exit; }
	...
	fp = (struct bext2fs_file *)malloc(sizeof(struct bext2fs_file));
	memset((void *)fp, '\0', sizeof(struct bext2fs_file));
	fp->f_dev = *dev;
	/*
	...
	while (*cp) {
	    ...
	    *cp = c;
...
#endif	/* IC_FASTLINK */
		{
		    ...
		    mach_msg_type_number_t	buf_size;
		    daddr_t	disk_block;
		    register struct bext2_super_block *fs = fp->f_fs;
	...
	*private = (fs_private_t) fp;
    ...
    exit:
	free(namebuf);
	bext2fs_close_file((fs_private_t)fp);
	return rc;
}
...
bext2fs_close_file(
	fs_private_t private)
{
	register struct bext2fs_file *fp = (struct bext2fs_file *)private;
...
boolean_t
bext2fs_file_is_directory(fs_private_t private)
{
  	register struct bext2fs_file	*fp = (struct bext2fs_file *)private;
...
size_t
bext2fs_file_size(fs_private_t private)
{
  	register struct bext2fs_file	*fp = (struct bext2fs_file *)private;
...
boolean_t
bext2fs_file_is_executable(fs_private_t private)
{
  	register struct bext2fs_file	*fp = (struct bext2fs_file *)private;
