...
#if 1
# define ERROR(code, str)	(hfs_error = (char *)(str), errno = (code))
#else
...
struct _hfsvol_ {
#if 0
  int fd;		/* volume's open file descriptor--not used in mach */
#endif
  int flags;		/* bit flags */

  struct device f_dev;  /* Mach device */

  char		*cache_data;	/* Buffer cache.. */
