...
#define BREAK_MAX 50
typedef struct {
    u_int       addr;                     /* Address of where BPT is */
...
*/
static int
tgdb_break_installed(u_int addr)
...
*/
static void
tgdb_add_breakpoint(u_int addr, char *inst)
...
*/
static int
tgdb_check_breakpoint(mach_port_t task, u_int addr, unsigned char *data)
...
*/
static int
tgdb_fix_breakpoint(mach_port_t task, u_int addr)
...
tgdb_attach(unsigned int task_id)
{
	...
	if (task_set_exception_ports(
		...
		printf("tgdb: can't set exception port\n");
		tgdb_deallocate_session(session);
		simple_unlock(&session->lock);
		return 0;
	}
		      ...
		      exception_data_t codes, mach_msg_type_number_t codeCnt)
{
	...
	if (exception != EXC_BREAKPOINT) {
		printf("tgdb: caught non-breakpoint exception (%d)\n",
		       exception);
	}
	...
	gdb_response_t response)
{
	...
	if (request->size > sizeof (int))
		thread_offset = *(((int *)(void *)request->data)+1);
	...
	if (session->thread_count-thread_offset < threads)
	  threads = session->thread_count-thread_offset;
...
tgdb_request(gdb_request_t request, gdb_response_t response)
{
	...
	case 'g':		/* read registers */
		...
		hp700ts_to_gdb(&ts, &fs, (void *) response->data);
		break;

	case 'G':		/* write registers */
		assert(request->thread_id < session->thread_count);
		session->current_thread = session->thread[request->thread_id];
		gdb_to_hp700ts((void *) request->data, &ts, &fs);
