...
/* Revision 1.3  1999/07/28 00:08:40  dgatwood
/*
/* Okay, the thing refuses to ignore those, so... who cares.  HP700 fixes
/* were contributed by Eric Schnoebelen <eric@cirr.com>.  Maybe one of these
/* days I'll get around to folding in the x86 changes and making sure they
/* don't break things....  Would anyone like to volunteer to do diffs between
/* a handful of files and stock DR3 and then apply them to the tree and
/* make sure the PowerMac port still compiles and boots?  Please?  :-)
/*
/*
/* dgatwood@mvista.com
/*
 * Revision 1.2  1998/11/01 01:16:08  trini
 * Add in patches from Troy Benjegerdes <hozer@drgw.net> for i386
 *    (checked w/ him before hand) - Tom
 *
 * Revision 1.1.1.1  1998/09/28 15:10:08  root
 *
...
/*
 * the driver should also be cleaned, removing the requirement for 
 * the <mach_glue.h> include file.  Aka, the driver should be ported
 * to purely MACH semantics, and not require the BSD glue layer
 * (or maybe not.. :-)
 */
...
#include <hp_pa/HP700/mach_glue.h>
...
unsigned long	ncr53c700_byteswap(
				   unsigned long);
...
unsigned long
ncr53c700_byteswap(unsigned long  l)
{
#if	hp_pa
	register int _ov = (l), _nv;
	...
	return(_nv);
        ...
        x.l = l;
        for (j = 0, i = sizeof(x.c)-1; i >= 0; --i, ++j)
                y.c[j] = x.c[i];
        return(y.l);
#endif
}
...
#define C700_MAX_INTR_LOOP	20
	...
	if (scp->wd.nactive)
	    ...
	    if (it) {
                bzero((const char *)it, sizeof(struct itrace)); 
...
ncr53c700_attach(hp_ctlrinfo_t *ctlrinfop)
{
	...
	if ((cdata = (struct controller_data *) kalloc(
        ...
        scp->info = cdata;
        bzero((const char *)cdata, sizeof(*cdata));
        cdata->csr = csr;
	cdata->ctype = ctype;
	...
	target_info_t		*tgt)
{
	...
	if ((cmd = (struct scsi_cmd *)kalloc(sizeof(struct scsi_cmd))) == 0)
	  	goto nomem;
	bzero((const char *)cmd, sizeof(struct scsi_cmd));
	cmd->tgt = tgt;
	...
	if ((tdata = (struct target_data *)kalloc(
	  	...
	  	goto nomem;
 	bzero((const char *)tdata, sizeof(struct target_data));
