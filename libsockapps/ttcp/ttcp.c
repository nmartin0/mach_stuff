...
char **argv;
{
	 ...
	 */
	{
	  extern void network_init(char *);
	  extern int libnet_verbose;
	  
	  libnet_verbose = 0;
	  network_init("tcpclient.conf");
	}
...
#if defined(SO_RCVBUF) || defined(SO_SNDBUF) /* XXX ??? CMU ??? XXX */
		/*
		 * fd is new, so have to set sockbuf size again
		 */
		if (sockbufsize) {
		  int sbsize, reslen;

		  if (setsockopt(fd,SOL_SOCKET,SO_RCVBUF,
				 &sockbufsize,sizeof(int)) < 0)
		    err("setsockopt");

		  /* make sure we got it */
		  reslen = sizeof(sbsize);
		  if (getsockopt(fd,SOL_SOCKET,SO_RCVBUF,
				 &sbsize,&reslen) < 0)
		    err("getsockopt");
		  fprintf(stderr, "ttcp-r: rcvbuf size: %d\n",
			  sbsize);
		}
#endif
