 ...
 * HISTORY
 * $Log:	do_copyarea.c,v $
 * Revision 5.1.2.2  92/02/27  18:12:28  chs
 * 	Rename the static variable shminfo to xshminfo,
 * 	to avoid a name conflict with the OSF <sys/shm.h>.
 * 	[92/02/26  16:11:03  chs]
 * 
 * $EndLog$
 */
    ...
    int     reps;
{
    ...
    xshminfo.shmid = shmget(IPC_PRIVATE, image_size, IPC_CREAT|0777);
    if (xshminfo.shmid < 0)
    {
	perror ("shmget");
	return False;
    ...
    XShmAttach (xp->d, &xshminfo);
    shm_image.data = xshminfo.shmaddr;
    bcopy (image->data, shm_image.data, image_size);
    shm_image.obdata = (char *) &xshminfo;
    return reps;
}
