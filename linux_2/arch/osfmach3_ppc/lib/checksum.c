...
extern unsigned long _csum_partial(const unsigned char *buff,
				   int len,
				   unsigned int sum);
extern unsigned short _csum_tcpudp_magic(unsigned long saddr,
					 unsigned long daddr,
					 unsigned int sum,
					 unsigned long proto);
...
#ifdef	CONFIG_OSFMACH3
unsigned int csum_partial_copy_fromuser(
	const char *src,
	char *dst,
	int len,
	int sum)
