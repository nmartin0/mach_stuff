...
/* TEMP */
pci_irq_line = 15;
outl(0x0000, pci_ioaddr+LANCE_ADDR);  /* This should put it into 32 bit (DWIO) mode */
reset_val = inl(pci_ioaddr+LANCE_RESET); /* Reset the LANCE */
/* TEMP */
#endif	/* __powerpc__ */
...
#ifdef	__powerpc__
	if ((reset_val = inw(ioaddr+LANCE_DATA)) != 0x0004) {
	    printk("Not reset = %x\n", reset_val);
	}
#else	/* __powerpc__ */
...
#ifdef __powerpc__
lance_version = PCNET_PCI;
#else
	/* Get the version of the chip. */
	outw(88, ioaddr+LANCE_ADDR);
#ifdef	__powerpc__
	if ((reset_val = inw(ioaddr+LANCE_ADDR)) != 88) {
	    printk("Unknown LANCE Version: %x(%d)\n", reset_val, reset_val);
#else	/* __powerpc__ */
