	     ...
	     unsigned int cmd, unsigned long arg)
{
	...
	case KDSKBENT:
	{
		...
		if (i)
			return i;
#if	defined(CONFIG_OSFMACH3) && defined(CONFIG_PMAC_KEYBOARD)
		if ((i = get_user(&a->kb_index)) > NR_KEYS)
			return -EINVAL;
#else	/* CONFIG_OSFMACH3 && CONFIG_PMAC_KEYBOARD */
		if ((i = get_user(&a->kb_index)) >= NR_KEYS)
			return -EINVAL;
#endif	/* CONFIG_OSFMACH3 && CONFIG_PMAC_KEYBOARD */
		if ((s = get_user(&a->kb_table)) >= MAX_NR_KEYMAPS)
			return -EINVAL;
...
#if	defined(CONFIG_OSFMACH3) && defined(CONFIG_PMAC_KEYBOARD)
		/* assignment to entry NR_KEYS only tests validity of args */
		if (i == NR_KEYS)
			return 0;
#else	/* CONFIG_OSFMACH3 && CONFIG_PMAC_KEYBOARD */
	...
	case VC_GETMODE:
		{
#ifdef	CONFIG_OSFMACH3
			extern int console_getmode(struct vc_mode *mode);
#endif	/* CONFIG_OSFMACH3 */
			struct vc_mode mode;
			...
			if (i)
				return i;
			memcpy_tofs((void *) arg, &mode, sizeof(mode));
			return 0;
		}
	case VC_SETMODE:
	case VC_INQMODE:
		{
#ifdef	CONFIG_OSFMACH3
			extern int console_setmode(struct vc_mode *mode,
						   int doit);
#endif	/* CONFIG_OSFMACH3 */
			struct vc_mode mode;
			...
			if (i)
				return i;
			memcpy_fromfs(&mode, (void *) arg, sizeof(mode));
	...
	case VC_GETCMAP:
		{
			extern int console_set_get_cmap(void *cmap, int doset);

			if (!suser())
				return -EPERM;
			return console_set_get_cmap((void *) arg,
						    cmd == VC_SETCMAP);
		}
#endif	/* CONFIG_OSFMACH3 */
#endif	/* CONFIG_PMAC_CONSOLE */
