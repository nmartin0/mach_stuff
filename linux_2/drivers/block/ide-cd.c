...
#ifdef	__powerpc__
	cdrom_start_packet_command (drive, 4096 /*32768*/,
#else	/* __powerpc__ */
...
#ifdef	__powerpc__
	if (stat && 1 /*reqbuf->sense_key == ILLEGAL_REQUEST &&
	    reqbuf->asc == 0x20 &&
  ...
  /* no write access */
	...
	if (drive->usage == 1) {
		...
		/* If things worked ok, lock the door and read the
		   TOC information. */
#ifdef	__powerpc__
printk("CDROM: Lock door & read TOC\n");
#endif	/* __powerpc__ */
