...
static inline void generate(unsigned long sig, struct task_struct * p)
{
	...
	if (p->state == TASK_INTERRUPTIBLE && (p->signal & ~p->blocked))
		wake_up_process(p);
#ifdef	CONFIG_OSFMACH3
	if (p->osfmach3.thread->under_server_control == FALSE &&
	    p->osfmach3.thread->in_interrupt_list == FALSE &&
	    (p->signal & ~p->blocked)) {
		/*
		 * The process is in user mode and there are some
		 * signals to process: generate a fake interrupt
		 * to force the process to check for signals.
		 */
...
void release(struct task_struct * p)
{
	...
	for (i=1 ; i<NR_TASKS ; i++)
		if (task[i] == p) {
#ifdef	CONFIG_OSFMACH3
			while (p->osfmach3.thread->mach_thread_port !=
			       MACH_PORT_NULL) {
				/*
				 * Synchronize with osfmach3_task_terminate().
				 */
				osfmach3_yield();
			}
#endif	/* CONFIG_OSFMACH3 */
...
#ifdef	CONFIG_OSFMACH3
	osfmach3_synchronize_exit(current);
#endif	/* CONFIG_OSFMACH3 */
