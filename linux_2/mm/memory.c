...
int verify_area(int type, const void * addr, unsigned long size)
{
...
#if	defined(CONFIG_OSFMACH3) && defined(STACK_GROWTH_UP)
	unsigned long stack_base;
#endif	/* defined(CONFIG_OSFMACH3) && defined(STACK_GROWTH_UP) */
...
#ifdef	CONFIG_OSFMACH3
	if (current->osfmach3.task->mach_aware) {
		/*
		 * This task can change its VM space using Mach services,
		 * so our memory map is probably out of date.
		 * Let's ask Mach if this memory is valid.
		 */
		return user_memory_verify_area(type, addr, size);
	}
#endif	/* CONFIG_OSFMACH3 */
	...
	if (start >= stack_base) {
		if(vma = find_vma_intersection(current->mm,
					       stack_base, start+size))
	...
	if (!vma)
		goto bad_area;
#else	/* defined(CONFIG_OSFMACH3) && defined(STACK_GROWTH_UP) */
	vma = find_vma(current->mm, start);
	if (!vma)
		goto bad_area;
...
check_stack:
	if (!(vma->vm_flags & VM_GROWSDOWN))
		goto bad_area;
#if	defined(CONFIG_OSFMACH3) && defined(STACK_GROWTH_UP)
	if (expand_stack(vma, start+size) == 0)
		goto good_area;
#else	/* defined(CONFIG_OSFMACH3) && defined(STACK_GROWTH_UP) */
...
void vmtruncate(struct inode * inode, unsigned long offset)
{
	...
	mpnt = inode->i_mmap;
#ifdef	CONFIG_OSFMACH3
	printk("vmtruncate: not implemented!\n");
	return;
