 ...
 *		Linus
 */
static int bext2_secrm_seed = 152;	/* Random generator base */

#define RANDOM_INT (bext2_secrm_seed = bext2_secrm_seed * 69069l +1)
#endif
	...
	for (i = direct_block ; i < EXT2_NDIR_BLOCKS ; i++) {
		...
		*p = 0;
		...
		if (free_count == 0) {
			...
			bext2_free_blocks (inode, block_to_free, free_count);
			block_to_free = tmp;
			free_count = 1;
		}
/*		bext2_free_blocks (inode, tmp, 1); */
	}
	if (free_count > 0)
		bext2_free_blocks (inode, block_to_free, free_count);
	return retry;
...
void bext2_truncate (struct inode * inode)
{
	int retry;
	struct buffer_head * bh;
	int err;
