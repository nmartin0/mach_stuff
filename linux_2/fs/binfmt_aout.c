...
MACRO_BEGIN \
	if (file.f_op->write(inode, &file, (char *) (addr), (nr)) != (nr)) \
		goto close_coredump; \
	if (inode && inode->i_mem_object && inode->i_mem_object->imo_cacheable)\
...
static unsigned long * create_aout_tables(char * p, struct linux_binprm * bprm)
{
	...
	if (!vma) {
		printk("create_aout_tables: no stack at 0x%p !?\n", sp);
		force_sig(SIGSEGV, current);
		return sp;
	} else if ((unsigned long) sp < vma->vm_start) {
		ret = expand_stack(vma, (unsigned long) sp);
		if (ret != 0) {
			printk("create_aout_tables: expand_stack error %d\n",
			       ret);
			force_sig(SIGSEGV, current);
			return sp;
		}
