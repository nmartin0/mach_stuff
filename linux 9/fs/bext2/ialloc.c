 ...
 * when a file system is mounted (see bext2_read_super).
 */
			       ...
			       unsigned int bitmap_nr)
{
	...
	if (!bh)
		bext2_panic (sb, "read_inode_bitmap",
			      ...
			      unsigned int block_group)
{
	...
	if (block_group >= sb->u.ext2_sb.s_groups_count)
		bext2_panic (sb, "load_inode_bitmap",
			    "block_group >= groups_count - "
			    "block_group = %d, groups_count = %lu",
			     block_group, sb->u.ext2_sb.s_groups_count);
...
void bext2_free_inode (struct inode * inode)
{
	...
	if (!inode)
		return;
	if (!inode->i_dev) {
		printk ("bext2_free_inode: inode has no device\n");
		return;
	}
	if (inode->i_count > 1) {
		printk ("bext2_free_inode: inode has count=%d\n",
			(int)inode->i_count);
		return;
	}
	if (inode->i_nlink) {
		printk ("bext2_free_inode: inode has nlink=%d\n",
			inode->i_nlink);
		return;
	}
	if (!inode->i_sb) {
		printk("bext2_free_inode: inode on nonexistent device\n");
		return;
	}

	bext2_debug ("freeing inode %lu\n", inode->i_ino);

	sb = inode->i_sb;
	lock_super (sb);
	if (inode->i_ino < EXT2_FIRST_INO(sb) ||
	    inode->i_ino > le32_to_cpu(sb->u.ext2_sb.s_es->s_inodes_count)) {
		bext2_error (sb, "free_inode",
	...
	if (!clear_bit (bit, bh->b_data))
		bext2_warning (sb, "bext2_free_inode",
 ...
 */
struct inode * bext2_new_inode (const struct inode * dir, int mode, int * err)
{
	struct super_block * sb;
	struct buffer_head * bh;
	struct buffer_head * bh2;
...
*/
	...
	if ((j = find_first_zero_bit ((unsigned long *) bh->b_data,
		...
		if (set_bit (j, bh->b_data)) {
			bext2_warning (sb, "bext2_new_inode",
		...
		if (gdp->bg_free_inodes_count != 0) {
			bext2_error (sb, "bext2_new_inode",
	...
	bext2_debug ("allocating inode %lu\n", inode->i_ino);

	*err = 0;
	return inode;
}

unsigned long bext2_count_free_inodes (struct super_block * sb)
{
#ifdef EXT2FS_DEBUG
	struct ext2_super_block * es;
	...
	for (i = 0; i < sb->u.ext2_sb.s_groups_count; i++) {
		...
		bitmap_nr = load_inode_bitmap (sb, i);
		x = bext2_count_free (sb->u.ext2_sb.s_inode_bitmap[bitmap_nr],
				     EXT2_INODES_PER_GROUP(sb) / 8);
		printk ("group %d: stored = %d, counted = %lu\n",
			i, gdp->bg_free_inodes_count, x);
		bitmap_count += x;
	}
	printk("bext2_count_free_inodes: stored = %lu, computed = %lu, %lu\n",
...
void bext2_check_inodes_bitmap (struct super_block * sb)
{
	struct ext2_super_block * es;
	unsigned long desc_count, bitmap_count, x;
	int bitmap_nr;
	...
	for (i = 0; i < sb->u.ext2_sb.s_groups_count; i++) {
		...
		if (gdp->bg_free_inodes_count != x)
			bext2_error (sb, "bext2_check_inodes_bitmap",
