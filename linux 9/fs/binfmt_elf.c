	...
	if (nbyte) {
		...
		do {
			...
			if (nbyte > PAGE_SIZE) 
				...
				count = nbyte;
			memcpy_tofs(fpnt, osfmach3_zero_page, count);
			fpnt += count;
			nbyte -= count;
		} while (nbyte);
				 ...
				 unsigned long interp_load_addr, int ibcs)
{
...
#endif	/* CONFIG_OSFMACH3 */
#ifdef	__powerpc__
	unsigned long *dlinfo_base;
#endif	/* __powerpc__ */

#ifdef	CONFIG_OSFMACH3
	/*
	 * Pre-compute the final stack pointer and expand the
	 * stack if necessary.
	 */
#ifdef STACK_GROWTH_UP
	base = (STACK_TOP - current->rlim[RLIMIT_STACK].rlim_cur);

	sp = (unsigned long*)(base + (MAX_ARG_PAGES*PAGE_SIZE));
	...
	vma = find_vma_intersection(current->mm, base, (unsigned long)sp);

	if (!vma) {
		printk("create_elf_tables: no stack at 0x%p !?\n", sp);
		force_sig(SIGSEGV, current);
		return sp;
	} else if ((unsigned long)sp >= vma->vm_end) {
		ret = expand_stack(vma, (unsigned long)sp);
		if (ret != 0) {
			printk("create_elf_tables: expand_stack error %d\n",
			       ret);
			force_sig(SIGSEGV, current);
			return sp;
		}
	...
	--sp;
#ifdef	__powerpc__
	sp = (unsigned long *) (0xFFFFFFF0 & (unsigned long) sp);
#endif	/* __powerpc__ */

	vma = find_vma(current->mm, (unsigned long) sp);
	if (!vma) {
		printk("create_elf_tables: no stack at 0x%p !?\n", sp);
		force_sig(SIGSEGV, current);
		return sp;
	} else if ((unsigned long) sp < vma->vm_start) {
		ret = expand_stack(vma, (unsigned long) sp);
		if (ret != 0) {
			printk("create_elf_tables: expand_stack error %d\n",
			       ret);
	...
	dlinfo = sp;
#ifdef __powerpc__
	/* Stack must end up aligned on 16 byte boundary */
	dlinfo_base = dlinfo;
	sp -= (argc+1) + (envc+1) + 1;
	sp = (unsigned long *) (0xFFFFFFF0 & (unsigned long) sp);
	argv = sp+1;
...
#undef NEW_AUX_ENT
#ifdef __powerpc__
	/* Stack must be aligned to 16 byte boundary! */
...
static inline int do_load_elf_binary(struct linux_binprm *bprm, struct pt_regs *regs)
{
	...
	elf_stack = current->mm->start_stack;
	start_thread(regs, elf_entry, elf_stack);
#ifdef	CONFIG_OSFMACH3
#ifdef	hp_pa
	regs->state.arg0 = bprm->argc;
	regs->state.arg1 = elf_stack - (bprm->envc + 2 + 1 + bprm->argc + 1) * sizeof(long);
	regs->state.arg2 = elf_stack - (bprm->envc + 2 + 1) * sizeof(long);
#endif	/* hp_pa */
#endif	/* CONFIG_OSFMACH3 */
...
static int dump_write(struct file *file, const void *addr, int nr)
{
	...
	ret = (file->f_op->write(file->f_inode, file, addr, nr) == nr);
	if (file->f_inode && file->f_inode->i_mem_object &&
	    file->f_inode->i_mem_object->imo_cacheable)
		inode_pager_uncache(file->f_inode);
	return ret;
#else	/* CONFIG_OSFMACH3 */
...
#ifdef	CONFIG_OSFMACH3
	osfmach3_update_task_info(current);
#endif	/* CONFIG_OSFMACH3 */
