...
# /etc/newsys for MacMach BSD 4.3, version 02
...
FTP=""
case "$METHOD" in
  disk|DISK) METHOD="DISK"; FTP="";;
  ftp|FTP) METHOD="FTP"; FTP="YES";;
...
while [ ! "$ADDR" ]; do
  ...
  echo -n "Enter the network address of this system: [NONE] "
  read ADDR
  [ "$ADDR" ] || ADDR="NONE"
done
case "$ADDR" in
...
[ "$METHOD" = "FTP" -a "$ADDR" = "NONE" ] && {
  echo "ERROR: FTP requires a network address."
  EXIT=1; exit $EXIT
}
...
case "$METHOD" in
  DISK) DEFAULT="/";;
  FTP) DEFAULT="/users/macmach/mach";;
esac
ASKUSRSRC=""
while [ ! "$SRCDIR" ]; do
  echo ""
...
[ "$ADDR" ] && {
  # if network not up, start up using the specified address
