...
{
    unsigned long	t;
    extern long timeoffset; /* from locore.s */

    (void) GetDateTime(&t);

    /*
     * The Macintosh Operating System stores the time as local time,
     * and the Mach kernel the uses U**X style GMT.  The offset from
     * local time to GMT (in hours) is passed in as a boot argument.
     * Also, the T0 values for MacOS and U**X are different, so we also add
     * in the correct offset here.
     */
    time.seconds = t - T0_DELTA + (timeoffset * 60 * 60);
    time.microseconds = 0;
}

/*
 * Called to set the time of day clock when the system time is adjusted.
 * MacMach does not ever set the RTC time, the user is expected to set it
 * from MacOS to local time and then set the timeoffset in the control
 * panel to adjust local time to GMT.
 */
resettodr()
{
}
