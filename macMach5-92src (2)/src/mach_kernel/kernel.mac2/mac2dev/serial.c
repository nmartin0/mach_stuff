 ...
 *      b) set internal input buffer
 *      c) set default baud rate, char size, etc.
 *      d) disable Xon/Xoff and CTS handshaking
 *      e) flush input and output buffers
 *      f) turn on DTR
 *      g) setup for MACH generic tty layer
...
/* parameters to SerSetBuf */
typedef struct {
  Ptr serBptr;
  unsigned short serBlen;
} *SerSetBuf_t;
...
/* see device/chario.c for tty_inq_size and tty_outq_size */
...
static struct serial_port {
  ...
  char Buf[BUF_SIZE];          /* internal input buffer */
  char iBuf[IBUF_SIZE];        /* input buffer */
  char oBuf[OBUF_SIZE];        /* output buffer */
...
#define DEFAULT_BITS (BAUD_2400 | EIGHT_BIT_DATA | NO_PARITY)
#define ISPEED	B2400
...
int serial_start(struct tty *tp)
{
  ...
  p->oPb.ioReqCount = q_to_b(&tp->t_outq, p->oBuf, OBUF_SIZE);
...
io_return_t serial_close(int number)
{
  ...
  if (p->outRefNum && (tp->t_state & TS_HUPCLS)) {
    p->modem_bits &= ~TM_DTR;
    p->cPb.ioCRefNum = p->outRefNum;
    p->cPb.csCode = SerDTRoff;
...
io_return_t serial_open(int number, dev_mode_t mode, io_req_t ior)
{
  ...
  if ((tp->t_state & (TS_ISOPEN | TS_WOPEN)) == 0) {
    ...
    /* set internal input buffer */
    ((SerSetBuf_t)p->cPb.csParam)->serBptr = (Ptr)p->Buf;
    ((SerSetBuf_t)p->cPb.csParam)->serBlen = BUF_SIZE;
    p->cPb.ioCRefNum = p->inRefNum;
    p->cPb.csCode = SerSetBuf;
    if ((result = OSErr_io_return_t(PBControl(&p->cPb, FALSE))) != D_SUCCESS) {
      xprintf("serial_open: port %d input SerSetBuf ioResult is %d",
        number, p->cPb.ioResult);
      goto error;
    }
...
io_return_t serial_read(int number, io_req_t ior)
{
  ...
  else if (p->iPb.ioReqCount > IBUF_SIZE) p->iPb.ioReqCount = IBUF_SIZE;
