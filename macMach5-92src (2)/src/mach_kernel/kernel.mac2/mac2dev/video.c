...
video_open(dev_t dev)
{
  register struct video	*vp;
  int unit = minor(dev);

  if (unit >= NVIDEO) return D_NO_SUCH_DEVICE;
  vp = &video[unit];
  if ((vp->video_flags & VIDEO_F_INITED) == 0) return D_NO_SUCH_DEVICE;
  if ((vp->video_flags & VIDEO_F_OPEN) == 0) vp->video_flags |= VIDEO_F_OPEN;
  return D_SUCCESS;

} /* video_open() */

video_close(dev_t dev)
{
...
video_mmap(dev_t dev, register vm_offset_t offset, int prot)
{
  register struct video *vp = &video[minor(dev)];
...
/* Retrieve a video parameter block. */
boolean_t
...
/*
 * Called from autoconfig.  The console device has already been initialized.
 */
boolean_t
 ...
 */
boolean_t video_setup(register struct video *vp)
{
    boolean_t		result = TRUE;
