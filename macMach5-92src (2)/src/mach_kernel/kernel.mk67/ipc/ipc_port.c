...
#if	NORMA_IPC
	port->ip_norma_uid = 0;
	port->ip_norma_scan = 0;
	port->ip_norma_stransit = 0;
	port->ip_norma_sotransit = 0;
	port->ip_norma_dest_node = 0;
	port->ip_norma_exported = FALSE;
	port->ip_norma_imported = FALSE;
	port->ip_norma_is_proxy = FALSE;
	port->ip_norma_dead_uid = FALSE;
	port->ip_norma_is_special = FALSE;
	port->ip_norma_wanted = FALSE;
	port->ip_norma_migrating = FALSE;
	port->ip_norma_newly_migrated = FALSE;
	...
	ipc_port_t port;
{
	...
	printf(",rcvrs = 0x%x\n", port->ip_messages.imq_threads.ithq_base);
	iprintf("sndrs = 0x%x", port->ip_blocked.ithq_base);
	printf(",kobj = 0x%x\n", port->ip_kobject);
#if	NORMA_IPC
	iprintf("ip_norma_uid = %d:%d",
		IP_NORMA_NODE(port->ip_norma_uid),
		IP_NORMA_LID(port->ip_norma_uid));
	printf(",ip_norma_next = 0x%x\n", port->ip_norma_next);
	iprintf("ip_norma_scan=%d,stransit=%d,sotransit=%d,dest_node=%d\n",
		port->ip_norma_scan,
		port->ip_norma_stransit,
		port->ip_norma_sotransit,
		port->ip_norma_dest_node);
	iprintf("ip_norma_exported=%d,imported=%d,is_proxy=%d,dead_uid=%d\n",
		port->ip_norma_exported,
		port->ip_norma_imported,
		port->ip_norma_is_proxy,
		port->ip_norma_dead_uid);
	iprintf("ip_norma_is_special=%d,wanted=%d,migrating=%d,",
		port->ip_norma_is_special,
		port->ip_norma_wanted,
		port->ip_norma_migrating);
	printf("newly_migrated=%d\n", port->ip_norma_newly_migrated);
#endif	NORMA_IPC
