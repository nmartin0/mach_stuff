...
void
exception(exception, code, subcode)
	int exception, code, subcode;
{
	...
	if (!IP_VALID(exc_port)) {
		ith_unlock(self);
		exception_try_task(exception, code, subcode);
		/*NOTREACHED*/
		return;
	}
	...
	int exception, code, subcode;
{
	...
	if (copyoutmsg((char *) &kmsg->ikm_header, (char *) receiver->ith_msg,
	...
	if (IP_NORMA_IS_PROXY(dest_port)) {
		norma_ipc_send_port(exc->thread, MACH_MSG_TYPE_PORT_SEND);
		norma_ipc_send_port(exc->task, MACH_MSG_TYPE_PORT_SEND);
		exc->thread =
		    (mach_port_t) ((ipc_port_t) exc->thread)->ip_norma_uid;
		exc->task =
		    (mach_port_t) ((ipc_port_t) exc->task)->ip_norma_uid;
	...
	if (!ip_active(reply_port)) {
		ip_unlock(reply_port);
		exception_raise_continue_slow(MACH_RCV_PORT_DIED, IKM_NULL);
		/*NOTREACHED*/
		return;
	}
