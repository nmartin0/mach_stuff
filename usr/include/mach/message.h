 ...
 * HISTORY
 * $Log: message.h,v $
 * Revision 1.2.56.1  1997/03/27  18:53:24  barbou
 * 	DIPC:  add MACH_MSGH_BITS_CONNECTING and BITS_CONNECT_WAIT.
 * 	[95/11/27            alanl]
 * 	[97/02/26            barbou]
 *
 * Revision 1.2.31.7  1995/10/30  19:22:33  devrcs
 * 	Remove duplicate definition.
 * 	[1995/10/26  21:30:45  watkins]
 * 
 * Revision 1.2.31.6  1995/10/09  17:21:05  devrcs
 * 	Merge RT IPC
 * 	[1995/09/21  23:10:33  widyono]
 * 
 * 	Merge in ipc message delivery fix from rt3.
 * 	Add internal token for MACH_RCV_IN_PROGRESS_TIMED.
 * 	[1995/09/07  20:36:40  watkins]
 * 
 * Revision 1.2.31.5  1995/09/18  19:20:43  devrcs
 * 	Merge in ipc message delivery fix from rt3.
 * 	Add internal token for MACH_RCV_IN_PROGRESS_TIMED.
 * 	[1995/09/07  20:36:40  watkins]
 * 
 * Revision 1.2.31.4  1995/06/09  02:28:08  mmp
 * 	Replaced MACH_SEND_VOLATILE and MACH_MSGH_BITS_VOLATILE with a
 * 	new descriptor type, MACH_MSG_OOL_VOLATILE_DESCRIPTOR.  This is a
 * 	better fit for the semantics we want:  per-OOL region volatility.
 * 	[95/04/03            alanl]
 * 	[95/04/03            alanl]
 * 
 * Revision 1.2.31.3  1995/02/24  14:48:30  alanl
 * 	Added new msgh bits: RTALLOC, RPC, DIPC_EXPANSION.
 * 	[95/01/04            mmp]
 * 
 * 	Merge with DIPC2_SHARED.
 * 	[1995/01/04  14:17:13  alanl]
 * 
 * Revision 1.2.33.2  1994/10/14  03:51:08  dwm
 * 	mk6 CR668 - 1.3b26 merge
 * 	64bit changes
 * 	[1994/10/14  03:43:00  dwm]
 * 
 * Revision 1.2.31.1  1994/09/23  02:40:16  ezf
 * 	change marker to not FREE
 * 	[1994/09/22  21:41:43  ezf]
 * 
 * Revision 1.2.28.5  1994/12/27  20:05:18  alanl
 * 	Add MACH_SEND_VOLATILE and MACH_MSGH_BITS_VOLATILE so that the
 * 	user can declare that OOL regions won't be touched during a
 * 	message transfer.  This flag permits an optimization eliminating
 * 	copy-on-write costs.
 * 	[94/12/19            alanl]
 * 
 * Revision 1.2.28.4  1994/10/19  20:57:24  mmp
 * 	Added MACH_MSGH_BITS_REF_CONVERT to msgh bits.
 * 	[94/10/19            sjs]
 * 
 * 	Added MACH_MSG_OVERWRITE_DIPC as a new descriptor copy type so that
 * 	DIPC can arrange to have incoming data placed directly into the target
 * 	address space.
 * 	[94/10/19            mmp]
 * 
 * Revision 1.2.28.3  1994/10/03  20:29:04  mmp
 * 	Added MACH_RCV_TRANSPORT_ERROR for DIPC.
 * 	[1994/10/03  20:27:26  mmp]
 * 
 * Revision 1.2.28.2  1994/09/28  15:55:10  alanl
 * 	Added MACH_MSGH_BITS_HANDLE and MACH_MSGH_BITS_RECEIVING.
 * 	[94/09/23            alanl]
 * 
 * Revision 1.2.28.1  1994/08/04  02:28:25  mmp
 * 	NOTE: file was moved back to b11 version for dipc2_shared but
 * 	prototype added for mach_msg_overwrite was retained.
 * 
 * 	nmk17 merge: put back SEND_CANCEL, delete obsolete options:
 * 	*SEND_NOTIFY* and MACH_RCV_LARGE.
 * 	Change MACH_MSGH_BITS_RESTARTING to MACH_MSGH_BITS_PLACEHOLDER.
 * 	[1994/08/03  20:41:18  mmp]
 * 
 * Revision 1.2.23.6  1994/06/10  14:04:13  mmp
 * 	DIPC: Added MACH_SEND_LOCAL_QUEUE option to indicate
 * 	that the message should be enqueued on the proxy.
 * 	Added MACH_SEND_RESEND_FAILED error.
 * 	[1994/06/10  13:52:41  mmp]
 * 
 * 	DIPC:  defined "network format" for a message.
 * 	Also:  replaced MACH_MSGH_BITS_MIGRATED with
 * 	MACH_MSGH_BITS_MIGRATING.  Replaced three bits,
 * 	MACH_MSGH_BITS_ COMPLEX_PORTS, COMPLEX_DATA
 * 	and INLINE_ONLY with one bit:  COMPLEX_OOL;
 * 	this bit is only defined under DIPC.  Eliminated
 * 	MACH_MSGH_BITS_MESSAGE_DONE, which is unneeded.
 * 	Updated associated comments.
 * 	Also:  Added MACH_MSGH_BITS_RESTARTING, for
 * 	message migration.
 * 	Also:  Added MACH_MSGH_BITS_LOCAL_NMS, to indicate
 * 	that NMS detection is enabled on the local port
 * 	carried in a message.
 * 	Also:  Added DIPC_TYPE_BITS, stealing unused bits
 * 	from the type descriptor.
 * 	Also:  Added a comment describing the internal
 * 	format of DIPC messages.
 * 	[1994/05/20  18:30:05  alanl]
 * 
 * 	DIPC: Add dipc_trailer.
 * 	[1994/05/11  20:39:38  mmp]
 * 
 * 	DIPC:  defined "network format" for a message.
 * 	Also:  replaced MACH_MSGH_BITS_MIGRATED with
 * 	MACH_MSGH_BITS_MIGRATING.  Replaced three bits,
 ...
 * 	this bit is only defined under DIPC.  Eliminated
 * 	MACH_MSGH_BITS_MESSAGE_DONE, which is unneeded.
 * 	Updated associated comments.  Added
 * 	MACH_MSGH_BITS_RESTARTING, for message migration.
 * 	Also:  Added DIPC_TYPE_BITS, stealing unused bits
 * 	from the type descriptor.
 * 	Also:  Added a comment describing the internal
 * 	format of DIPC messages.
 * 	[1994/05/11  17:34:45  alanl]
 * 
 * 	Renamed MACH_MSGH_BITS_NETWORK to MACH_MSGH_BITS_DIPC_FORMAT,
 * 	Removed MACH_MSGH_BITS_INTR_KMSG; instead, a special value in
 * 	the kmsg's ikm_size field indicates that the message originated
 * 	from interrupt level.
 * 	Defined MACH_MSGH_BITS_INLINE_ONLY (from mmp).
 * 	which is more intuitive.
 * 	[1994/04/22  17:11:14  alanl]
 * 
 * 	DIPC:  Delete all support for send notifications.  They have
 * 	been removed from the Mach specification and DIPC is easier
 * 	to implement without this feature.  In this file:  the
 * 	following user-level options are now obsolete:
 * 	MACH_SEND_NOTIFY
 * 	MACH_SEND_NOTIFY_IN_PROGRESS
 * 	MACH_SEND_CANCEL
 * 	SEND_NOTIFY
 * 	SEND_NOTIFY_IN_PROGRESS
 * 	However, for the moment, we leave these definitions intact
 * 	so that the kernel can catch user-level code using these options.
 * 
 * 	Added definitions for MACH_MSGH_BITS_DIPC_FORMAT (kmsg has been
 * 	converted to "network" format) and MACH_MSGH_BITS_META_KMSG
 * 	(the structure is really a meta_kmsg, not a kmsg).  Changed
 * 	MACH_MSGH_BITS_UNUSED definition to match.
 * 	[1994/04/20  18:31:33  alanl]
 * 	Revision 1.2.10.9  1993/10/20  19:12:46  gm
 * 	CR9915: Added mach_msg_overwrite() prototype.
 * 	[1993/10/13  17:21:24  gm]
 * 
 * Revision 1.2.10.8  1993/09/09  16:07:40  jeffc
 * 	CR9745 - Delete message accepted notifications
 * 	[1993/09/03  20:45:40  jeffc]
 * 
 * Revision 1.2.10.7  1993/08/05  19:09:28  jeffc
 * 	CR9508 - Delete dead code. Remove MACH_IPC_TYPED
 * 	[1993/08/03  20:18:30  jeffc]
 * 
 * 	CR9508 - Delete dead code. Remove MACH_IPC_COMPAT
 * 	[1993/08/03  17:09:14  jeffc]
 * 
 * Revision 1.2.10.6  1993/08/03  18:29:41  gm
 * 	CR9596: Change KERNEL to MACH_KERNEL.
 * 	[1993/08/02  18:24:34  gm]
 * 
 * Revision 1.2.10.5  1993/07/19  18:32:55  travos
 * 	Define KERNEL_SECURITY_TOKEN_VALUE.
 * 	Zap MSG_AND_TRAILER_SIZE macro
 * 	[1993/07/19  18:30:21  travos]
 * 
 * Revision 1.2.10.4  1993/07/12  18:07:31  gm
 * 	CR9339: Added trailing 'U' to MACH_MSGH_BITS_COMPLEX definition.
 * 	[1993/07/12  13:37:01  gm]
 * 
 * Revision 1.2.10.3  1993/07/08  19:35:37  travos
 * 	Put the the definition of MIG built-in types (MACH_MSG_TYPE_INTEGER_32)
 * 	under MACH_IPC_TYPED=1. In fact, in the Untyped IPC kernel there is no
 * 	need for the kernel to know about these types (which are MIG internal).
 * 	[Note: the code under MACH_IPC_TYPED=1 is eligible for being phased out]
 * 
 * 	Add the define MACH_MSG_TRAILER_MINIMUM_SIZE.
 * 	Add the define MACH_RCV_TRAILER_MASK (to mask-off the trailer bits in
 * 	the option field). Zap the code under ENUMS (ANSI C would not let us use
 * 	enum when assigning bitfields).
 * 	[1993/07/08  15:36:41  travos]
 * 
 * Revision 1.2.10.2  1993/06/09  02:42:16  gm
 * 	2.5 compatability support for untyped IPC.  CR #8972.
 * 	[1993/04/27  11:29:21  rod]
 * 
 * 	CR9194 - Temporary fix to removed dependency on UNIX <sys/types.h>
 * 	header file. Need to define a machine-depended 64-bit type to
 * 	properly handle this problem, rather than just assuming that
 * 	a thing called a "quad" exists and is appropriate.
 * 	[1993/05/07  18:57:03  jeffc]
 * 
 * Revision 1.2  1993/04/19  16:36:13  devrcs
 * 	Added trailer support to untyped ipc.		[travos@osf.org]
 * 	[1993/04/06  16:51:31  travos]
 * 
 * 	ANSI C fixes: zap enumerated types, zero length array.
 * 	[1993/03/23  16:49:40  rod]
 * 
 * 	Merge untyped ipc:
 * 	Avoid conflicts between enums and #defines
 * 	[1993/03/05  23:11:26  travos]
 * 	Add MACH_MSG_PAGE_LIST_COPY_T.
 * 	[1993/02/26  19:22:31  rod]
 * 	Added internal copy type MACH_MSG_KALLOC_COPY_T.
 * 	[1993/02/26  00:26:18  rod]
 * 	Define mach_msg_type_t and mach_msg_type_long_t iff MACH_IPC_TYPED=1
 * 	[1993/02/24  19:32:11  travos]
 * 	Untyped ipc support. [fdr@osf.org, rod@osf.org]
 * 	[1993/01/27  11:25:07  rod]
 * 
 * 	ansi C conformance changes
 * 	[1993/02/02  18:53:37  david]
 * 
 * Revision 1.1  1992/09/30  02:31:28  robert
 * 	Initial revision
 * 
 * $EndLog$
 */
