 ...
 * Revision 1.1.29.1  1997/10/30  15:32:32  barbou
 * 	Added prototypes for new system calls.
 * 	[1997/10/30  15:22:58  barbou]
 *
 * Revision 1.1.17.7  1996/07/31  09:57:19  paire
 * 	Merged with nmk20b7_shared (1.1.27.1)
 * 	[96/06/10            paire]
 * 
...
extern kern_return_t mig_vm_wire(
...
extern kern_return_t mig_vm_machine_attribute(
				mach_port_t			target_map,
				vm_address_t			address,
				vm_size_t			size,
...
extern kern_return_t mig_thread_info(
				mach_port_t		thread,
				thread_flavor_t		flavor,
				thread_info_t		info_out,
				mach_msg_type_number_t	*info_cnt);

extern kern_return_t mig_thread_create(
				mach_port_t	parent_task,
				mach_port_t	*child_thread);

extern kern_return_t mig_thread_terminate(
				mach_port_t	target_thread);
				
extern kern_return_t mig_thread_abort_safely(
				mach_port_t	target_thread);

extern kern_return_t mig_thread_set_state(
				mach_port_t		target_thread,
				int			flavor,
				thread_state_t		state,
				mach_msg_type_number_t	state_count);

extern kern_return_t mig_thread_suspend(
				mach_port_t	target_thread);

extern kern_return_t mig_host_statistics(
				mach_port_t		host_port,
				host_flavor_t		flavor,
				host_info_t		info,
				mach_msg_type_number_t	*count);

extern kern_return_t mig_mach_port_mod_refs(
				mach_port_t		target_task,
				mach_port_t		port_name,
				mach_port_right_t	right,
				mach_port_delta_t	delta);

extern kern_return_t mig_mach_port_request_notification(
				mach_port_t		target_task,
				mach_port_t		port_name,
				mach_msg_id_t		id,
...
extern kern_return_t mig_memory_object_change_attributes(
...
extern kern_return_t mig_memory_object_data_error(
				mach_port_t	object_name,
				vm_offset_t	offset,
				vm_size_t	size,
				kern_return_t	error_value);

extern kern_return_t mig_memory_object_data_unavailable(
				mach_port_t	object_name,
				vm_offset_t	offset,
				vm_size_t	size);

extern kern_return_t mig_memory_object_synchronize_completed(
				mach_port_t	object_name,
				vm_offset_t	offset,
				vm_size_t	size);

extern kern_return_t mig_memory_object_lock_request(
				mach_port_t		object_name,
				vm_offset_t		offset,
				vm_size_t		size,
...
extern kern_return_t mig_memory_object_discard_reply(
				mach_port_t		object_name,
...
extern kern_return_t mig_memory_object_data_supply(
				mach_port_t		object_name,
				vm_offset_t		offset,
				vm_offset_t		data,
