...
void bext2_error (struct super_block * sb, const char * function,
		 const char * fmt, ...)
{
	va_list args;
	...
	if (test_opt (sb, ERRORS_PANIC) ||
		...
		panic ("BEXT2-fs panic (device %s): %s: %s\n",
		       kdevname(sb->s_dev), function, error_buf);
	printk (KERN_CRIT "BEXT2-fs error (device %s): %s: %s\n",
		kdevname(sb->s_dev), function, error_buf);
...
NORET_TYPE void bext2_panic (struct super_block * sb, const char * function,
			    const char * fmt, ...)
{
	va_list args;
...
void bext2_warning (struct super_block * sb, const char * function,
		   const char * fmt, ...)
{
	va_list args;
	...
	vsprintf (error_buf, fmt, args);
	va_end (args);
	printk (KERN_WARNING "BEXT2-fs warning (device %s): %s: %s\n",
		kdevname(sb->s_dev), function, error_buf);
}

void bext2_put_super (struct super_block * sb)
{
	int db_count;
	int i;
...
static struct super_operations bext2_sops = { 
	bext2_read_inode,
	NULL,
	bext2_write_inode,
	bext2_put_inode,
	...
	for (this_char = strtok (options, ",");
		...
		if (!strcmp (this_char, "bsddf"))
			...
			if (!value || !*value)
				...
				printk ("BEXT2-fs: Invalid check option: %s\n",
					value);
				return 0;
			}
		}
			...
			if (!value || !*value) {
				printk ("BEXT2-fs: the errors option requires "
					"an argument");
				return 0;
			}
			if (!strcmp (value, "continue")) {
				...
				printk ("BEXT2-fs: Invalid errors option: %s\n",
					value);
				return 0;
			}
		}
			...
			if (!value || !*value) {
				printk ("BEXT2-fs: the resgid option requires "
					"an argument");
				return 0;
			}
			*resgid = simple_strtoul (value, &value, 0);
			if (*value) {
				printk ("BEXT2-fs: Invalid resgid option: %s\n",
					value);
				return 0;
			}
		}
			...
			if (!value || !*value) {
				printk ("BEXT2-fs: the resuid option requires "
					"an argument");
				return 0;
			}
			*resuid = simple_strtoul (value, &value, 0);
			if (*value) {
				printk ("BEXT2-fs: Invalid resuid option: %s\n",
					value);
				return 0;
			}
		}
			...
			if (!value || !*value) {
				printk ("BEXT2-fs: the sb option requires "
					"an argument");
				return 0;
			}
			*sb_block = simple_strtoul (value, &value, 0);
			if (*value) {
				printk ("BEXT2-fs: Invalid sb option: %s\n",
					value);
				return 0;
			}
		}
		/* Silently ignore the quota options */
			...
			printk ("BEXT2-fs: Unrecognized mount option %s\n", this_char);
			return 0;
		}
	}
	return 1;
...
static void bext2_setup_super (struct super_block * sb,
			      struct ext2_super_block * es)
{
	if (es->s_rev_level > EXT2_MAX_SUPP_REV) {
			printk ("BEXT2-fs warning: revision level too high, "
				"forcing read/only mode\n");
			sb->s_flags |= MS_RDONLY;
	}
	if (!(sb->s_flags & MS_RDONLY)) {
		if (!(sb->u.ext2_sb.s_mount_state & EXT2_VALID_FS))
			printk ("BEXT2-fs warning: mounting unchecked fs, "
				"running e2fsck is recommended\n");
		else if ((sb->u.ext2_sb.s_mount_state & EXT2_ERROR_FS))
			printk ("BEXT2-fs warning: mounting fs with errors, "
				"running e2fsck is recommended\n");
		else if (es->s_max_mnt_count >= 0 &&
		         es->s_mnt_count >= (unsigned short) es->s_max_mnt_count)
			printk ("BEXT2-fs warning: maximal mount count reached, "
				"running e2fsck is recommended\n");
		else if (es->s_checkinterval &&
			(es->s_lastcheck + es->s_checkinterval <= CURRENT_TIME))
			printk ("BEXT2-fs warning: checktime reached, "
				"running e2fsck is recommended\n");
		...
		if (test_opt (sb, CHECK)) {
			bext2_check_blocks_bitmap (sb);
			bext2_check_inodes_bitmap (sb);
		}
	}
}

static int bext2_check_descriptors (struct super_block * sb)
{
	int i;
	int desc_block = 0;
	...
	bext2_debug ("Checking group descriptors");
	...
	for (i = 0; i < sb->u.ext2_sb.s_groups_count; i++)
	{
		...
		{
			bext2_error (sb, "bext2_check_descriptors",
...
struct super_block * bext2_read_super (struct super_block * sb, void * data,
				      int silent)
{
	...
	if (!(bh = bread (dev, sb_block, BLOCK_SIZE))) {
		sb->s_dev = 0;
		unlock_super (sb);
		printk ("BEXT2-fs: unable to read superblock\n");
		MOD_DEC_USE_COUNT;
		return NULL;
	}
	/*
	...
	if (sb->s_magic != EXT2_SUPER_MAGIC) {
		if (!silent)
			printk ("VFS: Can't find a big-endian ext2 filesystem on dev "
				"%s.\n", kdevname(dev));
	failed_mount:
		sb->s_dev = 0;
		unlock_super (sb);
	...
	if (es->s_rev_level > EXT2_GOOD_OLD_REV) {
		if (es->s_feature_incompat & ~EXT2_FEATURE_INCOMPAT_SUPP) {
			printk("BEXT2-fs: %s: couldn't mount because of "
		...
		if (!(sb->s_flags & MS_RDONLY) &&
			...
			printk("BEXT2-fs: %s: couldn't mount RDWR because of "
			       "unsupported optional features.\n", 
			       kdevname(dev));
			goto failed_mount;
		}
	...
	if (sb->s_blocksize != BLOCK_SIZE && 
		...
		if(!bh) {
			printk("BEXT2-fs: Couldn't read superblock on "
			       "2nd try.\n");
			goto failed_mount;
		}
		...
		if (es->s_magic != EXT2_SUPER_MAGIC) {
			printk ("BEXT2-fs: Magic mismatch, very weird !\n");
			goto failed_mount;
		}
	}
	if (es->s_rev_level == EXT2_GOOD_OLD_REV) {
		...
		if (sb->u.ext2_sb.s_inode_size != EXT2_GOOD_OLD_INODE_SIZE) {
			printk ("BEXT2-fs: unsupported inode size: %d\n",
	...
	if (sb->s_magic != EXT2_SUPER_MAGIC) {
		if (!silent)
			printk ("VFS: Can't find an bext2 filesystem on dev "
				"%s.\n",
				kdevname(dev));
		goto failed_mount;
	}
	...
	if (sb->u.ext2_sb.s_group_desc == NULL) {
		printk ("BEXT2-fs: not enough memory\n");
		goto failed_mount;
	}
	for (i = 0; i < db_count; i++) {
		...
		if (!sb->u.ext2_sb.s_group_desc[i]) {
			...
			kfree_s (sb->u.ext2_sb.s_group_desc,
				 db_count * sizeof (struct buffer_head *));
			printk ("BEXT2-fs: unable to read group descriptors\n");
			goto failed_mount;
		}
	}
	if (!bext2_check_descriptors (sb)) {
		for (j = 0; j < db_count; j++)
			brelse (sb->u.ext2_sb.s_group_desc[j]);
		kfree_s (sb->u.ext2_sb.s_group_desc,
			 db_count * sizeof (struct buffer_head *));
		printk ("BEXT2-fs: group descriptors corrupted !\n");
		goto failed_mount;
	}
	for (i = 0; i < EXT2_MAX_GROUP_LOADED; i++) {
	...
	if (!(sb->s_mounted = iget (sb, EXT2_ROOT_INO))) {
		...
		kfree_s (sb->u.ext2_sb.s_group_desc,
			 db_count * sizeof (struct buffer_head *));
		brelse (bh);
		printk ("BEXT2-fs: get root inode failed\n");
		MOD_DEC_USE_COUNT;
		return NULL;
	}
	bext2_setup_super (sb, es);
#ifndef hp_pa /* don't be alarmist, this is normal */
	printk(KERN_WARNING "VFS: %s contains an old-fashioned big-endian EXT2 filesystem\n",
	       kdevname(dev));
#endif
	return sb;
}

static void bext2_commit_super (struct super_block * sb,
			       struct ext2_super_block * es)
{
...
void bext2_write_super (struct super_block * sb)
{
	struct ext2_super_block * es;
	...
	if (!(sb->s_flags & MS_RDONLY)) {
		...
		bext2_debug ("setting valid to 0\n");

		if (es->s_state & EXT2_VALID_FS) {
			es->s_state &= ~EXT2_VALID_FS;
			es->s_mtime = CURRENT_TIME;
		}
		bext2_commit_super (sb, es);
	}
	sb->s_dirt = 0;
}

int bext2_remount (struct super_block * sb, int * flags, char * data)
{
	struct ext2_super_block * es;
...
static struct file_system_type bext2_fs_type = {
        bext2_read_super, "bext2", 1, NULL
};

int init_bext2_fs(void)
{
        return register_filesystem(&bext2_fs_type);
}
...
{
        unregister_filesystem(&bext2_fs_type);
}

#endif

void bext2_statfs (struct super_block * sb, struct statfs * buf, int bufsiz)
{
	...
	tmp.f_bfree = bext2_count_free_blocks (sb);
	...
	tmp.f_ffree = bext2_count_free_inodes (sb);
