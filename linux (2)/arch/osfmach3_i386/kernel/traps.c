...
{ \
	...
	tsk->osfmach3.thread->trap_no = trapnr; \
	force_sig(signr, tsk); \
	die_if_kernel(str,regs,error_code); \
}

#define get_seg_byte(seg,addr) (*((unsigned char *) addr))
#define get_seg_long(seg,addr) (*((unsigned long *) addr))
...
#else	/* CONFIG_OSFMACH3 */
		     (addr >= (unsigned long) &start_kernel) &&
#endif	/* CONFIG_OSFMACH3 */
...
asmlinkage void do_general_protection(struct pt_regs * regs, long error_code)
{
	...
	current->osfmach3.thread->trap_no = 13;
#else	/* CONFIG_OSFMACH3 */
