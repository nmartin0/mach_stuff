#ifndef lint
static char *RCSid = "$Id: doname.c,v 1.11 91/01/03 13:00:08 bww Exp $";
#endif
 ...
 * HISTORY
 * $Log:	doname.c,v $
 * Revision 5.2  91/10/14  13:40:07  berman
 * 	setvar() now returns a value.
 * 	From STUMP "[91/01/03  12:51:17  bww]".
 * 	[91/10/10            bww]
 * 
	...
	struct chain **ochain;
{
	...
	for (lp = p->linep; lp; lp = lp->nxtlineblock) {
		...
		if (lp->shp && (ptime < td || (ptime == 0 && td == 0) || lp->depp == 0)) {
			...
			if (!questflag) {
				...
				if (cochain)
					co(cochain);
				(void) strcpy(abuf, mkqlist(achain));
				(void) setvar(">", abuf, 1);
				(void) setvar("@", p->namep, 1);
				(void) setvar("%", pct, 1);
				(void) setvar("?", mkqlist(qchain), 1);
				errstat += docom(lp);
				(void) setvar("@", (char *) 0, 1);
			}
			achain = 0;
	...
	if (errstat == 0 && (ptime < tdep || (ptime == 0 && tdep == 0))) {
		...
		if (cochain)
			co(cochain);
		if (setimpl) {
			(void) setvar("*", prefix, 1);
			(void) setvar("<", setimpl, 1);
		} else if (inobj)
			(void) setvar("<", inobj, 1);
		(void) strcpy(abuf, mkqlist(achain));
		(void) setvar(">", abuf, 1);
