#ifndef lint
static char *RCSid = "$Id: misc.c,v 1.13 91/01/03 13:01:17 bww Exp $";
#endif
 ...
 * HISTORY
 * $Log:	misc.c,v $
 * Revision 5.3  91/10/14  13:42:17  berman
 * 	setvar() and eqsign() now return values.  Also use
 * 	the "envover" bit in support of the -E/-X switches.
 * 	From STUMP "[91/01/03  12:56:02  bww]".
 * 	Added a rudimentary form of translation tracing.
 * 	From STUMP "[90/08/27  00:07:22  bww]".
 * 	Also fixed bug in uniqtrans() which was eliding any
 * 	new word which was a suffix of a previous word, and
 * 	translation of $^ to $> for compatibility with GNU.
 * 	[91/10/10            bww]
 * 
 * Revision 5.2  90/08/06  18:22:16  mrt
 * 	Added code to recognize * as the same as .*
 * 	[90/07/13            mrt]
 * 
 * Revision 1.11  90/05/08  11:18:47  bww
 * 	Give the name of one of the macros in the
 * 	supposed infinite substitution recursion.
 * 	[90/05/08  11:18:15  bww]
 * 
 * Revision 1.10  90/05/07  16:00:51  bww
 * 	Changed fatal() to accept printf-style arguments, and
 * 	so did away with fatal1().  Purged unix conditional.
 * 	[90/05/07  15:54:40  bww]
 * 
 * Revision 1.9  90/04/30  15:22:19  bww
 * 	Added support for full substitution-style macro
 * 	translation via the facilities of regexp(3).
 * 	[90/04/30  15:13:05  bww]
 * 
 * Revision 1.8  90/03/03  18:29:13  bww
 * 	New setenviron() function for checking error
 * 	returns from setenv().  All errors are fatal.
 * 	[90/03/03  18:28:09  bww]
 * 
 * Revision 1.7  90/02/22  17:39:08  bww
 * 	Added forward declarations for static functions.
 * 	Also work around "hc" bug in unsetvar().
 * 	[90/02/22  17:34:45  bww]
 * 
 * Revision 1.6  90/02/20  18:33:12  bww
 * 	Propagate "onlysub" through conditional expressions.
 * 	[90/02/20  18:32:43  bww]
 * 
 * Revision 1.5  90/02/10  13:01:46  bww
 * 	Added support for unsetvar(), although there is no
 * 	user-level interface as yet.  Also removed readenv()
 * 	madness which refused to set null-valued variables.
 * 	[90/02/10  13:00:16  bww]
 * 
 * Revision 1.4  90/02/01  20:48:53  bww
 * 	Delinted.
 * 	[90/02/01  20:44:27  bww]
 * 
 * Revision 1.3  89/11/20  14:18:31  bww
 * 	Removed support for adding command line assignments to the
 * 	environment.  If you want sub-makes to see these, you will
 * 	have to make your own arrangements.
 * 	[89/11/20  14:16:36  bww]
 * 
 * Revision 1.2  89/05/26  10:05:56  bww
 * 	CMU CS as of 89/05/15
 * 	[89/05/26  09:46:50  bww]
 * 
 * Revision 5.1  89/05/12  18:50:24  bww
 * 	Added entry and exit banners under -d to clearly delimit
 * 	debug output in recursive makes.
 * 	[89/05/12  18:45:00  bww]
 * 
	...
	register char *a, *b;
{
	...
	while (*a) {
		...
		if (depth++ >= 100)
			fatal("%s: infinitely recursive macro?", vname);
	...
	char *vname;
{
	...
	if (amatch(vname, "[@*<][DF]"))  /* no lists, so no $? or $> */
...
{
	register char *p;
	static int transdebug = -1;
	extern char *getenv();

	if (transdebug == -1)
		transdebug = getenv("MAKE_TRANS_DEBUG") ? 1 : 0;
	for (p = vn; *p; p++)
		if (*p & QUOTE)
			*p &= ~QUOTE;
	if (*vb == 0)
		p = straightrans(b, vn);
	else if (amatch(vb, ":[uU]"))
		p = uniqtrans(b, vn);
	else if (amatch(vb, ":[ehrtEHRT]"))
		p = cshtrans(b, vn, *(vb+1));
	else if (xp->x_sepp && xp->x_func)
		p = (*xp->x_func)(b, vn, vb+1, xp->x_sepp, xp->x_endp, only);
	else
		fatal("Unrecognized modifier '%s'", vb);
	if (transdebug)
		printf("$(%s%s) => '%.*s'\n", vn, vb, p - b, b);
	return p;
}
	...
	char *s;
{
	...
	fatal("%s, line %d: %s", curfname, yylineno, s);
}
