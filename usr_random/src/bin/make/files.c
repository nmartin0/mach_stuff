#ifndef lint
static char *RCSid = "$Id: files.c,v 1.21 91/01/03 13:00:37 bww Exp $";
#endif
 ...
 * HISTORY
 * $Log:	files.c,v $
 * Revision 5.3  91/10/14  13:40:40  berman
 * 	setvar() now returns a value.
 * 	From STUMP "[91/01/03  12:52:18  bww]".
 * 	Added a rudimentary form of VPATH tracing.
 * 	From STUMP "[90/09/04  21:18:31  bww]".
 * 	[91/10/10            bww]
 * 
 * Revision 5.2  90/08/06  18:21:14  mrt
 * 	Picked up bww's changes from Mt Xinu
 * 	[90/08/06  17:22:25  mrt]
 * 
 * Revision 1.19  90/05/11  09:30:31  bww
 * 	Fixed bug in reldir() that corrupted the value of the
 * 	current working directory while fixing source paths.
 * 	[90/05/11  09:29:55  bww]
 * 
 * Revision 1.18  90/05/07  15:59:13  bww
 * 	fatal() now accepts printf-style arguments.
 * 	[90/05/07  15:51:19  bww]
 * 
 * Revision 1.17  90/05/07  12:41:10  bww
 * 	isprecious() now takes a nameblock.
 * 	[90/05/07  12:34:32  bww]
 * 
 * Revision 1.16  90/04/30  15:20:50  bww
 * 	Added external declaration for pfopen().
 * 	[90/04/30  15:08:47  bww]
 * 
 * Revision 1.15  90/03/29  13:26:36  bww
 * 	Added INCL_COMPRESS conditionals for deleting non-
 * 	existant directories from NON-MAKE_CPP CFLAGS list.
 * 	[90/03/29  13:25:28  bww]
 * 
 * Revision 1.14  90/03/03  18:28:48  bww
 * 	Tell submakes whether a chdir() was made so
 * 	that they may unset OBJECTS when necessary.
 * 	[90/03/03  18:22:51  bww]
 * 
 * Revision 1.13  90/02/22  17:38:30  bww
 * 	Avoid writing into literal strings.
 * 	[90/02/22  17:33:03  bww]
 * 
 * Revision 1.12  90/02/10  13:01:17  bww
 * 	Unset OBJECTDIR if no chdir() takes place.  Also
 * 	remove redundant null directories from VPATH and
 * 	MAKECPP, and set MAKECPP and MAKEMCH variables.
 * 	[90/02/10  12:57:35  bww]
 * 
 * Revision 1.11  90/02/02  12:34:40  bww
 * 	Added warning under "-m" about being in a machine subdir.
 * 	[90/02/02  12:34:00  bww]
 * 
 * Revision 1.10  90/02/01  20:48:05  bww
 * 	Included support for ${MAKETOP} and ${MAKESUB} variables.
 * 	The former is the relative path from the current directory
 * 	to the "source root" (i.e. all ..'s), while the latter is
 * 	the path back again.  Both end with a '/' is they were
 * 	otherwise non-null.  Also added docom() modifications.
 * 	[90/02/01  20:39:35  bww]
 * 
 * Revision 1.9  89/12/07  12:41:33  bww
 * 	Split MAKECPP into MAKECPP and MAKEMCH to correctly handle
 * 	multi-component, machine-specific include files.
 * 	[89/12/07  12:41:05  bww]
 * 
 * Revision 1.8  89/12/03  11:19:08  bww
 * 	${MACHINE} -> ${TARGET_MACHINE}.
 * 	[89/12/03  11:18:36  bww]
 * 
 * Revision 1.7  89/12/02  22:53:50  bww
 * 	Use HAS_MAKECPP definition.
 * 	[89/12/02  22:46:50  bww]
 * 
 * Revision 1.6  89/11/20  14:17:46  bww
 * 	Added missing setvpath().  Dynamically set "hascmurfs".
 * 	[89/11/20  14:13:54  bww]
 * 
 * Revision 1.5  89/10/02  11:34:06  bww
 * 	Changed ordering of $(MACHINE) directories under -m.
 * 	[89/10/02  11:33:40  bww]
 * 
 * Revision 1.4  89/09/06  11:53:18  bww
 * 	Changed "hasmakecpp" and "hascmurfs" conditionals to CMU.
 * 	[89/09/06  11:52:51  bww]
 * 
 * Revision 1.3  89/09/06  11:42:25  bww
 * 	Added kludge code to get around AFS RO/RW volume mismatch.
 * 	[89/09/06  11:41:24  bww]
 * 
 * Revision 1.2  89/05/26  10:05:38  bww
 * 	CMU CS as of 89/05/15
 * 	[89/05/26  09:46:50  bww]
 * 
 * Revision 5.1  89/04/13  20:45:36  bww
 * 	Made CMU specific behaviour patchable via "hascmurfs" and
 * 	"hasmakecpp" variables.  Avoid adding single "-I.".
 * 	[89/04/13  20:43:23  bww]
 * 
	...
	register int icd;
{
	...
	static int vpathdebug = -1;
	extern char *getenv();
	extern time_t lookarch();
	extern char *execat(), *canon();

	if (vpathdebug == -1)
		vpathdebug = getenv("MAKE_VPATH_DEBUG") ? 1 : 0;
	...
	struct stat *wb;
{
	...
	*s++ = '/';
	...
	if (wb && sb->st_mtime <= wb->st_mtime)
		...
		if (!keepgoing)
			fatal("%s: old RCS working file", fn);
	...
	register struct chain *ch;
{
	...
	for (; ch; ch = ch->nextp) {
		...
		nocmds = NO;
		(void) setvar("@", file, 1);
		(void) setvar("<", RCSfile, 1);
		i = docom(co_cmd);
		(void) setvar("@", (char *) 0, 1);
		nocmds = onocmds;
		if (i)
...
rm()
{
	...
	if (lp == 0)
		return;
	if (rmchain) {
		(void) setvar("?", mkqlist(rmchain), 1);
		(void) docom(lp);	
	}
	if (deschain) {
		(void) setvar("?", mkqlist(deschain), 1);
		onoexflag = noexflag;
		noexflag = NO;
		(void) docom(lp);	
		noexflag = onoexflag;
		...
		*p = 0;  *q = 0;
		if (stat(ofn, &sbuf) == -1) {
			...
			|| (sbuf.st_mode & S_IFMT) != S_IFDIR)
				fatal("mkobjdir: dir botch: %s", sfn);
			if (!silflag)
				printf("%smkdir %s\n", (noexflag
					? "" : prompt), ofn);
			if (mkdir(ofn, (int) sbuf.st_mode & 0777) == -1)
				fatal("Couldn't make directory: %s", ofn);
		} else if ((sbuf.st_mode & S_IFMT) != S_IFDIR)
			fatal("Not a directory: %s", ofn);
		for (*q++ = '/', *p++ = '/'; *q == '/'; p++, q++)
			;
	}
	if (!silflag)
	...
	*vpath = 0;  /* should configuration files be found on VPATH? */
	...
	if (machdep) {
		...
		if (strcmp(p, machine) == 0)
			fprintf(stderr, "Warning: make in machine directory\n");
	}
	ch = 0;
	...
	char *dir;
{
	...
	(void) setvar(MAKEDIR, copys(dir), 0);
	while (*dir == '/')
		dir++;
	p = temp;  q = dir;
	while (*q) {
	...
	*p = 0;
	(void) setvar(MAKETOP, copys(temp), 0);
	p = temp;  q = dir;
	for (q = dir; *q; q++)
		*p++ = *q;
	if (p != temp && *(p-1) != '/')
		*p++ = '/';
	*p = 0;
	(void) setvar(MAKESUB, copys(temp), 0);
}
	...
	char *pre, *suf;
{
	...
	char obj[MAXPATHLEN+1], curdir[1];
	...
	struct nameblock *p;
{
	...
	if (cwd) {
		...
		if (dbgflag)
			printf("%s=%s\n", MAKECONF, buf);
		(void) setvar(MAKECONF, copys(buf), 0);
	}
	if (p->RCSnamep) {
		(void) setvar("<", p->RCSnamep, 1);
		p->RCSnamep = 0;
		(void) subst(cmd, buf);
		if (!silflag)
			printf("%s%s\n", (noexflag ? "" : prompt), buf);
...
/*
 * CMU has a modified lib/cpp which uses MAKECPP
 * and MAKEMCH to handle include files correctly
 */
#ifndef	HAS_MAKECPP
#define	HAS_MAKECPP 0
#endif
int hasmakecpp		= HAS_MAKECPP;	/* patch me */
FSTATIC char MAKECPP[]	= "MAKECPP";
FSTATIC char MAKEMCH[]	= "MAKEMCH";
...
addincdirs()
{
	...
	*ovp = 0;
	p = vtop(ov, temp);
	setenviron(MAKECPP, p, 1);
	if (dbgflag)
		printf("%s=%s\n", MAKECPP, p);
	(void) setvar(MAKECPP, copys(p), 0);
	if (machdep) {
		setenviron(MAKEMCH, machine, 1);
		if (dbgflag)
			printf("%s=%s\n", MAKEMCH, machine);
		(void) setvar(MAKEMCH, copys(machine), 0);
	}
	if (hasmakecpp)
		return;
	if (ov[0] && *ov[0] == 0 && ov[1] == 0)
		return;
	cp = varptr(CFLAGS);
	p = temp;
	if (machdep) {
