...
/* machterm.c -- mach terminal utility
 *
 * Created by Zonnie L. Williamson at CMU, 1992
 *
 * This is a simple display server and terminal utility.  Its usage is:
 *   machterm [ -sh | -csh ]
...
#ifndef RAMDISK_MACHTERM

typedef char machterm_context_t[256];

#ifdef SMALL_MACHTERM

extern int small_machterm_setup(machterm_context_t *machterm_context,
                                char *display_buffer,
                                int display_height,
                                int display_width,
                                int display_rowsize,
...
#ifdef SMALL_MACHTERM
extern void small_machterm(machterm_context_t *machterm_context, char c);
#else
                       ...
                       int *display_rowsize)
{
  ...
  if (vm_allocate(mach_task_self(), (vm_address_t *)&buffer, size, TRUE)) goto error;
...
typedef struct process {
  ...
  long out_count;
  long time_stamp;
  machterm_context_t machterm_context;
} *process_t;
...
#ifdef SMALL_MACHTERM
  while (*s) {
    small_machterm(&p->machterm_context, *s++);
    p->out_count++;
  }
#else
  while (*s) {
    machterm(&p->machterm_context, *s++);
    p->out_count++;
  }
#endif
}
...
static char *help(process_t p, char *x)
{
  ...
  for (i = 0, n = -1; i < process_count; i++) {
    ...
    sprintf(&x[strlen(x)],
            ...
            process[i].in_count,
            process[i].out_count,
            now - process[i].time_stamp);
    process[i].time_stamp = now;
    process[i].in_count = 0;
    process[i].out_count = 0;
  }
  sprintf(&x[strlen(x)], "This is screen #%d.\n\r", n);
  return x;
...
#ifdef SMALL_MACHTERM
  result = small_machterm_setup(&p->machterm_context,
                                p->buffer,
                                p->height,
                                p->width,
                                p->rowsize,
                                ...
                                beep);
#else
  result = machterm_setup(&p->machterm_context,
                          p->buffer,
                          p->height,
                          p->width,
                          p->rowsize,
...
static void setup_virtual_terminal(process_t p, process_t root)
{
  ...
  if (!root->virtual_buffer) {
    ...
    if (vm_allocate(mach_task_self(),
                    (vm_address_t *)&root->virtual_buffer,
                    root->virtual_rowsize * root->height,
                    TRUE)) root->virtual_buffer = 0;
  }
  ...
  if (vm_allocate(mach_task_self(),
                  (vm_address_t *)&p->virtual_buffer,
                  p->virtual_rowsize * p->height,
                  TRUE)) p->virtual_buffer = 0;
...
static void switch_process(process_t last, process_t next)
{
  ...
  for (i = 0; i < process_count; i++)
    if ((process[i].device == next->device) &&
        ...
        !process[i].hidden) {
      machterm_buffer(&process[i].machterm_context,
                      process[i].virtual_buffer,
  ...
  machterm_buffer(&next->machterm_context, next->buffer, next->rowsize);
  next->hidden = 0;
        ...
        for (chars[n] = 0, s = chars; *s; s++) {
          if (*s != (']' - '@')) write(current->pty, s, 1);
            ...
            /* if ^]-? then give some help */
              ...
              process_puts(current, help(current, x));
...
static void display_server(int argc, char **argv)
{
  ...
  if (!ttyname(0) || strcmp((char *)ttyname(0), "/dev/console")) {
...
static void status_line(int argc, char **argv)
{
  ...
  /* send ^A to enter status line */
  write(tty, (char []){ 'A' - '@' }, 1);

  /* write text to status line */
  for (space = 0; argc > 0; argc--, argv++) {
