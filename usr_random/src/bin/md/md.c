...
newtoken: ;
	...
	order_files(&argc, argv);
	for (; argc--; argv++) {
		dep_file_index = 0;

		if (read_dep(*argv, IObuffer)) {
...
struct stat statbuf;
	...
	if ((size = read(fd, file_array, req_size)) < 0) {
		fprintf(stderr, "%s: file = \"%s\" ", name, file);
		perror("read");
		(void) fflush(stdout);
		(void) fflush(stderr);
...
register int c, waswhite = 0;

	/* copy a list of filenames, until a colon,
	   removing trailing whitespace,
	   and compressing inter-name whitespace
	   to a single space */
...
/*
 *	Sort a list of files, not by their names, but by the dot-o files for
 *	which they define dependencies.  Remove from the list those files
 *	that could not be read.
 */
order_files(cntp, file)
int *cntp;	/* file count (in/out) */
char *file[];	/* array of filenames */
{
	int cnt, newcnt, i, j;
	char *f;
	char (*saved_dot_o)[MAXPATHLEN+100];

	cnt = *cntp;
	/* Allocate space to hold all the dot_o files. */
	saved_dot_o = (char(*)[MAXPATHLEN+100]) malloc(cnt * (MAXPATHLEN+100));
	newcnt = 0;
	for (i = 0; i < cnt; i++) {
		f = file[i];
		if (read_dep(f, MAXPATHLEN+100)) {
			save_dot_o();
			/* Insert f and dot_o in lists, copying if necessary */
			j = newcnt;
			while ((j>0) && (strcmp(dot_o,saved_dot_o[j-1]) < 0)) {
				file[j] = file[j-1];
				strcpy(saved_dot_o[j], saved_dot_o[j-1]);
				j--;
			}
			file[j] = f;
			strcpy(saved_dot_o[j], dot_o);
			newcnt++;
		}
	}
	*cntp = newcnt;
	free((char *) saved_dot_o);
}
