 ...
 * HISTORY
 * $Log:	supcmeat.c,v $
 * Revision 1.6  90/01/10  17:16:38  bww
 ...
 * 
 * Revision 1.3  89/08/15  15:31:15  bww
...
register TREE *t;
{
	...
	(void) sprintf (buf,FILEWHEN,collname);
...
int listfiles ()
{
	...
	(void) sprintf (buf,FILELAST,collname);
	f = fopen (buf,"r");
	if (f) {
...
int x;
{
	...
	(void) sprintf (fname,FILEWHEN,collname);
	...
	(void) sprintf (fname,FILELAST,collname);
...
#endif
{
	...
	if (protver < 6) {
		if (goawayreason)
			free (goawayreason);
		goawayreason = (fmt) ? (char *)salloc (buf) : (char *)NULL;
		(void) msggoaway ();
	}
	else {
		doneack = value;
