...
void FlushICache()
{
  (void) mach_msg_trap((mach_msg_header_t *)&flush_icache_msg,
		       MACH_SEND_MSG,
...
void MemoryDispatch(os_reg_t regs)
{
  stop(&regs, 0);
}
...
void SndDisposeChannel(tbox_reg_t regs)
{
  stop(&regs, 1);
}

void SndAddModifier(tbox_reg_t regs)
{
  stop(&regs, 2);
}

void SndDoCommand(tbox_reg_t regs)
{
  stop(&regs, 3);
}

void SndDoImmediate(tbox_reg_t regs)
{
  stop(&regs, 4);
}

void SndPlay(tbox_reg_t regs)
{
  stop(&regs, 5);
}

void SndControl(tbox_reg_t regs)
{
  stop(&regs, 6);
}

void SndNewChannel(tbox_reg_t regs)
{
  P_BEGIN
...
void enqueue(register QElemPtr entry, register QHdrPtr queue)
{
  int prio;
...
dequeue(register QElemPtr entry, register QHdrPtr queue)
{
...
void Enqueue(tbox_reg_t regs)
{
  (void)enqueue((QElemPtr)regs.a_0, (QHdrPtr)regs.a_1);
}

void Dequeue(tbox_reg_t regs)
{
  regs.d_0 = dequeue((QElemPtr)regs.a_0, (QHdrPtr)regs.a_1);
}
...
static void initSCSI()
{
  ...
  for (drv = (DrvQElPtr)DrvQHdr.qHead; drv; drv = (DrvQElPtr)drv->qLink) {
    i = ~drv->dQRefNum - 0x20;
    if (i >= 0 && i <= 6) {
      if (ndrv[i]++ == 0)
        drvq[i] = drv;
  ...
  for (i = 0; i <= 6; i++) {
    if (dctl = (DCtlHandle)UTableBase[0x20 + i])
      scsiInstall(*dctl, i, drvq[i], ndrv[i]);
  }
}

static void initSONY()
{
  register DrvQElPtr drv;
  register DCtlHandle dctl;
  static int ndrv;
  ...
  if (dctl = (DCtlHandle)UTableBase[0x4])
...
static void initETHER()
{
  ...
  for (i = 1; i < e; i++) {
    if (dctl = (DCtlHandle)UTableBase[i]) {
      ...
      if (d->drvrName.length == 5
        && !strncmp(".ENET", (char *)d->drvrName.text, 5)) {
...
static void initSOUND()
{
  ...
  if (dctl = (DCtlHandle)UTableBase[0x3]) {
