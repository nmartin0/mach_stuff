...
#ifdef MACH
/* Nonzero means print details of include file search. */
/* Set nonzero if CPP_INCL_DEBUG environment variable is defined. */
static int cpp_incl_debug = 0;
#endif
...
#ifdef MACH
  /* set cpp_incl_debug non-zero if CPP_INCL_DEBUG exists */
  cpp_incl_debug = (getenv("CPP_INCL_DEBUG") != 0);
#endif
...
get_filename:
  ...
  if (*fbeg == '/') {
    ...
    fname[flen] = 0;
#ifdef MACH
    if (cpp_incl_debug) fprintf(stderr, "CPP_INCL_DEBUG: %s\n", fname);
#endif
