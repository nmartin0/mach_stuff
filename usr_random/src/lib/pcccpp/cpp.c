 ...
 *	Added predefines and include directories, symsiz=7500 and -MD for mac2.
 */
...
#ifdef mac2
STATIC	int mdflag;	/* makefile dependencies to a file */
#endif
...
# endif
	...
	if (mflag) {
		...
		if (mdflag) {
			tf=(char *)rindex(infile, '.');
			tf[1]='d';
			if (NULL==(mout=fopen(infile, "w"))) {
				pperror("Can't open dependency file: %s", infile);
				exit(8);
			}
			tf[1]='o';
		}
...
# endif
# if mac2
	dirs[nd++] = "/usr/mach/include";
	dirs[nd++] = "/usr/include/cmu";
# endif
# if ibm
# ifndef gimpel
	dirs[nd++] = "BTL$CLIB";
...
# endif
#ifdef mac2
	ysysloc=stsym("mc68000");
	ysysloc=stsym("mc68881");
	ysysloc=stsym("mc68020"); /* what about 68030? */
	ysysloc=stsym("CMU");
	ysysloc=stsym("MACH");
	ysysloc=stsym("CMUCS");
	ysysloc=stsym("BYTE_MSF");
#endif
