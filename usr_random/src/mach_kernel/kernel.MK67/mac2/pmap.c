...
#ifdef MODE24
/*
 * Describe special mappings that must be entered in the
 * kernel pmap when mach is booted on a 24 bit system.
 */
#define PMAP_ROM_MODE24_ADDR	((vm_offset_t)0x00800000)
#define PMAP_ROM_MODE24_SIZE	((vm_size_t)(1*1024*1024))

#define PMAP_IO_MODE24_ADDR	((vm_offset_t)0x00f00000)
#define PMAP_IO_MODE24_SIZE	((vm_size_t)(1*1024*1024))

#define PMAP_SLOT_MODE24_ADDR(slot) \
    				((vm_offset_t)((slot) << 20))
#define PMAP_SLOT_MODE24_SIZE	((vm_size_t)(1*1024*1024))

#endif /* MODE24 */

/*
 * Describe special mappings that must be entered as
 * virt == phys in the kernel pmap at startup.
 */
...
/*
 * Count of pt pages allocated at boot.  Can be patched.
 */
 ...
 * return a ptr to the corresponding pt_desc.
 */
...
/*
 * Return the size of memory mapped by one page-pointer table.
 */
#define	pt_page_map_size	(mac2_ptob(PAGE_SIZE / sizeof(pp_t)))
...
/*
 * list of above pages that contain at least one free ptptbl
 */
 ...
 * Initialize a bitmap.
 */
static inline void init_bitmap_byte(unsigned char *bp)
{
    asm("st	%0@" : : "a" (bp));
}
 ...
 * returns -1 if no free elements, otherwise the bit offset.
 */
static inline int alloc_bitmap_byte(unsigned char *bp)
{
    int bit;
 ...
 * NB: the element number is not checked for range errors.
 */
static inline int free_bitmap_byte(unsigned char *bp, int bit)
{
    bit = 7 - bit;
...
static inline unsigned long phys_to_pfn(phys_offset_t phys)
{
  union phys_to_pfn {
  ...
  return ((union phys_to_pfn)phys).f.pfn;
}

static inline phys_offset_t pfn_to_phys(unsigned long pfn)
{
  union {
    phys_offset_t full_offset;
    struct {
...
static inline unsigned long vm_offset_to_index(vm_offset_t v)
{
  return ((union vm_offset_fields)v).pg_index;
}

static inline vm_offset_t index_to_vm_offset(unsigned long i)
{
  union vm_offset_fields v;
 ...
 * page-table-pointer.
 */
static inline ptp_t *ptptbl_to_ptp(ptp_t p[], vm_offset_t v)
{
  return &p[((union vm_offset_fields)v).ptptbl_index];
}
 ...
 * page-pointer.
 */
static inline pp_t *pptbl_to_pp(pp_t p[], vm_offset_t v)
{
  return &p[((union vm_offset_fields)v).pptbl_index];
}
 ...
 * virtual offset.
 */
static inline vm_offset_t ptp_to_va(ptp_t *p)
{
  union {
 ...
 * page descriptors and is indexed by the next lower 11 (or 12) bits of
 * of the virtual address, 2048 (or 4096) entries.  Each pptbl consists
 * of 8192 (or 4096) bytes (a machine dependent memory page), and is
 * allocated as a pt_page by the pmap module.  Each of these
 * page-pointers (pp) can point to a physical memory page, which
 * is indexed by the lower 13 (or 12) bits of the virtual address.
 *
 ...
 *	Statistics
 */
int pt_page_active_count;
int pt_page_free_count;
int pt_page_wired_count;
int pt_page_max_active;
int pt_page_max_wired;
int pt_root_page_alloc_count;
int pt_root_page_free_count;
...
{								\
    if ((s) < (e) &&						\
	(e) - (s) < ATC_PAGE_INVAL_MAX * PAGE_SIZE) {		\
	register vm_offset_t _va;				\
\
	if (is68030) {						\
	    for (_va = (s); _va < (e); _va += PAGE_SIZE)     \
	        pmmu_flush_supr(_va);				\
	}							\
	else {							\
	    for (_va = (s); _va < (e); _va += PAGE_SIZE)     \
...
#ifdef MODE24
/*
 * A special translation tree is setup for 24 bit mode.
 */
typedef struct mode24_pmap {
    phys_offset_t	root;
    struct pt_root_page *
			ptrt;
    pp_t		*
...
static struct mode24_pmap	kernel_mode24_pmap_store;
mode24_pmap_t			kernel_mode24_pmap;
#endif /* MODE24 */

/*
 * Zone used to allocate pmap structures.
 */
zone_t		pmap_zone;

/*
 * Page-table-pointer tables exist 1-to-1 with pmaps and
 * are created at pmap creation time.
 */
#define		pmap_ptptbl_size	(256*sizeof(ptp_t))

/*
 * Number of page-pointers required to map one machine indepedent page.
...
static inline ptp_t * root_to_ptptbl(register vm_offset_t page,
                                     register phys_offset_t root)
{
  union root_to_ptptbl {
    phys_offset_t full_offset;
    struct {
      phys_offset_t
                  :19,
  ...
  x.full_offset = page;
  x.f.index = ((union root_to_ptptbl)root).f.index;

  return (ptp_t *)x.full_offset;
}
 ...
 */
static inline pp_t *ptp_to_pptbl(register ptp_t *p, register pmap_t pmap)
{
  if (pmap == kernel_pmap)
 ...
 * page-pointer.
 */
static inline pp_t *ptp_to_pp(register ptp_t *p, register pmap_t pmap, register vm_offset_t v)
{
 ...
 */
inline pp_t *pmap_pp(register pmap_t pmap, register vm_offset_t va)
{
  register ptp_t *ptp;

  ptp = ptptbl_to_ptp(root_to_ptptbl(pmap->ptrt->page, pmap->root), va);
  if (ptp->valid != PMMU_VALID_PTP) return PP_NULL;
 ...
 */
inline ptp_t *pmap_ptp(register pmap_t pmap, register vm_offset_t va)
{
  return (ptptbl_to_ptp(root_to_ptptbl(pmap->ptrt->page, pmap->root), va));
}

static inline phys_offset_t pmap_phys(pmap_t pmap, register vm_offset_t va)
{
  register pp_t		*pp;
  union {
    phys_offset_t	full_offset;
    struct {
      phys_offset_t	pfn:32 - PAGE_SHIFT,
		        offset:PAGE_SHIFT;
    } f;
  } pa;

  if ((pp = pmap_pp(pmap, va)) == PP_NULL) {
    pp = (pp_t *)pmap_ptp(pmap, va);
    if (pp->valid == PMMU_VALID_PP) {
      pa.full_offset = ((union vm_offset_fields)va).ptptbl_offset;
      pa.f.pfn += pp->pfn;
    }
    else pa.full_offset = (phys_offset_t)0;
  }
  else {
    pa.f.pfn = pp->pfn;
    pa.f.offset = ((union vm_offset_fields)va).pg_offset;
  }
 ...
 */
void pmap_update_atc(register pmap_t pmap,
...
static pmmu_prot_map_init(void)
{
  register comb;

  for (comb = 0; comb < 8; comb++) switch (comb) {
    case VM_PROT_NONE:
 ...
 */
static vm_offset_t pmap_map(register vm_offset_t va,
 ...
 */
static void pmap_alloc_kernel_pt(vm_offset_t *aphys_free,
                                 vm_offset_t va,
                                 register vm_size_t size)
{
    ptp_t		*ptp;
...
/*
 * Map memory in kernel pmap using early termination pp entries in the ptptbl.
 * Each mapping is worth 16MB.
 */
static void pmap_map_trunc(register vm_offset_t va,
                           register vm_offset_t phys,
                           register vm_size_t size,
                           register int prot)
{
...
#ifdef MODE24
/*
 * Setup mappings in the 24 bit mode translation tree.
 */
static void pmap_map_mode24(register vm_offset_t va,
                            register vm_offset_t phys,
                            register vm_size_t size,
                            register int prot)
{
    ...
    pp_t		template;
    int			mprot;
#define each		PAGE_SIZE

    prot = pmmu_prot_map[prot];
    mprot = pmmu_prot(prot);

    pp = pptbl_to_pp(kernel_mode24_pmap->pptbl, va);
...
#ifdef MODE24
/*
 * Allocate the one page table used for the 24 bit mode
 * translation tree.
 */
static void pmap_alloc_kernel_mode24_pt(vm_offset_t *aphys_free)
{
  ...
  ptp = root_to_ptptbl(kernel_mode24_pmap->ptrt->page,
		       kernel_mode24_pmap->root);
  if (ptp->valid == PMMU_VALID_PTP) return;
  a = *aphys_free;
  *aphys_free += PAGE_SIZE;
  bzero(a, PAGE_SIZE);
  kernel_mode24_pmap->pptbl = (pp_t *)a;
  phys = Log2Phys(a);
  ptp->pfn = phys_to_pfn(phys);
  ptp->valid = PMMU_VALID_PTP;
  template = *ptp;
  eptp = ptp+(pmap_ptptbl_size/sizeof (ptp_t));
  for (; ptp < eptp;) *ptp++ = template;
}
#endif /* MODE24 */

#ifdef MODE24
/*
 * Change mapping protections in the 24 bit mode tree.
 */
static void pmap_protect_mode24(register vm_offset_t start,
                                register vm_offset_t end,
                                register int prot)
{
    register pp_t	*pp;
...
#define each		PAGE_SIZE
    ...
    pp = pptbl_to_pp(kernel_mode24_pmap->pptbl, start);
 ...
 */
static boolean_t pmap_setup(vm_offset_t *aphys_free,
...
#ifdef MODE24
    kernel_mode24_pmap->root = kernel_pmap->root + (2 * pmap_ptptbl_size);
#endif /* MODE24 */

#ifdef MODE24
    pmap_alloc_kernel_mode24_pt(aphys_free);
#endif /* MODE24 */
...
#ifdef MODE24
#define pmap_map_mode24_kernel(va, pa, size, prot)	\
    {								\
	pmap_map_mode24((va), (pa), (size), (prot));		\
    }
#endif /* MODE24 */
	...
	if (mem_size > 0x800000)
	    mode24_mem_size = 0x800000;
	else
	    mode24_mem_size = mem_size;
    }
#endif /* MODE24 */
	...
	switch (bp->usage) {
	  ...
	  case kUsageRAM:
#ifdef MODE24
	    if (!mac32bit && a < mode24_mem_size) {
		s = round_page(bp->physical.rangeSize);
		if ((a + s) > mode24_mem_size)
		    s -= (a + s - mode24_mem_size);
		pmap_map_mode24_kernel(a,
				       bp->physical.address,
				       s,
	  ...
	  case kUsageBootGlobals:
	    if (bp->physical.rangeSize >= PAGE_SIZE) {
#ifdef MODE24
		if (!mac32bit && a < mode24_mem_size) {
		    s = round_page(bp->physical.rangeSize);
		    if ((a + s) > mode24_mem_size)
...
#ifdef MODE24
    if (!mac32bit)
	pmap_protect_mode24((vm_offset_t)&start,
			    round_page(&etext),
			    VM_PROT_READ);
#endif /* MODE24 */
...
#undef	pmap_map_kernel_mode24
    ...
    *aphys_free += round_page(sizeof (pt_desc_t) * pt_page_alloc_count);
	...
	for (ptpg = pt_desc_tbl; a < pt_last_kern; ptpg++, a += PAGE_SIZE) {
...
#ifdef MODE24
    if (!mac32bit && (mem_size > mode24_mem_size)) {
	askip = pg_last_kern;
    ...
    *aphys_free += round_page(sizeof (pg_desc_t) *
	...
	for (pd = pg_desc_tbl; a < pg_last_kern; a += PAGE_SIZE, pd++) {
#ifdef MODE24
	    if (mac32bit || (mem_size == mode24_mem_size))
		pd->kern = a;
	    else {
		if (a < mode24_mem_size) {
		    if (a >= askip)
			;
		    else
			pd->kern = a;
...
/*
 * Setup the kernel pmap on older (pre-ci) machines.
 */
static void pmap_setup_direct(vm_offset_t *aphys_free,
                              vm_offset_t *aphys_last,
                              vm_offset_t *avirt_free,
                              vm_offset_t *avirt_last)
{
...
#ifdef MODE24
    pmap_map_kernel(a, 0, mode24_mem_size, VM_PROT_READ|VM_PROT_WRITE);
#else /* MODE24 */
    pmap_map_kernel(a, 0, mem_size, VM_PROT_READ|VM_PROT_WRITE);
#endif

#ifdef MODE24    
    if (mem_size > mode24_mem_size)
	pmap_map_kernel(a,
			mode24_mem_size,
			mem_size - mode24_mem_size,
			VM_PROT_READ|VM_PROT_WRITE);
#endif /* MODE24 */
...
#ifdef MODE24
#define pmap_map_kernel_mode24(addr, size, prot, phys) \
    {									\
	pmap_map_mode24((addr), (phys), (size), (prot));		\
    }
#endif /* MODE24 */
...
#ifdef MODE24
#ifdef	PMAP_ROM_MODE24_ADDR
    pmap_map_kernel_mode24(PMAP_ROM_MODE24_ADDR,
			   PMAP_ROM_MODE24_SIZE,
			   PMAP_ROM_PROT,
			   PMAP_ROM_ADDR);
#endif
#endif /* MODE24 */
...
#ifdef MODE24
#ifdef	PMAP_IO_MODE24_ADDR
    pmap_map_kernel_mode24(PMAP_IO_MODE24_ADDR,
			   PMAP_IO_MODE24_SIZE,
			   PMAP_IO_PROT,
			   PMAP_IO_ADDR);
#endif
...
#ifdef MODE24
#ifdef	PMAP_SLOT_MODE24_ADDR
    pmap_map_kernel_mode24(PMAP_SLOT_MODE24_ADDR(slot),
			   PMAP_SLOT_MODE24_SIZE,
			   PMAP_SLOT_PROT,
			   PMAP_SLOT_ADDR(slot));
#endif
#endif /* MODE24 */
...
#ifdef MODE24
    kernel_mode24_pmap = &kernel_mode24_pmap_store;
#endif /* MODE24 */
...
#ifdef MODE24
    kernel_mode24_pmap->ptrt = kernel_pmap->ptrt;
#endif /* MODE24 */

    /*
     * zero it.
     */
    bzero(kernel_pmap->ptrt->page, PAGE_SIZE);
...
static inline boolean_t pmap_valid_page(register vm_offset_t pa)
{
  register bank_ptr_t	bp;

  if ((phys_offset_t)pa >= BANKB_PHYS) bp = &bankB;
  else bp = &bankA;
  if (bp->pg_desc_tbl != PG_DESC_NULL &&
      (phys_offset_t)pa >= bp->pg_first_phys &&
      (phys_offset_t)pa < bp->pg_last_phys) {
    if (pg_desc(pa)->kern != 0) return TRUE;
  }
  return FALSE;
}

boolean_t pmap_next_page(register vm_offset_t *addrp)
{
  while (!pmap_valid_page(avail_next)) {
    if (avail_next == avail_end) return FALSE;
...
/*
 * Allocate a free pt page.  The affected pmap must be locked.
 * Returns NULL if there are no free pt pages.
 */
static pt_desc_ptr_t pt_free_page_alloc(register pmap_t pmap, vm_offset_t va)
{
  register pt_desc_ptr_t	ptpg;
  ...
  if (ptpg->pmap != PMAP_NULL || ptpg->va != (vm_offset_t) 0) panic("ptpg in use!");
 ...
 */
static pt_desc_ptr_t pt_page_alloc(pmap_t pmap, vm_offset_t va)
{
  register pt_desc_ptr_t	ptpg;
  ...
  if (queue_empty(&pt_page_queue_free)) {
    ...
    if (ptpg == PT_DESC_NULL) panic("not enough pt pages!");

    /* free the pt page */
    old_pmap = ptpg->pmap;
    old_va = ptpg->va;
    spp = pmap_pp(old_pmap, old_va);
    epp = spp + PAGE_SIZE / sizeof(pp_t);
 ...
 */
static void pt_page_free(register pt_desc_ptr_t ptpg)
{
 ...
 */
static void pt_page_wire(pt_desc_ptr_t ptpg)
{
  simple_lock(&pt_page_queue_lock);
 ...
 * Unwire a pt page.
 */
static void pt_page_unwire(pt_desc_ptr_t ptpg)
{
  simple_lock(&pt_page_queue_lock);
...
/*
 * Allocate a pt root and return the pt root page
 * structure pointer and the pt root physical address.
 */
void pt_root_alloc(register pt_root_desc_ptr_t *aptrt,
 ...
 * Free a pt root.
 */
static void pt_root_free(register pt_root_desc_ptr_t ptrt,
                         register phys_offset_t root)
{
    register ndx;

#define pt_root_to_index(p)	(((p)&(PAGE_SIZE-1))>>10)
    /*
...
#undef pt_root_to_index
	...
	kmem_free(kernel_map, ptrt->page, PAGE_SIZE);
 ...
 * Called at ipl high.
 */
void pmap_set_kern_addr_mode(register boolean_t to32bit)
{
  PMMU_RP_reg			rp;
  ...
  if (to32bit) rp.phys = kernel_pmap->root;
  else rp.phys = kernel_mode24_pmap->root;
...
#ifdef MODE24
/*
 * Setup an alternate root page that simulates 24 bit addressing.
 */
boolean_t pmap_do_24bit_map(register pmap_t pmap)
{
    register pt_desc_ptr_t	ptpg;
    register ptp_t		*ptp;
    register			ipl;
...
#ifdef MODE24
/*
 * Remove references on behalf of alternate root.
 */
void pmap_undo_24bit_map(register pmap_t pmap)
{
    register pt_desc_ptr_t	ptpg;
    register ptp_t		*ptp;
    register			ipl;
 ...
 */
kern_return_t pmap_attribute(register pmap_t pmap,
                             vm_offset_t addr,
                             vm_size_t size,
...
void pmap_remove_attributes(pmap, start, end)
{
}
 ...
 * no valid mappings.
 */
void pmap_destroy(register pmap_t pmap)
{
    register int	c, ipl;
 ...
 */
void pmap_reference(register pmap_t pmap)
{
  register ipl;
 ...
 *
 */
static void pmap_remove_range(pmap_t pmap,
                              vm_offset_t start,
                              pp_t *spp,
                              pp_t *epp,
 ...
 * bits to the pager.
 */
static void pmap_remove_all(vm_offset_t pa)
{
 ...
 */
static void pmap_copy_on_write(vm_offset_t pa)
{
...
/*
 * Set the physical protection on the specified range of this pmap as requested.
 */
void pmap_protect(pmap_t pmap, vm_offset_t start, vm_offset_t end, vm_prot_t prot)
{
    register ptp_t	*sptp, *eptp;
    register pp_t	*spp, *epp;
    register int	mprot;
 ...
 */
void pmap_enter(register pmap_t pmap,
                vm_offset_t va,
 ...
 */
void pmap_change_wiring(register pmap_t pmap,
                        vm_offset_t va,
                        boolean_t wired)
{
    register pp_t	*pp;
 ...
 */
vm_offset_t pmap_extract(register pmap_t pmap, vm_offset_t va)
{
    int			ipl;
    phys_offset_t	pa;
...
{
    if (pa >= vm_first_phys && pa < vm_last_phys)
	bzero(pg_desc(pa)->kern, PAGE_SIZE);			/* XXX */
}
...
register phys_offset_t	src, dst;
{
    ...
    bcopy((vm_offset_t)src, (vm_offset_t)dst, PAGE_SIZE);	/* XXX */
}
...
unsigned int pmap_list_resident_pages(pmap_t pmap,
