...
machine_startup()
{
    char C[] = { 27, 'n', 'a', 27, 'o', 0 }; /* copyright symbol */
    extern int		start, etext, edata, end;
    extern int		hz;
    ...
    ivect_init();
    set_vector_base((unsigned long)ivect_tbl);

    /*
     * Setup the kernel pmap and switch to the kernel translation
     * tree.  After this returns, we can no longer do MacOS calls.
     */
    pmap_bootstrap();

    /*
     * Initialize the interface to the macintosh operating system code.
     * After this returns, we can again do MacOS calls.
     */
    os_interface_init();

    /*
     * Find the console screen and initialize it enough so that
     * kernel printfs work.
     */
    console_printf_init();
    ...
    printf("\n%s\n", version);
    printf("Mach Operating System\n");
    printf("Copyright %s 1992 Carnegie Mellon University.\n", C);
    printf("All Rights Reserved.");
    printf("\n");
    printf("MacMach was developed by CMU with support from Apple Computer, Inc.\n");
    printf("Use of this software is constrained by the MacMach End-User license.\n");
    printf("\n");
#ifdef MODE24
    ...
    pmap_protect(kernel_pmap,
		 ...
		 (vm_offset_t)ivect_tbl + PAGE_SIZE,
		 VM_PROT_READ);
...
vm_offset_t	offset;
{
    if (!kern_valid_region_check(offset, PAGE_SIZE, prot))
	return (-1);
