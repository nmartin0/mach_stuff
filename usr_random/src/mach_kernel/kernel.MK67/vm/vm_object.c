...
void vm_object_init()
{
	...
	vm_object_template.pager_request = IP_NULL;
 ...
 *
 *	Gets another reference to the given object,
 *	uncaching it if it was cached.
 */
void vm_object_incr(object)
	register vm_object_t	object;
{
	if (object == VM_OBJECT_NULL)
		return;
	...
	register vm_object_t	object;
{
	...
	if (mo != IP_NULL) {
		/* consumes our rights for mo, pager_request, pager_name */
		(void) memory_object_terminate(mo,
...
{
	ipc_port_t	old_object, old_control, old_name;
 ...
 *		VM_OBJECT_NULL.
 */
kern_return_t	vm_object_copy_slowly(src_object, src_offset, size,
				interruptible, dest_wired, _result_object)
	register
	vm_object_t	src_object;
	vm_offset_t	src_offset;
	vm_size_t	size;
	boolean_t	interruptible;
	boolean_t	dest_wired;
	...
	vm_object_t	*_result_object;	/* OUT */
{
	...
	ipc_kobject_set(new_memory_object,
			(ipc_kobject_t) src_object, IKOT_PAGER);
...
vm_object_t	vm_object_lookup(pager)
	ipc_port_t	pager;
{
	return (vm_object_t) pager;
}
#define	vm_object_lookup(pager)	real_vm_object_lookup(pager)
#endif	NORMA_VM
...
#if	NORMA_VM
	/* printf("norma_vm: avoiding dealloc of old_request\n"); */
#else
	...
	boolean_t	internal;
{
	...
	vm_object_cache_lock();
#if	NORMA_VM
	/* XXX we also need a while loop, probably the same while loop */
	if (ip_kotype(pager) == IKOT_XMM_PAGER) {
		/*
		 *	We must atomically replace xmm_pager association
		 *	with vm_object association. We store the old
		 *	xmm_pager in new_object->pager_request,
		 *	so that it can be retrieved by memory_object_init.
		 */
		...
		if (ip_kotype(pager) == IKOT_XMM_PAGER) {
			/* yuck */
			new_object->pager_request =
			    (ipc_port_t) pager->ip_kobject;
...
#endif	NORMA_VM
	while ((po = ip_kotype(pager)) == IKOT_NONE) {
		/*
...
#if	NORMA_VM
			if (new_object->pager_request) {
				panic("vm_object_enter: pager_request");
			}
#endif	NORMA_VM
			ipc_kobject_set(pager,
					(ipc_kobject_t) new_object,
	...
	if (must_init) {
		...
		if (!internal /* XXX object->temporary */ ) {
			object->pager_name = ipc_port_alloc_kernel();
		...
		if (backing_object->ref_count == 1) {
			...
			switch (vm_object_collapse_debug) {
				...
				if ((backing_object->pager == IP_NULL) &&
				    (backing_object->pager_request == IP_NULL))
				    break;
				/* Fall through to... */
			...
			if (object->pager_request != IP_NULL)
#if	NORMA_VM
				xmm_object_set(object->pager_request, object);
#else	NORMA_VM
				ipc_kobject_set(object->pager_request,
						(ipc_kobject_t) object,
						IKOT_PAGING_REQUEST);
 ...
 *		with this object.
 */
ipc_port_t	vm_object_name(object)
	vm_object_t	object;
{
	ipc_port_t	p;
 ...
 */
void
vm_object_page_map(object, offset, size, map_fn, map_fn_data)
	vm_object_t	object;
	vm_offset_t	offset;
	vm_size_t	size;
	vm_offset_t	(*map_fn)(/* char *map_fn_data, vm_offset_t offset */);
	char *		map_fn_data;	/* private to map_fn */
{
	int	num_pages;
	int	i;
	vm_page_t	m;
