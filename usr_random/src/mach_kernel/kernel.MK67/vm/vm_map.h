...
#define max_offset		hdr.links.end	/* end of range */
	...
	boolean_t		wait_for_space;	/* Should callers wait
						   for space? */
	boolean_t		wiring_allowed;	/* can users wire pages? */
...
#if	NORMA_IPC
/*
 *	Workaround for missing page list continuation support
 *	in NORMA_IPC.  Is it worth putting this array at the
 *	end of the structure and allowing it to be variable sized??
 */
...
/*
 *	Useful macros for page list copies.
 */
...
/*
 *	Continuation macros for entry list copies
 */
...
#define vm_map_entry_wait(map, interruptible)    	\
        MACRO_BEGIN                                     \
        assert_wait(&(map)->hdr, interruptible);	\
        vm_map_unlock(map);                             \
	thread_block((void (*)()) 0);			\
        MACRO_END

#define vm_map_entry_wakeup(map)        thread_wakeup(&(map)->hdr)
