...
void vm_page_replace(mem, object, offset)
	register vm_page_t	mem;
	register vm_object_t	object;
	register vm_offset_t	offset;
{
 ...
 */
void vm_page_init(mem, phys_addr)
	vm_page_t	mem;
...
vm_page_t vm_page_grab_fictitious()
{
	register vm_page_t m;
...
void vm_page_release_fictitious(m)
	register vm_page_t m;
{
	simple_lock(&vm_page_queue_free_lock);
	if (m->free)
 ...
 *	Conditions:
 *		Nothing locked.  If successful, returns non-wired memory
 *		in the ipc_kernel_map which the caller must consume.
 *	Returns:
	...
	mach_msg_type_number_t *countp;
{
	...
	kr = vm_allocate(ipc_kernel_map, (vm_offset_t *) &info,
			 (vm_size_t) (vm_page_bucket_count * sizeof *info),
			 TRUE);
	if (kr != KERN_SUCCESS)
	...
	for (i = 0; i < vm_page_bucket_count; i++) {
		...
		info[i].hib_count = count;
	}

	*infop = info;
	*countp = vm_page_bucket_count;
	return KERN_SUCCESS;
}
#endif	MACH_VM_DEBUG
