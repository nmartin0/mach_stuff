	...
	register vm_map_t	map;
{
	...
	(void) vm_map_delete(map, map->min_offset, map->max_offset, FALSE);
 ...
 *
 *		Assumes that it is being called on a main map (not
 *		a sharing map); the map must be locked.
 *
 ...
 *	Assumptions:
 *		This routine assumes that it is being called on
 *		a main map (not a sharing map).
 */
 ...
 */		
void vm_map_entry_delete(map, entry, keep_wired)
	register vm_map_t	map;
	register vm_map_entry_t	entry;
	boolean_t		keep_wired;
{
	...
	if (entry->wired_count != 0) {
	    /*
	     * If keep_wired, do not unwire temporary
	     * objects.
	     */
	    if (!(keep_wired && object->temporary)) {
		vm_fault_unwire(map, entry);
		entry->wired_count = 0;
		entry->user_wired_count = 0;
	    }
 ...
 *
 *	Does not unwire temporary objects if keep_wired is
 *	true.  This cooperates with vm_map_copyin, which
 *	moves wired temporary objects to the copy (instead of
 *	copying the pages) if destroying the source.
 */

kern_return_t vm_map_delete(map, start, end, keep_wired)
	register vm_map_t	map;
	register vm_offset_t	start;
	register vm_offset_t	end;
	boolean_t		keep_wired;
{
	...
	while ((entry != vm_map_to_entry(map)) && (entry->vme_start < end)) {
		...
		vm_map_entry_delete(map, entry, keep_wired);
		entry = next;
	}
	...
	register vm_offset_t	end;
{
	...
	vm_map_lock(map);
	VM_MAP_RANGE_CHECK(map, start, end);
	result = vm_map_delete(map, start, end, FALSE);
	pmap_remove_attributes(map->pmap, start, end);
	vm_map_unlock(map);
...
start_pass_1:
	...
	for (entry = tmp_entry;;) {
		...
		/*
		 *	Check for share maps and for permanent objects
		 *	in the destination.
		 */
		...
		if (entry->is_a_map) {
			...
			if (interruptible) {
				vm_map_t	sub_map = entry->object.share_map;
	...
	while (vm_map_copy_first_entry(copy) != vm_map_copy_to_entry(copy)) {
		...
		if (entry->is_a_map) {
			...
			sub_map = entry->object.share_map;
			vm_map_lock(sub_map);
			(void) vm_map_lookup_entry(
					sub_map,
					entry->offset,
...
StartAgain:
	...
	if (last == vm_map_to_entry(dst_map) ||
	    ...
	    last->wired_count != 0) { /*  wc == 0 => user_wired_count == 0 */
		goto create_object;
	}
...
insert_pages:
	...
	*dst_addr = start + (copy->offset - trunc_page(copy->offset));
...
error:
	...
	if (result != KERN_SUCCESS)
		vm_map_delete(dst_map, start, end, FALSE);
 ...
 *		The source map should not be locked on entry.
 */
kern_return_t vm_map_copyin(src_map, src_addr, len, src_destroy, keep_wired,
			    copy_result)
	vm_map_t	src_map;
...
#define	RETURN(x)						\
	...
	while (TRUE) {
		...
		if (src_destroy &&
		    ...
		    /*
		     * Wire the copy if the source is wired and we
		     * want to carry over wired mappings.  If we
		     * do not, the object`s pages will be unwired when
		     * the source is destroyed.
		     */
		    if (was_wired) {
			new_entry->wired_count = (keep_wired) ? 1 : 0;
		    }
	 ...
	 * copy was successful.  Keep objects wired if the copy entries
	 * are wired.
	 */
	if (src_destroy)
...
{
	vm_map_copy_t	copy;		/* Resulting copy */
	vm_map_entry_t	entry;		/* entry in copy object */
...
vm_map_copy_t		*copy_result;	/* OUT */
{
	...
	kfree(cont_args, sizeof(vm_map_copyin_args_data_t));
...
do_map_lookup:
	...
	while (TRUE) {
		...
		if (src_entry->is_a_map) {
			vm_map_entry_t	tmp_entry;
			vm_size_t	sub_size;
			vm_map_t	sub_map = src_entry->object.share_map;

			can_steal = FALSE;

			vm_map_lock(sub_map);

			if (!vm_map_lookup_entry(sub_map, src_entry->offset,
						 &tmp_entry))
			       panic("vm_map_copyin_page_list: submap lookup");

			/*
			 *	The sharing map entry may reduce the size
			 *	we can do in this pass.
			 */

			assert(src_entry->offset >= tmp_entry->vme_start);
			sub_size = tmp_entry->vme_end - 
				(src_entry->offset +
			...
			src_object = tmp_entry->object.vm_object;
			src_offset = tmp_entry->offset +
			  (src_entry->offset - tmp_entry->vme_start) +
			  (src_start - src_entry->vme_start);
			
			/*
			 *	If src_object is NULL, allocate it now;
			 *	we're going to fault on it shortly.
			 *	Must do it here because the sub_map lock
			 *	must be held.
			 */
			if (src_object == VM_OBJECT_NULL) {
				src_object = vm_object_allocate((vm_size_t)
					tmp_entry->vme_end -
					tmp_entry->vme_start);
				tmp_entry->object.vm_object = src_object;
			}
...
make_continuation:
			    ...
			    cont_args->src_len = src_end - src_start;
			    if (src_destroy) {
			...
			if (((m = vm_page_lookup(src_object, src_offset)) !=
				 ...
				 *	can happen, hence:
				 *	  (1) No page stealing.
				 *	  (2) Must lookup map entry before
				 *		continuing.
				 *
				 * XXX	May want to allow page stealing
				 * XXX	if this is the first page in range.
				 */
				vm_map_unlock(src_map);
				can_steal = FALSE;
...
retry:
			 ...
			 *	TRUE.
			 *
			 *	If the page is not at the top of the
			 *	shadow chain, then it can't be stolen.
			 *	(this is a slight oversimplification, but
			 *	we assume that shadow chains with ref counts
			 *	of 1 down the chain get collapsed elsewhere).
			 */
			if (src_object != m->object)
				can_steal = FALSE;
	...
	if (steal_pages) {
		...
		for (i = 0; i < copy->cpy_npages; i++) {
			 ...
			 *	      not shared (no sharing map, object at top
			 *	      of shadow chain, obj ref count of 1)
			 *	(2) The page is not precious.
			 *
			 *	The no sharing map and object at top
			 *	of shadow chain checks are performed in
			 *	the main loop above.  can_steal is FALSE
			 *	if either check fails.  This means that
			 *	one bad page disables stealing for the entire
			 *	list, but that's the breaks.  The common
			 *	cases are 1 page, and all pages alike.
			 *
		...
		if (src_destroy) {
		    (void) vm_map_delete(src_map, src_start, src_end, FALSE);
		}
	}
	else {
		/*
	...
	for (
		...
		case VM_INHERIT_SHARE:
			...
			if (!old_entry->is_a_map) {
				...
				vm_map_entry_copy(new_share_entry, old_entry);
