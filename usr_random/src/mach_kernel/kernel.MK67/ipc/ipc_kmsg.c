	...
	vm_offset_t eaddr;
{
	...
	boolean_t remote_ports;

	msg = (mach_msg_header_t *) saddr - 1;
	remote_ports = (msg->msgh_bits & MACH_MSGH_BITS_COMPLEX_PORTS);
#endif	NORMA_IPC
	...
	while (saddr < eaddr) {
		...
		if (is_port) {
			...
			for (i = 0; i < number; i++) {
				...
				if (remote_ports) {
					/*
					 * XXX Should map back from
					 * XXX uid to object.
					 * XXX For now, we just punt.
					 */
					printf("clean_body: ");
					printf("skipping remote port\n");
					continue;
				}
#endif	NORMA_IPC
				if (!IO_VALID(object))
	...
	mach_msg_type_number_t number;
{
	...
	if (dolast) {
...
#if	NORMA_IPC
		boolean_t remote_ports =
		    (mbits & MACH_MSGH_BITS_COMPLEX_PORTS);
#endif	NORMA_IPC
		...
		if (type->msgtl_header.msgt_longform) {
			...
			name = type->msgtl_header.msgt_name;
			size = type->msgtl_header.msgt_size;
			rnumber = type->msgtl_header.msgt_number;
 ...
 *
 *		The implementation of OOL memory holding port rights
 *		uses copyin(), implicitly assuming that the supplied
	...
	mach_port_t notify;
{
...
#if	NORMA_IPC
	is_remote = IP_NORMA_IS_PROXY((ipc_port_t) dest);
	assert(! (is_remote && use_page_lists));
#endif	NORMA_IPC
	...
	while (saddr < eaddr) {
		...
		is_inline = type->msgtl_header.msgt_inline;
		dealloc = type->msgtl_header.msgt_deallocate;
		if (longform) {
			name = type->msgtl_name;
			size = type->msgtl_size;
			number = type->msgtl_number;
		...
		if (is_inline) {
			...
			if (length == 0)
				...
				if (is_remote) {
					...
					kr = norma_ipc_copyin_page_list(map,
				        	addr, length, dealloc,
						&copy, FALSE);
				} else
#endif	NORMA_IPC
		...
		if (is_port) {
			...
			if (longform)
				...
				type->msgtl_header.msgt_name = newname;
...
#if	NORMA_IPC
				if (is_remote) {
					ipc_port_t ip = (ipc_port_t) object;
					norma_ipc_send_port(ip, newname);
...
#if	NORMA_IPC
	is_remote = IP_NORMA_IS_PROXY((ipc_port_t) remote);
#endif	NORMA_IPC
	...
	while (saddr < eaddr) {
...
#if	NORMA_IPC
			if (is_remote && ! is_port) {
				kmsg->ikm_header.msgh_bits |=
				    MACH_MSGH_BITS_COMPLEX_DATA;
				/*
				 * XXX This can fail.  Now what???
				 * XXX If the error were detected
				 * XXX before the MiG stub was invoked
				 * XXX the stub would do the right thing,
				 * XXX but it's a bit late now.
				 */
		...
		if (is_port) {
			...
			for (i = 0; i < number; i++) {
				...
				if (is_remote) {
					ipc_port_t ip = (ipc_port_t) object;
					norma_ipc_send_sright(ip, newname);
					objects[i] = (ipc_object_t)
					    ip->ip_norma_uid;
				}
#endif	NORMA_IPC
 ...
 *		Used by ipc_kserver loop to send replies
 *		(which are prepared in copied-out form).
	...
	ipc_kmsg_t kmsg;
{
	...
	while (saddr < eaddr) {
		...
		if (is_inline) {
			 ...
			 *	The sender should supply ready-made memory
			 *	for us, so we don't need to do anything
			 *	beyond translating into page-list copy object.
			 */
		...
		if (is_port) {
			...
			for (i = 0; i < number; i++) {
				...
				ports[i] = (ipc_port_t) ports[i]->ip_norma_uid;
			}
		}
	}
}
	...
	vm_map_t map;
{
	...
	while (saddr < eaddr) {
		...
		if (is_inline) {
			...
			type->msgtl_header.msgt_deallocate = FALSE;
			saddr += (length + 3) &~ 3;
		} else {
			vm_offset_t data = * (vm_offset_t *) saddr;
			...
			if (length == 0) {
				...
				(void) copyout((char *) data, (char *) addr,
					       length);
				kfree(data, length);
			} else {
			...
			type->msgtl_header.msgt_deallocate = TRUE;
 ...
 *
 *		The implementation of OOL memory holding port rights
 *		uses copyout(), implicitly assuming that the supplied
 *		map is the current map.
 *	Conditions:
 *		Nothing locked.
 *	Returns:
...
#if	NORMA_IPC
		/* XXX I think we need to do something here */
#endif	NORMA_IPC
		mr |= ipc_kmsg_copyout_body(saddr, eaddr, space, map);
	}
