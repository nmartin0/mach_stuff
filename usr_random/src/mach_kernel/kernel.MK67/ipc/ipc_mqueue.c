...
mach_msg_return_t
ipc_mqueue_send(kmsg, option, timeout)
	ipc_kmsg_t kmsg;
...
#if	NORMA_IPC
	/* XXX should eventually assert that we are not here? */
	if (IP_NORMA_IS_PROXY(port)) {
		mach_msg_return_t mr;
		/*
		 *	We will eventually need to check queue length,
		 *	which we try to keep as up to date as possible.
		 */
		mr = norma_ipc_send(kmsg, option, timeout);
		ip_unlock(port);
		return mr;
	}
#endif	NORMA_IPC
...
mach_msg_return_t
ipc_mqueue_receive(mqueue, option, max_size, timeout,
...
#if	NORMA_IPC
				/* evidently this happens on norma systems */
