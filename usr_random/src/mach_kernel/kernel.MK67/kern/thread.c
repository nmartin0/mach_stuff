...
#ifdef	KEEP_STACKS
	stack_alloc(new_thread, thread_continue);
#endif	KEEP_STACKS
	/*
	...
	boolean_t		must_halt;
{
	...
	while (TRUE) {
		 ...
		 */
		if ((((thread->swap_func == (void (*)()) mach_msg_continue) ||
		      (thread->swap_func == (void (*)())
...
thread_t kernel_thread(task, start, arg)
	task_t	task;
	void	(*start)();
	char	*arg;
{
 ...
 */
void
thread_set_own_priority(priority)
int	priority;
{
    int		s;
...
kern_return_t host_stack_usage(host, reservedp, totalp,
			       spacep, residentp,
			       maxusagep, maxstackp)
	host_t		host;
	vm_size_t	*reservedp;
...
kern_return_t processor_set_stack_usage(pset, totalp,
					spacep, residentp,
					maxusagep, maxstackp)
	processor_set_t	pset;
	unsigned int	*totalp;
