...
#include <vm/vm_map.h>
#ifdef mac2 /* kernel has no machparam.h */
...
thread_t thread_select(myprocessor)
...
boolean_t thread_invoke(old_thread, continuation, new_thread)
	register thread_t old_thread;
	void (*continuation)();
	register thread_t new_thread;
{
	/*
	 *	On multi-processors, save_context must switch to a
	 *	special per-processor stack.  This will let another
	 *	processor resume old_thread after we put it back in
	 *	the run queues.
	 */
	...
	if (save_context() == 0) {
		...
		PMAP_DEACTIVATE_USER(vm_map_pmap(old_thread->task->map),
				     old_thread, cpu_number());
		PMAP_ACTIVATE_USER(vm_map_pmap(new_thread->task->map),
				   new_thread, cpu_number());

		active_threads[cpu_number()] = new_thread;

		old_thread->swap_func = (void (*)()) 0;
	    ...
	    (continuation != (void (*)()) 0))
	{
		...
		case TH_SWAPPED:
			...
			case TH_RUN | TH_WAIT | TH_SUSP:
				...
				thread_wakeup((int)&old_thread->wake_active);
 ...
 *	Called at splsched.
 */
void thread_continue(old_thread)
	register thread_t old_thread;
{
	register void (*continuation)() = current_thread()->swap_func;
...
thread_t choose_pset_thread(myprocessor,pset)
register processor_t myprocessor;
