	...
	thread_t thread;
{
	...
	switch (state & TH_SCHED_STATE) {
		...
		/*
		 *	Interruptible wait - leave thread suspended
		 */
...
boolean_t
thread_handoff(old, continuation, new)
	...
	register thread_t new;
{
	...
	assert(old->stack_privilege != current_stack());
	if ((new->state != (TH_WAIT|TH_SWAPPED)) ||
	...
	if (old->state == TH_RUN) {
		...
		if (old->wake_active) {
			...
			thread_unlock(old);
			thread_wakeup((int)&old->wake_active);
