...
extern void eml_init(), eml_task_reference(), eml_task_deallocate();
 ...
 */
task_t	kernel_task_create(parent_task, map_size)
	task_t		parent_task;
	vm_size_t	map_size;
{
	task_t		new_task;
...
boolean_t	assign_threads;
{
	...
	if (task->assign_active) {
		...
		thread_wakeup((int)&task->assign_active);
	}
	task_unlock(task);
	if (prev_thread != THREAD_NULL)
		thread_deallocate(prev_thread);		/* may block */
