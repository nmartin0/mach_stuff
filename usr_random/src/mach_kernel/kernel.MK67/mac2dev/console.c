...
extern int small_machterm_setup(int terminal_context,
                                char *display_buffer,
                                int display_height,
                                int display_width,
                                int display_rowsize,
...
extern void small_machterm(int terminal_context, char c);

#else

extern int machterm_setup(int terminal_context,
                          char *display_buffer,
                          int display_height,
                          int display_width,
                          int display_rowsize,
...
extern void machterm(int terminal_context, char c);
...
static console_output(register struct tty *tp)
{
  ...
  for (;;) {
    ...
    c = getc(&tp->t_outq);
    if (c <= 127 || tp->t_flags & LITOUT) machterm(0, c);
    else {
...
io_return_t console_open(dev_t dev, int flag, io_req_t ior)
{
  ...
  if ((tp->t_state&TS_ISOPEN) == 0) {
    if (console_adb_query(ADB_ADDR_KEYBOARD, -1, 0) != ADB_ADDR_KEYBOARD)
  ...
  if ((tp->t_state&TS_ISOPEN) == 0) {
    if (!console_adb_open(ADB_ADDR_KEYBOARD, adb_input)) return (D_IO_ERROR);
  }
  result =  char_open(dev, tp, flag, ior);
  if (result != D_SUCCESS) {
    if ((tp->t_state & TS_ISOPEN) == 0) console_adb_close(ADB_ADDR_KEYBOARD);
    return result;
  }
...
{
  console_adb_close(ADB_ADDR_KEYBOARD);
  cancelact(key_data.key_act);
...
io_return_t console_getstat(dev_t dev,
                            int flavor,
                            int *data,
                            unsigned int *count)
{
...
io_return_t console_setstat(dev_t dev,
                            int flavor,
                            int *data,
                            unsigned int count)
{
  /* TTY_SET_BREAK resets the console */
...
/* if non-zero, then OK to do printf() */
boolean_t printf_enable;
...
console_printf_init()
{
  ...
  VPBlockPtr v;
  vm_offset_t devbase;
  extern boolean_t console_video_init(vm_offset_t *devbase, VPBlockPtr *v);
  extern void ttyinput(char, struct tty *);

  s = spl7();
  /* if anything fails here, the panic may not be displayed... */
  if (console_video_init(&devbase, &v) &&
#ifdef SMALL_MACHTERM
      !small_machterm_setup(0,
                            (char *)(devbase + v->vpBaseOffset),
                            v->vpBounds.bottom - v->vpBounds.top,
                            v->vpBounds.right - v->vpBounds.left,
                            v->vpRowBytes,
                            0, 0, 0, 0, /* auto-select font */
                            'e', /* closed oval cursor */
                            0, /* black on white */
                            (void (*)(char, char*))ttyinput,
                            (char *)&console_tty,
                            0)) printf_enable = TRUE;
#else
      !machterm_setup(0,
                      (char *)(devbase + v->vpBaseOffset),
                      v->vpBounds.bottom - v->vpBounds.top,
...
cngetc()
{
  panic("mac2 does not support cngetc");
}
