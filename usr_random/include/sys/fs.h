 ...
 * HISTORY
 * $Log:	fs.h,v $
 * Revision 2.2  89/11/29  15:29:37  af
 * 	Added FS_CLEAN flag definition.
 * 	[89/11/27            af]
 * 
 * Revision 2.1  89/08/04  14:45:38  rwd
 * Created.
 * 
 * Revision 2.3  88/08/24  02:28:21  mwyoung
 * 	Adjusted include file references.
 * 	[88/08/17  02:13:03  mwyoung]
 * 
 *
 * 19-Apr-88  Mike Accetta (mja) at Carnegie-Mellon University
 *	Removed FS_?NOSPC definitions (these are now defined under
 *	slightly different names in <sys/user.h>).
 *	[ V5.1(XF23) ]
 *
 * 09-Apr-88  Mike Accetta (mja) at Carnegie-Mellon University
 *	Revised freefrags() macro to impose lower bound on full blocks
 *	and flushed obsolete 4.1-based file system structure
 *	organization.
 *	[ V5.1(XF23) ]
 *
...
#if	CMUCS
	/*
	 *  It is convenient to have these values easily accessible in this
	 *  structure so we may as well claim this reserved space for internal
	 *  use for now.  These values are zeroed in the fields of the
	 *  structure written back to the file system when the super-block is
	 *  updated so as not to corrupt the file system.
	 */
#define	fs_fhiwat fs_sparecon[1]	/* fragment high water mark */
#define	fs_flowat fs_sparecon[2]	/* fragment low water mark */
#define	fs_ihiwat fs_sparecon[3]	/* inode high water mark */
#define	fs_ilowat fs_sparecon[4]	/* inode low water mark */
#endif	CMUCS
...
#define FS_CLEAN 30			/* was unmounted properly flag */
...
/* 
 *  Determine the absolute number of available frags.  We must worry about the
 *  pathological case when most blocks are in use but sufficient fragments
 *  remain to exceed the high water mark (which will prevent any resource pause
 *  from taking effect) and arbitrarily require at least 100 fragments worth of
 *  blocks before indicating any.
 */
#define freefrags(fs)					\
(							\
    ((fs)->fs_cstotal.cs_nbfree > fragstoblks((fs), 100))\
    ?							\
