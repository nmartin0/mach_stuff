	...
	cthread_t p;
{
	 ...
	 */
	if (cthread_stack_size_override == 0)
	    cthread_status.stack_size = stack_top - stack_bottom;
	else
	    cthread_status.stack_size = cthread_stack_size_override;
	 ...
	 */
	return cthread_stack_base(p, sizeof(cthread_t));
}
	...
	cthread_t p;
{
	...
	for (base = next_stack_base;
	     vm_allocate(mach_task_self(), &base, cthread_status.stack_size, FALSE) != KERN_SUCCESS;
	     base += cthread_status.stack_size)
		;
	next_stack_base = base + cthread_status.stack_size;
	setup_stack(p, base);
}
