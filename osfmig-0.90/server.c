    ...
    statement_t *stats;
{
	...
	fprintf(file, "static struct %s %s = {\n", ServerSubsys, ServerSubsys);
    }
    fprintf(file, "\t0,\n");
    fprintf(file, "\t%d,\n", SubsystemBase);
    ...
    register argument_t *arg;
{
	...
	int	multiplier = btype->itSize / (8 * btype->itNumber);
        ...
        if (IsKernelServer)
            fprintf(file, "\t%svm_map_copy_discard(%s);\n", 
	...
	fprintf(file, "\t%s = (vm_offset_t) 0;\n", 
	    InArgMsgField(arg, ""));
	if (arg->argKPD_Type == MACH_MSG_OOL_PORTS_DESCRIPTOR)
	  fprintf (file, "\tIn%dP->%s.count = (mach_msg_size_t) 0;\n",
    ...
    register argument_t *arg;
{
	...
	(it->itOutName == MACH_MSG_TYPE_PORT_SEND)) {
	    ...
	    fprintf(file, "\tif (IP_VALID((ipc_port_t)%s))\n", 
		    InArgMsgField(arg, ""));
	    fprintf(file, "\t\t%sipc_port_release_send((ipc_port_t)%s);\n",
		    SubrPrefix, InArgMsgField(arg, ""));
    }
}
    ...
    register argument_t *arg;
{
    ...
    fprintf(file, "\t    %sipc_port_check_circularity((ipc_port_t) OutP->%s.name, (ipc_port_t) In0P->Head.msgh_reply_port))\n",
