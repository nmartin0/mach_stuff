    ...
    statement_t *stats;
{
    ...
    if (!BeAnsiC) {
        fprintf(file, "#if\t%s\n", NewCDecl);
        fprintf(file, "#else\t/* %s */\n", NewCDecl);
        fprintf(file, "extern mach_port_t %smig_get_reply_port();\n",
		SubrPrefix);
        fprintf(file, "extern void %smig_dealloc_reply_port();\n",
		SubrPrefix);
...
static void
WriteOneMachErrorDefine(file, name, timeout)
    FILE *file;
    char *name;
    boolean_t timeout;
{
    ...
    fprintf(file, "\t\t%smig_dealloc_reply_port("
    ...
    routine_t *rt;
{
	...
	fprintf(file, "\tInP->%s = %smig_get_reply_port();\n",
		rt->rtReplyPort->argMsgField, SubrPrefix);
    ...
    routine_t *rt;
{
    ...
    if (IsKernelUser)
    {
	fprintf(file, "\tmsg_result = %smach_msg_send_from_kernel(",
		SubrPrefix);
	fprintf(file, "&InP->Head, %s);\n", SendSize);
    }
    else
    {
	fprintf(file, "\tmsg_result = %smach_msg_overwrite(&InP->Head, MACH_SEND_MSG|%s, %s, 0,",
		SubrPrefix, rt->rtMsgOption->argVarName,
		SendSize);
    ...
    char *success;
{
	...
	fprintf(file,
		"\t\t%smig_dealloc_reply_port(InP->Head.msgh_reply_port);\n",
		SubrPrefix);
#else
    ...
    routine_t *rt;
{
    ...
    fprintf(file, "\tmsg_result = %smach_msg_overwrite(&Out0P->Head, MACH_RCV_MSG|%s%s%s, 0, sizeof(Reply), InP->Head.msgh_local_port, %s, MACH_PORT_NULL, ",
	    SubrPrefix, rt->rtMsgOption->argVarName,
    ...
    routine_t *rt;
{
	...
	fprintf(file, "\tmsg_result = %smach_msg_overwrite(&InP->Head, MACH_SEND_MSG|MACH_RCV_MSG|%s%s%s%s, %s, sizeof(Reply), InP->Head.msgh_reply_port, %s, MACH_PORT_NULL, ",
	    SubrPrefix,
	    rt->rtMsgOption->argVarName,
	    rt->rtUserImpl != 0 ? "|MACH_RCV_TRAILER_TYPE(MACH_MSG_TRAILER_FORMAT_0)" : "",
    ...
    register argument_t *arg;
{
    ...
    fprintf(file,
	"\tOut%dP = (Reply *) ((pointer_t) Out%dP + msgh_size_delta - %d);\n\n",
    ...
    register argument_t *arg;
{
	...
	if (argType->itString) {
	     ...
	     */
	    fprintf(file, "\t(void) %smig_strncpy(%s%s, %s->%s, %d);\n",
		SubrPrefix,
		ref,
		arg->argVarName,
		who,
    ...
    register argument_t *arg;
{
	...
	case MACH_MSG_OOL_DESCRIPTOR:
		...
		GetArraySize(arg, size);
		fprintf(file, "\t(void)%vm_read(%smach_task_self(),\n",
			SubrPrefix, SubrPrefix);
    ...
    register argument_t *arg;
{
	...
	case MACH_MSG_OOL_DESCRIPTOR:
		...
		fprintf(file, "\t(void)%vm_deallocate(%smach_task_self(),",
			SubrPrefix, SubrPrefix);
