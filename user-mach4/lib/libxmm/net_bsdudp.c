 ...
 * HISTORY
 * $Log: net_bsdudp.c,v $
 * Revision 1.2  1995/05/04  07:08:49  sclawson
 * changed includes: cthreads.h -> mach/cthreads.h
 *
 * Revision 2.4  93/04/14  11:48:32  mrt
 * 	Added some casts to correspond to the protypes.
 * 	[92/12/12            mrt]
 * 
 * Revision 2.3  92/01/22  22:53:47  rpd
 * 	Fixed includes to use "" when appropriate.
 * 	[92/01/18            rpd]
 * 
 * Revision 2.2  91/07/06  15:08:03  jsb
 * 	First checkin.
 * 
 */
/*
	...
	int len;
{
	...
	for (;;) {
		sin_size = sizeof(sin);
		rv = recvfrom(udp_server_socket, buf, len, 0, 
	...
	int length;
{
	...
	rv = sendto(udp_server_socket, (void *) data, length, 0, 
