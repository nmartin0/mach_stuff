...
/*
 * HISTORY
 * $Log: xmm_pagingfile.c,v $
 * Revision 1.1.1.1  1995/05/04  06:56:42  sclawson
 * New files.
 *
 * Revision 2.3  92/01/22  22:54:48  rpd
 ...
 * 	[92/01/18            rpd]
 * 
 * Revision 2.2  91/07/06  15:01:57  jsb
 * 	First checkin.
 * 
 */
/*
	...
	vm_prot_t desired_access;
{
	...
	rv = read(MOBJ->fd, data, vm_page_size);
	if (rv != vm_page_size) {
		perror("read");
		K_DATA_ERROR(kobj, offset, length, KERN_FAILURE);
		return;
	...
	vm_size_t length;
{
	...
	rv = write(MOBJ->fd, data, vm_page_size);
	if (rv != vm_page_size) {
		K_DATA_ERROR(kobj, offset, length, KERN_FAILURE);/* ??? */
		perror("write");
