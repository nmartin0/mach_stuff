...
/*
 * HISTORY
 * 28-Nov-93  Johannes Helander (jvh) at Helsinki University of Technology
 *	Check that utmp opening succeeds. Try /var/run/utmp first and
 *	/etc/utmp as a second try.
 *	Cast first arg to fread to void * to avoid gcc warnings.
 *
 * $Log: w.c,v $
 * Revision 1.5  1996/03/01  14:51:41  sclawson
 * Changes to install man pages.
 *
 * Revision 1.4  1995/09/28  23:13:44  mike
 * strXXXn (sys5) -> strnXXX (posix)
 *
 * Revision 1.3  1995/09/28  23:03:14  mike
 * include param.h before table.h
 *
 * Revision 1.2  1995/05/04  07:06:03  sclawson
 * various additions for the parisc.
 *
 * Revision 2.3  92/01/25  13:28:27  rpd
 * 	Removed bogus fread declaration.
 * 	[92/01/25            rpd]
 * 
...
#include <sys/dir.h>
#include <sys/time.h>
#ifdef TBL_MACHFACTOR		/* XXX not BSDSS */
#include <sys/user.h>
#else
#define BSDSS
#endif
...
#ifdef BSDSS
/* bsdss */
struct tbl_loadavg tmf;
#define tmf_machfactor tl_mach_factor
#define tmf_lscale tl_lscale
#define TBL_MACHFACTOR TBL_LOADAVG
#else
/* ux */
	...
	char **argv;
{
	...
	ut = fopen("/var/run/utmp","r");
	if (ut == NULL)
	    ut = fopen("/etc/utmp","r");
	if (ut == NULL) {
		printf("Could not open either /var/run/utmp or /etc/utmp");
		exit(1);
	}
	time(&now);
	if (header) {
		...
		while (fread((void *) &utmp, sizeof(utmp), 1, ut)) {
			if (utmp.ut_name[0] != '\0')
				nusers++;
		}
		rewind(ut);
	...
	for (;;) {	/* for each entry in utmp */
		if (fread((void *) &utmp, sizeof(utmp), 1, ut) == 0) {
			fclose(ut);
			exit(0);
		}
		if (utmp.ut_name[0] == '\0')
