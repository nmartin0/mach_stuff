 ...
 * New files.
 *
 * Revision 2.6  93/04/14  11:42:27  mrt
 * 	64bit cleanup.
 * 	[92/12/02            af]
 * 
...
uref_overflow_test_4()
{
    ...
    fillin_msg_type(&msg.type, MACH_MSG_TYPE_MOVE_SEND, 8*sizeof(mach_port_t), 1,
		    TRUE, FALSE);
    msg.port = port;
    ...
    check_msg_type(&msg.type, MACH_MSG_TYPE_PORT_SEND, 8*sizeof(mach_port_t), 1,
		   TRUE, FALSE);
    if (msg.port != port)
