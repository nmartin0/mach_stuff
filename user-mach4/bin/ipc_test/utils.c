 ...
 * New files.
 *
 * Revision 2.6  93/04/14  11:42:36  mrt
 * 	64bit cleanup.
 * 	[92/12/02            af]
 * 
...
fillin_msg_type_port(type)
    mach_msg_type_t *type;
{
    fillin_msg_type(type, MACH_MSG_TYPE_COPY_SEND, sizeof(mach_port_t)*8, 1, TRUE, FALSE);
}
...
check_msg_type_port(type)
    mach_msg_type_t *type;
{
    check_msg_type(type, MACH_MSG_TYPE_PORT_SEND, sizeof(mach_port_t)*8, 1, TRUE, FALSE);
}
...
fillin_msg_type_dummy_pointer(type)
    mach_msg_type_t *type;
{
    fillin_msg_type(type, MACH_MSG_TYPE_INTEGER_T, 8*sizeof(vm_offset_t), 0, FALSE, FALSE);
}
...
check_msg_type_dummy_pointer(type)
    mach_msg_type_t *type;
{
    check_msg_type(type, MACH_MSG_TYPE_INTEGER_T, 8*sizeof(vm_offset_t), 0, FALSE, TRUE);
}
...
fillin_msg_type_dummy_ports(type)
    mach_msg_type_t *type;
{
    fillin_msg_type(type, MACH_MSG_TYPE_COPY_SEND, sizeof(mach_port_t)*8, 0, FALSE, FALSE);
}
...
check_msg_type_dummy_ports(type)
    mach_msg_type_t *type;
{
    check_msg_type(type, MACH_MSG_TYPE_PORT_SEND, sizeof(mach_port_t)*8, 0, FALSE, TRUE);
}
...
fillin_msg_type_port_name(type)
    mach_msg_type_t *type;
{
    fillin_msg_type(type, MACH_MSG_TYPE_PORT_NAME, sizeof(mach_port_t)*8, 1, TRUE, FALSE);
}
...
check_msg_type_port_name(type)
    mach_msg_type_t *type;
{
    check_msg_type(type, MACH_MSG_TYPE_PORT_NAME, sizeof(mach_port_t)*8, 1, TRUE, FALSE);
}
    ...
    mach_msg_type_name_t poly_type;
{
    ...
    check_msg_type(&msg.polyType, poly_type, 8*sizeof(mach_port_t), 1, TRUE, FALSE);
    ...
    mach_msg_return_t result;
{
	...
	check_msg_type(&msg.reply.polyType, copyin_type(msgt_name),
		       8*sizeof(mach_port_t), 1, TRUE, FALSE);
    ...
    mach_port_t *port;
{
    ...
    check_msg_type(&msg.not_type, MACH_MSG_TYPE_MOVE_RECEIVE,
		   sizeof(mach_port_t)*8, 1, TRUE, FALSE);
