 ...
 * New files.
 *
 * Revision 2.6  93/04/14  11:42:13  mrt
 * 	64bit cleanup.
 * 	[92/12/02            af]
 * 
    ...
    unsigned int type;
{
    ...
    fillin_msg_type(&msg.t, type, 8*sizeof(mach_port_t), 1, TRUE, FALSE);
    msg.p = port;
    ...
    check_msg_type(&msg.t, type, 8*sizeof(mach_port_t), 1, TRUE, FALSE);
    if (msg.p != port)
