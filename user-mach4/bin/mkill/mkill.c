 ...
 * HISTORY
 * $Log: mkill.c,v $
 * Revision 1.1.1.1  1995/05/04  06:56:51  sclawson
 * New files.
 *
 * Revision 2.2  92/01/22  23:05:16  rpd
	...
	switch (type) {
	  ...
	  case MACH_TYPE_TASK:
	    switch (action) {
	      case Default:
	      case Kill:
		kr = machid_task_terminate(machid_server_port,
					   machid_auth_port,
					   id);
		break;
	      case Stop:
		kr = machid_task_suspend(machid_server_port, machid_auth_port,
					 id);
		break;
	      case Continue:
		kr = machid_task_resume(machid_server_port, machid_auth_port,
					id);
		break;
	      default:
		goto badaction;
	  ...
	  case MACH_TYPE_THREAD:
	    switch (action) {
	      case Default:
	      case Kill:
		kr = machid_thread_terminate(machid_server_port,
					     machid_auth_port,
					     id);
		break;
	      case Stop:
		kr = machid_thread_suspend(machid_server_port,
					   machid_auth_port,
					   id);
		break;
	      case Continue:
		kr = machid_thread_resume(machid_server_port, machid_auth_port,
					  id);
		break;
	      case Abort:
		kr = machid_thread_abort(machid_server_port, machid_auth_port,
					 id);
		break;
	      default:
		goto badaction;
	  ...
	  case MACH_TYPE_PROCESSOR_SET:
	    switch (action) {
	      case Default:
	      case Kill:
		kr = machid_processor_set_destroy(machid_server_port,
						  machid_auth_port,
						  id);
		break;
	      default:
	  ...
	  case MACH_TYPE_PROCESSOR:
	    switch (action) {
	      case Default:
	      case Stop:
		kr = machid_processor_exit(machid_server_port,
					   machid_auth_port,
					   id);
		break;
	      case Continue:
		kr = machid_processor_start(machid_server_port,
					    machid_auth_port,
					    id);
		break;
	      default:
