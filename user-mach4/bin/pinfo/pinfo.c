 ...
 * New files.
 *
 * Revision 2.3  93/04/14  11:44:24  mrt
 * 	64bit cleanup.
 * 	[92/12/02            af]
 * 
 * Revision 2.2  92/01/22  23:06:54  rpd
 * 	Moved to a separate directory.
 * 	[92/01/22            rpd]
 * 
 * Revision 2.6  92/01/17  14:24:49  rpd
 * 	Updated for CountInOut.
 * 	[92/01/17  14:12:49  rpd]
 * 
    ...
    mach_port_t *name;
{
    ...
    *name = MACH_PORT_NULL;
	...
	kr = machid_mach_register(machid_server_port, machid_auth_port,
				  mach_task_self(), MACH_TYPE_TASK, &task);
	if (kr != KERN_SUCCESS)
    ...
    char *argv[];
{
	...
	kr = machid_task_suspend(machid_server_port, machid_auth_port, task);
	if (kr != KERN_SUCCESS)
	...
	kr = machid_task_resume(machid_server_port, machid_auth_port, task);
	if (kr != KERN_SUCCESS)
    ...
    mach_port_type_t type;
{
	...
	kr = machid_port_get_refs(machid_server_port, machid_auth_port,
				  task, name,
	...
	kr = machid_port_get_refs(machid_server_port, machid_auth_port,
				  task, name, MACH_PORT_RIGHT_SEND, &refs);
	if (kr != KERN_SUCCESS)
	    quit(1, "pinfo: machid_port_get_refs(%x): %s\n",
		 name, mach_error_string(kr));

	kr = machid_port_get_receive_status(machid_server_port,
					    machid_auth_port,
					    task, name, &status);
	if (kr != KERN_SUCCESS)
	...
	if (status.mps_srights) {
	    kr = machid_port_get_srights(machid_server_port, machid_auth_port,
					 task, name, &srights);
	    if (kr == KERN_SUCCESS)
	...
	kr = machid_port_dnrequest_info(machid_server_port, machid_auth_port,
					task, name, &dntotal, &dnused);
	...
	break;
      ...
      case MACH_PORT_TYPE_PORT_SET: {
	mach_port_t members_buf[1024];
	mach_port_t *members = members_buf;
	natural_t membersCnt = sizeof members_buf/sizeof members_buf[0];
	int i;
	kern_return_t kr;

	kr = machid_port_get_set_status(machid_server_port, machid_auth_port,
	...
	if ((members != members_buf) && (membersCnt != 0)) {
    ...
    mach_port_t name;
{
    ...
    kr = machid_port_type(machid_server_port, machid_auth_port,
			  task, name, &type);
    if (kr != KERN_SUCCESS)
