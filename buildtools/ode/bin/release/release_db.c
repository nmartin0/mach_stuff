 ...
 * HISTORY
 * $Log:	release_db.c,v $
 * Revision 2.2  92/05/20  20:15:39  mrt
 * 	First checkin
 * 	[92/05/20  18:07:43  mrt]
 * 
 */
#if !defined(lint) && !defined(_NOIDENT)
static char rcsid[] = "@(#)$RCSfile: release_db.c,v $ $Revision: 2.2 $ (OSF) $Date: 92/05/20 20:15:39 $";
#endif
#include <sys/param.h>
#include <sys/time.h>
#include <sys/file.h>
...
register struct r_file **file_ptr;
{
...
register struct string *file_name;
struct r_file **file_ptr_ptr;
{
    struct r_file *file_ptr;

    file_ptr = (struct r_file *) calloc(1, sizeof(struct r_file));
    if (file_ptr == NULL) {
	warn("calloc failed");
	return(-1);
    }
	...
	if (_argbreak == '/' || isdir) {
		...
		*file_info_ptr_ptr = file_info_ptr;
		...
		i = binsearch((char *)dir_ptr->ftable.ftab,
			      dir_ptr->ftable.cur,
			      sizeof(struct r_file *),
			      part_string, check_file_name);
		if (i++ != -1 &&
		    check_file_name(part_string, (struct r_file **)
...
char *db_path;
{
	...
	for (;;) {
		...
		if (srcdir == NULL) {
		    ...
		    if (insert_file(srcdir, (struct r_file **)NULL,
...
FILE *fp;
{
	...
	for (d->ftable.max = 4;
	     ...
	     d->ftable.max <<= 1);
	d->ftable.ftab =
	    (struct r_file **)calloc(d->ftable.max, sizeof(struct r_file *));
...
char *db_path;
{
    ...
    for (file_table.max = 64;
	 ...
	 file_table.max <<= 1);
    file_table.ftab =
	(struct r_file **)calloc(file_table.max, sizeof(struct r_file *));
    ...
    buffer = malloc(file_table.cur * sizeof(struct r_file));
    if (buffer == NULL) {
	warn("malloc failed");
	return(-1);
    }
    for (i = 0; i < file_table.cur; i++) {
	file_table.ftab[i] = (struct r_file *)buffer;
	buffer += sizeof(struct r_file);
    }
...
u_short mode;
{
	...
	*ptr = '\0';
    ...
    if (insert_file(buf, (struct r_file **)NULL, &dir_ptr,
