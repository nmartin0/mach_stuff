     ...
     register expressionS *	expressionP;
{
  ...
  expressionP -> X_bignum = 0;	/* unless we find out otherwise */
 ...
 */
	  ...
	  if (small)
	    ...
	    {				/* Really a bignum, no floats */
	      expressionP -> X_bignum = bignum_new(generic_bignum, number);
	      expressionP -> X_add_number = number;
  ...
  } else if ( is_name_beginner(c) ) /* here if did not begin with a digit */
    {
      ...
      if (symbolP)
	    ...
	    {
	      expressionP -> X_add_number = low32(symbolP -> sy_value);
	    }
	  else
	    {
	      expressionP -> X_add_number  = 0;
 ...
 */
	...
	if ( c=='-' )
	  {
	    bignum_negate( expressionP -> X_bignum, expressionP -> X_add_number );
/*
 * Notice: '-' may  overflow: no warning is given. This is compatible
 * with other people's assemblers. Sigh.
 */
	  ...
	  {
	    bignum_one_complement( expressionP -> X_bignum, expressionP -> X_add_number );
	  }
	  break;
#endif	/* EXPR_TAKES_BIGNUMS */
     ...
     register expressionS *	resultP; /* Deliver result here. */
{
  ...
  while (op_left != O_illegal && op_rank [(int) op_left] > rank)
      ...
      if (SEG_NONE == expr (op_rank[(int) op_left], &right))
	{
      ...
      if (input_line_pointer[1] == c_right && (c_right == '<' || c_right == '>') )
	{
	  input_line_pointer ++;
	}			/* -> after operator. */
      know(   (int) op_right == 0
