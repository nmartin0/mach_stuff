...
private int alpha_ip();

relax_typeS md_relax_table[] = { 0 };

/* handle of the OPCODE hash table */
private struct hash_control *op_hash = NULL;

private void s_common(), s_skip(), s_frame(), s_mask(), s_base(), s_end(),
		s_ent();
...
private unsigned char octal[256];
#define isoctal(c)  octal[c]
private unsigned char toHex[256];

struct alpha_it {
    char    *error;
    unsigned long opcode;  /* either 32 bit or 64 bit longs are ok */
    struct nlist *nlistp;
    expressionS exp;
    int pcrel;
...
private void print_insn(struct alpha_it *insn);
private int getExpression(char *str, struct alpha_it *insn);
private char *expr_end;
...
/*
 * Here we deal with generating our special symbol with
 * frame informations for debuggers.
 */
#define	sy_pcreg		sy_nlist.n_other
#define	sy_framereg		sy_nlist.n_desc
#define	sy_framesize		sy_nlist.n_pad
#define	sy_regmasks		sy_nlist.n_value

#ifdef CROSS_COMPILE_32_TO_64_BITS
#define encode_regmasks(v, i,f)	{ (v).low = (f); (v).high = (i); }
#else
#define encode_regmasks(v,i,f)	{ (v) = (((long)(i)<<32)|(f)); }
#endif

private symbolS *current_function_framesym;
private char *current_function_name;

private symbolS *this_function()
{
	if (current_function_name == 0) {
		as_warn("Function context not defined (via .ent), using %s\n",
			symbol_lastP->sy_name);
		current_function_name = symbol_lastP->sy_name;
	}
	if (current_function_framesym == 0)
		current_function_framesym =
		  symbol_new (current_function_name, N_FRAME, 0, 0, 0, &zero_address_frag);
	return current_function_framesym;
}
...
s_ent()
{
	...
	SKIP_WHITESPACE();
	current_function_name = input_line_pointer;
	/* Do not make the symbol yet */
	current_function_framesym = 0;
...
s_end()
{
	...
	current_function_name = 0L;
	current_function_framesym = 0L;
	s_skip();
}
...
s_frame()
{
	...
	register symbolS *	symbolP;
	register int fs, framereg, pcreg;

	fs = 0;
	pcreg = 26;

	symbolP = this_function();
	framereg = SP;
	if (* input_line_pointer == '$') {
		input_line_pointer ++;
	...
	if (pcreg < 0 || pcreg > 30) {
		/* $31 is a special mips convention to
		   signify the PC is stored on the stack
		   in a standard place */
		if (pcreg != 31)
	...
	symbolP -> sy_framereg = framereg;
	symbolP -> sy_pcreg = pcreg;

	/* a .mask might (have) know better.  This is when FP (idiotic) */
	if (fs != 0)
		symbolP -> sy_framesize = fs;

	/* OSF BL9 adds here the offset of saved $a0 from $sp */
	s_skip();
}
...
s_mask()
{
	/* ".mask imask, pcregoffset" */

	register int	imask, saveoffset;
	register symbolS *	symbolP;

	imask = get_absolute_expression();
	if (*input_line_pointer == ',')
		input_line_pointer++;
	while (*input_line_pointer == ' ')
		input_line_pointer++;

	saveoffset = get_absolute_expression();

	/* I would love to flame here but it is useless */
	symbolP = this_function();
	if (symbolP -> sy_framesize == 0)
		symbolP -> sy_framesize = -saveoffset;

	encode_regmasks( symbolP -> sy_regmasks, imask, 0);
...
#define	MAX_INSNS	5
	...
	if (insns[i].reloc != NO_RELOC)
		...
		4,                                  /* size */
		insns[i].exp.X_add_symbol,
		insns[i].exp.X_subtract_symbol,
		insns[i].exp.X_add_number,
		insns[0].pcrel,
		insns[i].reloc
	    );
    }
...
doregister:
	     ...
	     * prepend to the instr an "ldah $r,%ml(expr)($base)" and
	     * turn this one (done later after we return) into something
	     * like "stq $r,%lo(expr)(at)" or "ldq $r,%lo(expr)($r)".
	     *
	     * NOTE: This can fail later on at link time if the offset
	     * from $base actually turns out to be more than 2**31 or
	     * 2**47 if use_large_offsets is set.
	     */
...
get_macro:
		/* all it is missing is the expression,
		   which is what we will get now */
		...
		if (insns[0].exp.X_seg == SEG_ABSOLUTE) {
		    ...
		    sval = val;
if (0) fprintf(stderr,"val %lx sval %lx\n", val ,sval);
			...
			/* Generate:
				ldah	at,x2(zero)	insn[0]
				ldah	REG,x1(GP)	insn[1]
				sll	at,16,at	insn[2]
				addq	REG,at,REG	insn[3]
				OP	?,x0(REG)	insn[4]	*/
			...
			insns[3].opcode = 0x40000400/*addq*/ | mask << SA |
					AT << SB | mask;

			insns[2].reloc = NO_RELOC;
			insns[2].opcode = 0x48001720/*sll ?,#,?*/ | 16 << SN |
					AT << SA | AT;

			insns[1].reloc = RELOC_16_31;
			insns[1].opcode = 0x24000000/*ldah*/ | mask << SA | (base_register << SB);

			insns[0].reloc = RELOC_32_47;
			opcode = 0x24000000/*ldah*/ | AT << SA | ZERO << SB;
		...
		case 'b':	/* setgp */
		    ...
		    opcode |= 0x48001600/* zap ?,#,?*/ | (0xf << SN) | (base_register);
		    continue;
    ...
    int seg_type;
{
    ...
    /*
     * NOTE: We assume to be called *after* the size of
	     the text and data segment have been computed. */
...
/*    case RELOC_BASE14: */
    ...
    case RELOC_0_63:
if (logorrea) fprintf(stderr, "%lx[%d] = %lx. %d\n", buf, n, val, seg_type);
	buf[0] = val;
	buf[1] = val >> 8;
	buf[2] = val >> 16;
	buf[3] = val >> 24;
	...
	if (val & 0x8000) val += 0x10000;	/* compensate for sign */
     ...
     struct relocation_info *ri_p, ri;
{
  ...
  md_number_to_chars(the_bytes + (long)&bf->r_address,
  			ri.r_address, sizeof(ri.r_address));
  md_number_to_chars(the_bytes + (long)&bf->r_index,
  			ri.r_index, sizeof(ri.r_index));
  md_number_to_chars(the_bytes + (long)&bf->r_extern,
  			ri.r_extern, sizeof(ri.r_extern));
/*  md_number_to_chars(the_bytes + (long)&bf->r_type,
  			ri.r_type, sizeof(ri.r_type));
   sigh. */
  the_bytes[(long)&bf->r_extern + 1] = ri.r_type;
  
  md_number_to_chars(the_bytes + (long)&bf->r_addend,
  			ri.r_addend, sizeof(ri.r_addend));
...
if (logorrea) fprintf(stderr, " r_addend %x", low32(ri.r_addend));
	    }
	    ri.r_type = fixP->fx_r_type;
	    if (fixP->fx_pcrel) {
/*		ri.r_addend -= fixP->fx_where;          */
		ri.r_addend = minus(ri.r_addend,
				    plus_a_32(ri.r_address, 4));
	    } else {
...
if (logorrea) fprintf(stderr, " %cr_addend %x @%x\n", fixP->fx_pcrel ? '-' : ' ',
	...
	char *name;
{
	...
	if (strcmp(name, "noat") == 0) return 1;
	if (strcmp(name, "reorder") == 0) return 1;
	if (strcmp(name, "noreorder") == 0) return 1;
	return 0;
}
...
struct alpha_opcode * find_opcode(int instruction)
{
	...
	if((alpha_opcodes[i].match & OPCODE_MASK) == (instruction & OPCODE_MASK))
	{
	    ...
	    {
		/* Jump (Memory format branch) format; check the jump function mask */
		if( (alpha_opcodes[i].match & JUMP_FUNCTION_MASK) == (instruction & JUMP_FUNCTION_MASK))
		{
	    ...
	    {
		/* EV4 hardware memory access format format; check the mode bits */
		if( (alpha_opcodes[i].match & HW_MODE_BITS_MASK) == (instruction & HW_MODE_BITS_MASK))
		{
		    /* This is what we want */
		    opcodedata = &alpha_opcodes[i];
		    break;
	    ...
	    {
		/* EV4 internal register access format format; check the register set mask */
		...
		if (opcodedata == 0) opcodedata = &alpha_opcodes[i];

		/* Don't exit the loop; there may be a more precise match yet */
	    }
	    else if( strcmp(alpha_opcodes[i].args, "0(2)") == 0)
	    {
		/* Miscellaneous instructions - Memory format with function code*/
	    ...
	    {
		/* Miscellaneous instructions with no arguments; should be exact match */
	    ...
	    /* All other cases are only used for predefined macros and pseudo-instructions;
	       ignore them */
	}
    }
...
void md_print_instruction(int instruction, FILE* stream)
{
	...
	case 'r':
	    fprintf(stream,"$%i",(instruction & RA_MASK) >> RA_SHIFT);
	    if(((instruction & RC_MASK) >> RC_SHIFT) != ((instruction & RA_MASK) >> RA_SHIFT))
	    {
	...
	case 'R':
	    fprintf(stream,"$%i",(instruction & RA_MASK) >> RA_SHIFT);
	    if(((instruction & RB_MASK) >> RB_SHIFT) != ((instruction & RA_MASK) >> RA_SHIFT))
	    {
		duplicate_reg_bad = TRUE;
		bad_reg_name = 'B';
	    ...
	    break;
	case 'I':
	    fprintf(stream,"0x%07x",(instruction & PALCODE_FUNCTION_MASK) >> PALCODE_FUNCTION_SHIFT);
	    break;	    
	case 'l':
	    fprintf(stream,"0x%04x",(instruction & MEM_DISP_MASK) >> MEM_DISP_SHIFT);
	    break;	    
	case 'h':
	    fprintf(stream,"0x%04x",((instruction & MEM_DISP_MASK) >> MEM_DISP_SHIFT) << 16);
	    break;	    
	case 't':
	    /* Special for EV4 hardware load/store instructions */
	    fprintf(stream,"0x%04x",((instruction & HW_MEM_DISP_MASK) >> HW_MEM_DISP_SHIFT));
	    break;	    
	case 'L':
	    fprintf(stream,".+0x%08x",((instruction & BRANCH_DISP_MASK) >> BRANCH_DISP_SHIFT) * 4);
	    break;	    
	case 'i':
	    fprintf(stream,".+0x%06x",((instruction & JUMP_HINT_MASK) >> JUMP_HINT_SHIFT) * 4);
	    break;	    
	case 'b':
	    fprintf(stream,"0x%02x",(instruction & LITERAL_MASK) >> LITERAL_SHIFT);
	    break;
	case '8':
	    fprintf(stream,"0x%02x",(instruction & MXPR_DISPLACEMENT_MASK));
	    break;
	/* All other possible characters are punctuation; print what we are given */
	default:
    ...
    {
	fprintf(stream, " WARNING: R%c register field does not match RA - value %i",
