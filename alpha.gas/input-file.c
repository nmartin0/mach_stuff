...
#ifdef	alpha	/* I find it extremely useful, thank you */
	if (docpp)
	{
...
#ifdef CMUCS
		s = runp("cpp", "cpp", "-D__GNU_AS__=1", filename, temporary_file_name, 0) < 0;
#else
		{ char *cmd, *malloc();
		  cmd = malloc(sizeof("/usr/lib/cpp -D__GNU_AS__=1 ") + strlen(filename) + strlen(temporary_file_name) + 2);
		  sprintf(cmd, "/usr/lib/cpp -D__GNU_AS__=1 %s %s", filename, temporary_file_name);
		  s = system(cmd);
		  free(cmd);
		}
#endif
		if (s) {
			as_perror("Could not run cpp");
			return;
		}
