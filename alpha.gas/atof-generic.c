...

  ...
  if (number_of_digits_available == 0)
	...
	+ 2;			/* 2 :: guard digits. */
#ifdef TRACE
printf("p %d na %d ma %d\n", precision, number_of_digits_available, maximum_useful_digits);
#endif
...
(void)putchar('\n');
printf("multiplicand:");flonum_print(multiplicand + place_number);
(void)putchar('\n');
#endif
...
#ifdef	TRACE
void flonum_print(FLONUM_TYPE *f)
{
	LITTLENUM_TYPE *l;

	for (l = f->low; l < f->high; l++) {
		printf("%d%c", *l, (l == f->leader) ? '.' : ' ');
	}

	printf("e%c%d ", f->sign, f->exponent);
}
#endif
