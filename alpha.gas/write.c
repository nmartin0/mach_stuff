 ...
 * HISTORY
 * $Log:	write.c,v $
 * Revision 1.2  90/08/31  09:39:51  bww
 * 	The r_bsr and r_disp fields are only defined in
 * 	the gas version of a.out.h, not the "system" one.
 * 	[90/08/31  09:37:42  bww]
 * 
 */
...
write_object_file()
{
  ...
  void gdb_end();
#endif
  extern integer_t omagic;	/* JF magic # to write out.  Is different for
...

   ...
   * We adjust the values of '$' local symbols, even if we do
   * not intend to emit them to the object file, because their values
   * are needed for fix-ups.
   *
   * Unless we saw a -L flag, remove all symbols that begin with '$'
  ...
  for(symbolP=symbol_rootP;symbolP;symbolP=symbolP->sy_next) {
  	if(symbolP->sy_forward) {
		symbolP->sy_value = plus(symbolP->sy_value,
					 plus_a_32(symbolP->sy_forward->sy_value,symbolP->sy_forward->sy_frag->fr_address));
  ...
  symbolPP = & symbol_rootP;	/* -> last symbol chain link. */
  {
	...
	symbolP -> sy_value = plus_a_32(symbolP -> sy_value, symbolP -> sy_frag -> fr_address);
	...
	if ( !name || (symbolP->sy_nlist.n_type&N_STAB)
	  ...
	  {
	    /* If we do not keep local symbols and this one is local
	       and still undefined  --> error */
	    if ((symbolP->sy_nlist.n_type == N_UNDF) &&
		(name && (name[0] == '$') && !flagseen[ 'L' ]))
	      as_fatal("Local Symbol \"%s\" was never defined\n", name);
	    * symbolPP = symbolP -> sy_next;
	  }
  ...
  for (fragP = text_frag_root;  fragP;  fragP = fragP -> fr_next)
	...
	if(!lie->added) {
	  ...
	  fix_new_ns32k(lie->frag,
			...
			0, 0, 2, 0, 0);
#endif
#if ! (defined(SPARC) || defined(alpha) || defined(NS32K))
	  fix_new(	lie->frag,  lie->word_goes_here - lie->frag->fr_literal,
	  		2,  lie->add,
			lie->sub,  lie->addnum,
			0);
	...
	for(m=0;lie && lie->dispfrag==fragP;m++,lie=lie->next_broken_word) {
	  ...
	  from_addr=table_addr;
	  to_addr=low32(lie->add->sy_value)+lie->addnum;
  ...
  md_number_to_chars((char *)&the_exec.a_magic,low32(omagic),sizeof(the_exec.a_magic));
...
#if	alpha
  /* the_exec . a_tstart == the_exec . a_dstart = 0 */
#endif
  ...
  for (fragP = text_frag_root;  fragP;  fragP = fragP -> fr_next)
    {
      ...
      /*
       * Sad note.  It is only here (well, in relax_segment() actually
       * but it makes no difference) that we find out if something was
       * wrong with how the user handled DOT.  All we can tell is the
       * whereabouts of the PC at this point.
       */
      if (((int)fragP->fr_offset) < 0)
	as_fatal("Something is screwed up in the way you played with the DOT, at or before address %lx", fragP->fr_address);
  ...
  for (   symbolP = symbol_rootP;   symbolP;   symbolP = symbolP -> sy_next   )
    {
      ...
      symbolP -> sy_nlist . n_un . n_pad_cross_compile = zero;
      ...
      md_number_to_chars((char *)&(symbolP->sy_nlist.n_pad),symbolP->sy_nlist.n_pad,sizeof(symbolP -> sy_nlist  . n_pad));
      md_number_to_chars((char *)&(symbolP->sy_nlist.n_other),symbolP->sy_nlist.n_other,sizeof(symbolP -> sy_nlist  . n_other));

      md_number_to_chars((char *)&low32(symbolP->sy_nlist.n_value),low32(symbolP->sy_nlist.n_value),sizeof(low32(symbolP->sy_nlist.n_value)));
#ifdef CROSS_COMPILE_32_TO_64_BITS
      md_number_to_chars((char *)&high32(symbolP->sy_nlist.n_value),high32(symbolP->sy_nlist.n_value),sizeof(high32(symbolP->sy_nlist.n_value)));
#endif
	...
	for (fragP = segment_frag_root;  fragP;  fragP = fragP -> fr_next)
	  {
		...
		growth=0;
			...
			if(lie->added)
				continue;
			offset=  lie->add->sy_frag->fr_address+low32(lie->add->sy_value) + lie->addnum -
				(lie->sub->sy_frag->fr_address+low32(lie->sub->sy_value));
			if(offset<=-32768 || offset>=32767) {
				...
				for(untruth=lie->next_broken_word;untruth && untruth->dispfrag==lie->dispfrag;untruth=untruth->next_broken_word)
					if(untruth->add->sy_frag==lie->add->sy_frag && eq64(untruth->add->sy_value,lie->add->sy_value)) {
						untruth->added=2;
	  ...
	  if(!add_symbolP)	/* Its just -sym */
	    {
	      ...
	      add_number-=low32(sub_symbolP->sy_value);
	    }
		  ...
		  || add_symbol_N_TYPE == N_ABS))
	    {
	      ...
	      add_number += low32(minus(add_symbolP -> sy_value, sub_symbolP -> sy_value));
	      add_symbolP = NULL;
	      fixP -> fx_addsy = NULL;
	    }
	  else
	  ...
	  if (add_symbol_N_TYPE == this_segment_type && pcrel)
	    {
	       ...
	       */
	      add_number += low32(add_symbolP -> sy_value);
...
/* A machine might need its own emit_relocations() */
#if ! (defined(SPARC) || defined(alpha))
/*
 *		emit_relocations()
 *
