...
mach_spin_lock_t	kget_space_lock;
...
kalloc_init(void)
{
	...
	for (i = 0; i < KLIST_MAX; i++) {
	    mach_spin_lock_init(&kfree_list[i].lock);
	    kfree_list[i].head = 0;
	}
	mach_spin_lock_init(&kget_space_lock);
...
kget_space(vm_offset_t size)
{
	...
	mach_spin_lock(&kget_space_lock);
	while (kalloc_next_space + size > kalloc_end_of_space) {
		 ...
		 * allocation area.
		 */
		mach_spin_unlock(&kget_space_lock);
...
kalloc(size_t size)
{
	...
	if (allocsize < kalloc_max) {
	    mach_spin_lock(&fl->lock);
	    if ((addr = fl->head) != 0) {
		fl->head = *(vm_offset_t *)addr;
		mach_spin_unlock(&fl->lock);
	    }
	    else {
		mach_spin_unlock(&fl->lock);
