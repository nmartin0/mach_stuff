 ...
 * HISTORY
 * $Log:	ipc_kmsg.h,v $
 * Revision 2.15  93/11/17  16:57:43  dbg
 * 	Added kmsg type for kernel reply messages.  Moved some more
 * 	common type definitions to ipc/ipc_types.h, which replaces
 * 	ipc/ipc_kmsg_queue.h.  Added ANSI function prototypes.
 * 	[93/03/29            dbg]
 * 
...
/*
 *	struct ipc_kmsg_queue is defined in ipc/ipc_types.h,
 *	instead of this file, so that kern/thread.h doesn`t have
 *	to include ipc/ipc_kmsg.h.
 */
...
/*
 *	Structure declarations needed for exported routines
 */
struct ipc_object;
struct vm_map;
...
extern mach_msg_return_t
ipc_kmsg_copyin_compat(ipc_kmsg_t, ipc_space_t, struct vm_map *);

extern mach_msg_return_t
ipc_kmsg_copyout_compat(ipc_kmsg_t, ipc_space_t, struct vm_map *);
