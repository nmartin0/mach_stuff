 ...
 * HISTORY
 * $Log:	ipc_notify.c,v $
 * Revision 2.10  93/11/17  16:59:17  dbg
 * 	Added ANSI function prototypes.
 * 	[93/09/23            dbg]
 * 
 * Revision 2.9  93/01/14  17:33:03  danner
 * 	64bit cleanup.
 * 	[92/11/30            af]
 * 
 * Revision 2.8  92/08/03  17:34:53  jfriedl
 * 	removed silly prototypes
 * 	[92/08/02            jfriedl]
 * 
 * Revision 2.7  92/05/21  17:10:54  jfriedl
 * 	tried prototypes.
 * 	[92/05/20            jfriedl]
 * 
 * Revision 2.6  92/03/10  16:26:04  jsb
	...
	mach_no_senders_notification_t *n)
{
	...
	t->msgt_name = MACH_MSG_TYPE_INTEGER_T;
	t->msgt_size = PORT_T_SIZE_IN_BITS;
	t->msgt_number = 1;
	t->msgt_inline = TRUE;
	t->msgt_longform = FALSE;
	...
	mach_port_t name)
{
	...
	if (kmsg == IKM_NULL) {
		printf("dropped port-deleted (%#08x, %#x)\n",
		       (vm_offset_t) port, name);
	...
	mach_port_t name)
{
	...
	kmsg = ikm_alloc(sizeof *n);
	if (kmsg == IKM_NULL) {
		printf("dropped msg-accepted (%#08x, %#x)\n",
		       (vm_offset_t) port, name);
		ipc_port_release_sonce(port);
		return;
	}
	...
	ipc_port_t right)
{
	...
	kmsg = ikm_alloc(sizeof *n);
	if (kmsg == IKM_NULL) {
		printf("dropped port-destroyed (%#08x, %#08x)\n",
		       (vm_offset_t)port, (vm_offset_t) right);
		ipc_port_release_sonce(port);
	...
	mach_port_mscount_t mscount)
{
	...
	kmsg = ikm_alloc(sizeof *n);
	if (kmsg == IKM_NULL) {
		printf("dropped no-senders (%#08x, %u)\n",
		       (vm_offset_t) port, mscount);
		ipc_port_release_sonce(port);
		return;
	}
	...
	mach_port_t name)
{
	...
	kmsg = ikm_alloc(sizeof *n);
	if (kmsg == IKM_NULL) {
		printf("dropped dead-name (%#08x, %#x)\n",
		       (vm_offset_t) port, name);
		ipc_port_release_sonce(port);
		return;
	}
	...
	mach_port_t name)
{
	...
	kmsg = ikm_alloc(sizeof *n);
	if (kmsg == IKM_NULL) {
		printf("dropped port-deleted-compat (%#08x, %#x)\n",
		       (vm_offset_t) port, name);
	...
	mach_port_t name)
{
	...
	kmsg = ikm_alloc(sizeof *n);
	if (kmsg == IKM_NULL) {
		printf("dropped msg-accepted-compat (%#08x, %#x)\n",
		       (vm_offset_t) port, name);
		ipc_port_release_send(port);
		return;
	}
	...
	ipc_port_t right)
{
	...
	kmsg = ikm_alloc(sizeof *n);
	if (kmsg == IKM_NULL) {
		printf("dropped port-destroyed-compat (%#08x, %#08x)\n",
		       (vm_offset_t) port, (vm_offset_t) right);
		ipc_port_release_send(port);
		ipc_port_release_receive(right);
		return;
