 ...
 * HISTORY
 * $Log:	ipc_object.c,v $
 * Revision 2.10  93/11/17  16:59:42  dbg
...
mach_msg_type_name_t
ipc_object_copyout_type_compat(
	mach_msg_type_name_t msgt_name)
{
	switch (msgt_name) {
	    case MACH_MSG_TYPE_PORT_SEND:
...
kern_return_t
ipc_object_copyout_name_compat(
	ipc_space_t space,
	ipc_object_t object,
	mach_msg_type_name_t msgt_name,
	mach_port_t name)
...
ipc_object_print(
	ipc_object_t object)
{
	db_iprintf("%s", io_active(object) ? "active" : "dead");
	db_printf(", refs=%d", object->io_references);
	db_printf(", otype=%d", io_otype(object));
	db_printf(", kotype=%d\n", io_kotype(object));
}
