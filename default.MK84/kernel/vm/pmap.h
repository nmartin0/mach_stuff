 ...
 * HISTORY
 * $Log:	pmap.h,v $
 * Revision 2.7  93/11/17  19:19:32  dbg
 * 	Added ANSI function prototypes.  Include mach/vm_prot.h to
 * 	get type declarations.
 * 	[93/09/21  22:09:41  dbg]
 * 
...
extern vm_offset_t	pmap_steal_memory(
		vm_size_t	size);		/* During VM initialization,
...
extern void		pmap_startup(
		vm_offset_t	*startp,
		vm_offset_t	*endp);		/* During VM initialization,
...
extern boolean_t	pmap_next_page(
		vm_offset_t	*addrp);	/* During VM initialization,
 ...
 */
extern pmap_t		pmap_create(vm_size_t);	/* Create a pmap_t. */
#ifndef pmap_kernel
extern pmap_t		pmap_kernel(void);	/* Return the kernel's pmap_t. */
#endif
extern void		pmap_reference(pmap_t);	/* Gain a reference. */
extern void		pmap_destroy(pmap_t);	/* Release a reference. */
...
/*
 *	Routines to copy and clear physical pages
 */
extern void		pmap_copy_page(		/* copy a physical page */
		vm_offset_t	src_phys_addr,
		vm_offset_t	dest_phys_addr);

extern void		pmap_zero_page(		/* zero a physical page */
		vm_offset_t	phys_addr);
...
extern int		pmap_resident_count(pmap_t);
#endif
...
extern vm_offset_t	pmap_extract(
		pmap_t		pmap,
		vm_offset_t	va);		/* Return a virtual-to-physical
						 * mapping, if possible.
						 */

extern void		pmap_collect(pmap_t);	/* Perform garbage
						 * collection, if any
						 */
