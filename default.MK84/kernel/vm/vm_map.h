 ...
 * HISTORY
 * $Log:	vm_map.h,v $
 * Revision 2.19  93/11/17  18:55:43  dbg
 ...
 *		or perform other operations.  The continuation takes 2
 *		arguments: a saved argument buffer and a pointer to a
 *		new vm_map_copy (returned).
 */
...
typedef struct vm_map_copy *vm_map_copy_t;	/* forward declaration */

/*
 *	Function to be called as continuation.
 *	The first argument is a generic pointer:
 *	it currently may be a vm_map_copy_t or
 *	a vm_map_copyin_args_t (below).
 */
typedef	kern_return_t	(*vm_map_copyin_cont_t)(void *, vm_map_copy_t *);
...
MACRO_BEGIN								\
	...
	*result = (*((old_copy)->cpy_cont))((old_copy)->cpy_cont_args,	\
					    new_copy);			\
	(old_copy)->cpy_cont = (vm_map_copyin_cont_t) 0;		\
MACRO_END
...
#define vm_map_copy_has_cont(copy)					\
    (((copy)->cpy_cont) != (vm_map_copyin_cont_t) 0)
...
extern kern_return_t	vm_map_find_entry(	/* Find space for a mapping */
		vm_map_t	map,		/* and add the entry */
		vm_offset_t	*address,
		vm_size_t	size,
		vm_offset_t	mask,
		vm_object_t	object,
...
extern void		vm_map_print(vm_map_t);	/* Debugging: print a map */
...
extern kern_return_t	vm_map_copyin_object(	/* Turn an object region
						   into a copy */
		vm_object_t	object,
		vm_offset_t	offset,
		vm_size_t	size,
...
extern void		vm_map_copy_discard(vm_map_copy_t);
						/* Discard a copy without
						 * using it */

extern void		vm_map_copy_page_discard(vm_map_copy_t);
						/* Discard the pages in
						   a page-list copy */

extern kern_return_t	vm_map_copy_discard_cont(
		void *		args,
		vm_map_copy_t 	*result);	/* Page list continuation
						 * version of previous */

vm_map_copy_t		vm_map_copy_copy(vm_map_copy_t);
						/* Move the information in
						   a vm_map_copy_t to a new
						   one, leaving the old one
						   empty (for deallocation) */
...
kern_return_t		vm_map_submap(
		...
		vm_offset_t	start,
		vm_offset_t	end,
		vm_map_t	submap);	/* mark range as handled by
						   a submap */

/*
 *	Lookup a map entry.  Map must be locked.
 *
 *	Exported only for use by vm_kern.c.
 */
