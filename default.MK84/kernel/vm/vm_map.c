 ...
 * HISTORY
 * $Log:	vm_map.c,v $
 * Revision 2.46  93/12/23  10:07:18  dbg
 ...
 * 	[93/12/14            dbg]
 * 
 * Revision 2.45  93/11/17  18:54:55  dbg
 ...
 * 
 * 	Use object->shadowed and object->use_shared_copy to
...
vm_map_entry_t _vm_map_entry_create(
...
kern_return_t	vm_map_copyin_page_list_cont(
	...
	vm_map_copy_t	*copy_result)	/* OUT */
{
	vm_map_copyin_args_t	cont_args = (vm_map_copyin_args_t) args;
	kern_return_t	result = KERN_SUCCESS;
	...
	boolean_t	is_cont)
{
	...
	copy->cpy_cont_args = (void *) VM_MAP_COPYIN_ARGS_NULL;
...
make_continuation:
			    ...
			    copy->cpy_cont_args = (void *) cont_args;
	...
	register vm_map_t	map)
{
	...
	db_iprintf("Task map 0x%X: pmap=0x%X,",
 		(vm_offset_t) map, (vm_offset_t) (map->pmap));
	db_printf("ref=%d,nentries=%d,", map->ref_count, map->hdr.nentries);
	db_printf("version=%d\n", map->timestamp);
	db_indent += 2;
	for (entry = vm_map_first_entry(map);
		...
		db_iprintf("map entry 0x%X: ", (vm_offset_t) entry);
		db_printf("start=0x%X, end=0x%X, ",
		...
		if (entry->in_transition) {
			db_printf("in transition");
			if (entry->needs_wakeup)
				db_printf("(wake request)");
			db_printf(", ");
		}
		if (entry->is_sub_map) {
...
void vm_map_copy_print(
	vm_map_copy_t copy)
{
	int i, npages;

	db_printf("copy object 0x%x\n", copy);

	db_indent += 2;

	db_iprintf("type=%d", copy->type);
	switch (copy->type) {
		case VM_MAP_COPY_ENTRY_LIST:
		db_printf("[entry_list]");
		break;
	...
	db_printf(", offset=0x%x", copy->offset);
	db_printf(", size=0x%x\n", copy->size);
	...
	switch (copy->type) {
		...
		case VM_MAP_COPY_OBJECT:
		db_iprintf("object=0x%x\n", copy->cpy_object);
		break;

		case VM_MAP_COPY_PAGE_LIST:
		db_iprintf("npages=%d", copy->cpy_npages);
		db_printf(", cont=%x", copy->cpy_cont);
		db_printf(", cont_args=%x\n", copy->cpy_cont_args);
		if (copy->cpy_npages < 0) {
			npages = 0;
		...
		db_iprintf("copy->cpy_page_list[0..%d] = {", npages);
		for (i = 0; i < npages - 1; i++) {
			db_printf("0x%x, ", copy->cpy_page_list[i]);
		}
		if (npages > 0) {
			db_printf("0x%x", copy->cpy_page_list[npages - 1]);
		}
		db_printf("}\n");
		break;
	}
	...
	vm_map_copy_t	*caller_copy)
{
	...
	new_copy->cpy_cont_args = (void *) copy;
