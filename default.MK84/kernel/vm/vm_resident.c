 ...
 * HISTORY
 * $Log:	vm_resident.c,v $
 * Revision 2.27  93/11/17  18:57:38  dbg
...
void		vm_page_print(p)
	vm_page_t	p;
{
	db_iprintf("Page 0x%X: object 0x%X,",
		   (vm_offset_t) p, (vm_offset_t) p->object);
	 db_printf(" offset 0x%X", (vm_offset_t) p->offset);
	 db_printf("wire_count %d,", p->wire_count);
	 db_printf(" %s",
	 ...
	 db_printf("%s,",
		(p->tabled ? "" : "not_tabled"));
	 db_printf("phys_addr = 0x%X, lock = 0x%X, unlock_request = 0x%X\n",
