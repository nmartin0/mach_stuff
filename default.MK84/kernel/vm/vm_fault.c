 ...
 * HISTORY
 * $Log:	vm_fault.c,v $
 * Revision 2.22  93/11/17  18:53:39  dbg
...
typedef struct vm_fault_state {
	/* state saved for vm_fault: */
	vm_map_t	vmf_map;		/* map */
	vm_offset_t	vmf_vaddr;		/* faulting address */
	vm_prot_t	vmf_fault_type;		/* access type */
	boolean_t	vmf_change_wiring;	/* wiring/unwiring? */
	no_return	(*vmf_continuation)(kern_return_t);
						/* routine to run instead
						   of returning */
	vm_map_version_t vmf_version;		/* map version */
	boolean_t	vmf_wired;		/* wire pages */
	vm_object_t	vmf_object;		/* object, */
	vm_offset_t	vmf_offset;		/* offset for page found */
	vm_prot_t	vmf_prot;		/* actual protection on
						   map entry */

	/* state saved for vm_fault_page: */
	boolean_t	vmfp_backoff;		/* backoff and retry after
						   page_wait? */
	vm_object_t	vmfp_object;		/* current object and */
	vm_offset_t	vmfp_offset;		/* offset being searched */
	vm_page_t	vmfp_first_m;		/* placeholder page in
						   top-level object */
	vm_prot_t	vmfp_access;		/* access needed */
} vm_fault_state_t;
		...
		if (look_for_page && !must_be_resident) {
			...
			if ((rc = memory_object_data_request(object->pager, 
				...
				if (rc != MACH_SEND_INTERRUPTED)
					printf("%s(%#x, %#x, %#x, %#x, %#x) failed, %d\n",
						"memory_object_data_request",
						(vm_offset_t) object->pager,
						(vm_offset_t) object->pager_request,
						m->offset + object->paging_offset, 
 ...
 *
 *		vm_fault and vm_fault_page save much state
 *		in the equivalent of a closure.  The state
...
vm_fault_continue(void)
{
	...
	vm_fault_noreturn(state->vmf_map,
			  state->vmf_vaddr,
	...
	no_return	(*continuation)(kern_return_t))
{
	...
	if (continuation != 0) {
		...
		state = (vm_fault_state_t *) zalloc(vm_fault_state_zone);
	...
	if (continuation != (void (*)(kern_return_t)) 0) {
		register vm_fault_state_t *state =
			(vm_fault_state_t *) current_thread()->ith_other;
