 ...
 * Revision 2.32  93/11/17  18:52:14  dbg
 * 	Moved code from PAGEOUT_PAGES macro into a function
 * 	called by the macro.  Added ANSI prototypes.
 * 	[93/09/21  22:09:18  dbg]
 * 
...
/*
 *	[ obsolete ]
 *	Old version of memory_object_data_supply.
 *	Does not allow precious pages or reply port.
 *
	...
	mach_msg_type_name_t	reply_to_type)
{
	...
	memory_object_lock_result_t pageout_action = 0; /* '=0' to quiet lint */
...
MACRO_BEGIN								\
	pageout_pages(object, paging_offset, new_object, new_offset,	\
		      pageout_action, should_flush, holding_pages);	\
...
boolean_t	memory_manager_default_port(
	ipc_port_t port)
{
	ipc_port_t current;
	boolean_t result;
