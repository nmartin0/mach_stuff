 ...
 * HISTORY
 * $Log:	dev_pager.c,v $
 * Revision 2.32  93/11/17  16:31:52  dbg
 ...
 * 	Removed unused variable 'result' from device_pager_setup()
 * 	[92/05/16            jfriedl]
 * 
...
#include <mach/mach_user_kernel.h>	/* our calls to kernel */
...
boolean_t	device_pager_data_request_done(io_req_t);	/* forward */
boolean_t	device_pager_data_write_done(io_req_t);		/* forward */
	...
	vm_prot_t	protection_required)
{
	...
	if (device_pager_debug)
	    printf("(device_pager)data_request: pager=%d, offset=%#x, length=%#x\n",
		   (vm_offset_t)pager, offset, length);
...
boolean_t device_pager_data_write_done(
	register io_req_t	ior)
{
	device_write_dealloc(ior);
	device_deallocate(ior->io_device);
	...
	vm_offset_t	offset)
{
	...
	return (*ds->device->dev_ops->d_mmap)
	...
	vm_size_t	pager_page_size)
{
	...
	if (device_pager_debug)
		printf("(device_pager)init: pager=%#x, request=%#x, name=%#x\n",
		       (vm_offset_t) pager, (vm_offset_t) pager_request,
		       (vm_offset_t) pager_name);
