...
#ifndef	_DEVICE_IO_REQ_H_
#define	_DEVICE_IO_REQ_H_
 ...
 */
typedef struct io_req *	io_req_t;	/* forward declaration */

struct io_req {
	io_req_t	io_next;	/* next, ... */
	io_req_t	io_prev;	/* prev pointers: link in done,
...
#define	io_data		io_un.data
	...
	io_req_t	io_link;	/* forward link (for driver header) */
	io_req_t	io_rlink;	/* reverse link (for driver header) */
 ...
 */
extern void	iodone(io_req_t);

/*
 * Routine to wait synchronously for IO completion.
 */
extern void	iowait(io_req_t);
