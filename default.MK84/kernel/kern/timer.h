 ...
 * HISTORY
 * $Log:	timer.h,v $
 * Revision 2.6  93/11/17  17:33:00  dbg
 * 	If no machine-dependent timer is used: maintain timer in seconds
 * 	and nanoseconds, and use microseconds (or the nearest binary
 * 	approximation thereof) for the scheduling units.
 * 	[93/01/25            dbg]
 * 
...
#if	STAT_TIME
/*
 *	Statistical timer definitions - use nanoseconds in timer, seconds
 *	in high unit field.  Use microseconds as scheduling unit: or,
 *	since we use a shift, 10**9/1024.
 */

/*
 *	Timers themselves are counted in seconds and nanoseconds.
 */
#define TIMER_HIGH_UNIT	NANOSEC_PER_SEC

/*
 *	USAGE_RATE is the unit for cpu_usage and sched_usage.
 *	It is nanoseconds >> 10 (10**9 / 1024).
 */
#define USAGE_RATE	(TIMER_HIGH_UNIT >> 10)
				/* (10**9)/(2**10) == 976562.5 */

#define	TIMER_LOW_TO_USAGE(nanos)	((nanos) >> 10)
#define	TIMER_HIGH_TO_USAGE(secs)	((secs) * USAGE_RATE)

	/* (((secs)*TIMER_HIGH_UNIT)>>10), but this will overflow */

#undef	TIMER_TO_TIMESPEC		/* timers == time_specs */
...
#define	TIMER_LOW_FULL	0x80000000U		/* assumes 32 bits! */
...
extern void		thread_read_times(
		thread_t	thread,
		time_spec_t	*user_time_p,
		time_spec_t	*system_time_p);

extern unsigned int	timer_delta(
		timer_t		timer,
		timer_save_t	save);
...
extern void
timer_normalize(timer_t);

#else	/* STAT_TIME */
/*
 *	Exported hardware interface to timers
 */
extern void	time_trap_uentry(unsigned int timestamp);
extern void	time_trap_uexit(unsigned int timestamp);
extern timer_t	time_int_entry(unsigned int timestamp, timer_t new_timer);
extern void	time_int_exit(unsigned int timestamp, timer_t old_timer);
#endif	/* STAT_TIME */
...
MACRO_BEGIN							\
	...
	if ((save).high != (timer).high_bits_check) {		\
	    ...
	    result += TIMER_LOW_TO_USAGE(temp - (save).low);	\
	    (save).low = temp;					\
	}							\
MACRO_END
