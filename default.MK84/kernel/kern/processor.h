 ...
 * HISTORY
 * $Log:	processor.h,v $
 * Revision 2.8  93/11/17  17:18:50  dbg
 * 	Use runq pointers in processor if NCPUS > 1 - processor_shutdown
 * 	does not depend on MACH_HOST.
 * 	[93/07/21            dbg]
 * 
 * 	Document changed locking hierarchy.
 * 	[93/06/01            dbg]
 * 
 * 	Remove max_priority.  Limits for per-policy scheduling
 * 	parameters are in the run queue structures.
 * 	[93/05/10            dbg]
 * 
 * 	Changed to more flexible run queue structure.  It also doubles
 * 	as a list of enabled scheduling policies.
 * 	[93/04/08            dbg]
 * 
 * 	Always enable fixed-priority threads.
 * 	[93/03/27            dbg]
 * 
 * 	Include kern/run_queues.h for run queue definitions.  Include
 * 	kern/sched_policy.h for scheduler policy structure.  Move
 * 	check_processor_set and check_bound_processor macros here.
 * 	[93/01/28            dbg]
 * 
...
/*
 *	Scheduler load factor (sched_load) is a scaled fixed-point number.
 */
#define SCHED_SCALE	128
#define SCHED_SHIFT	7

struct processor_set {
	struct run_queue_head	runq;		/* runq for this set */
#if	NCPUS > 1
...
struct processor {
#if	MACH_IO_BINDING
	struct run_queue_head runq;	/* local runq for this processor */
#else
#if	NCPUS > 1
	struct run_queue_head runq;	/* copy of runq pointers for this
					   processor: count and lock are
					   not used */
#endif	/* NCPUS > 1 */
#endif	/* MACH_IO_BINDING */
	queue_chain_t	processor_queue; /* idle/assign/shutdown queue link */
	int		state;		/* See below */
	thread_t	next_thread;	/* next thread to run if dispatched */
	thread_t	idle_thread;	/* this processor's idle thread. */
...
extern void	pset_remove_task(processor_set_t, task_t);
extern void	pset_add_task(processor_set_t, task_t);
extern void	pset_remove_thread(processor_set_t, thread_t);
extern void	pset_add_thread(processor_set_t, thread_t);
extern void	thread_change_psets(thread_t, processor_set_t, processor_set_t);

/* Checks that a thread can run on a processor */
...
extern kern_return_t processor_set_policy_add(
		processor_set_t	pset,
		int		policy,
		policy_param_t	limit,
		natural_t	count);

extern kern_return_t processor_set_policy_remove(
		processor_set_t	pset,
		int		policy);

extern kern_return_t processor_set_policy_limit(
		processor_set_t	pset,
		int		policy,
		policy_param_t	limit,
		natural_t	count,
