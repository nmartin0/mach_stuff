 ...
 * HISTORY
 * $Log:	printf.c,v $
 * Revision 2.16  93/11/17  17:17:30  dbg
 * 	ANSI-fied.  We now have stdarg.h!
 * 	Moved iprintf to ddb/db_output.c, since it is only used
 * 	by debugging print routines.
 * 	[93/10/27            dbg]
 * 
...
#include <kern/kern_io.h>	/* cnputc, cngetc */
#include <kern/lock.h>
...
void printf(const char *fmt, ...)
{
	...
	va_start(listp, fmt);
	_doprnt(fmt, &listp, (void (*)(char, void *))cnputc, 16, 0);
	va_end(listp);
}
	...
	...)
{
	...
	va_start(listp, fmt);
	_doprnt(fmt, &listp, sputc, 16, (void *)&buf);
	va_end(listp);
