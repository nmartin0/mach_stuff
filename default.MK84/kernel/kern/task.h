 ...
 * HISTORY
 * $Log:	task.h,v $
 * Revision 2.13  93/11/17  17:29:40  dbg
 * 	Remove itk_lock in favor of task_lock.
 * 	[93/07/12            dbg]
 * 
 ...
 * 	[93/06/10            dbg]
 * 
 * 	Add scheduling policy for newly created threads; change priority
 * 	to a policy-specific field.
 * 	[93/05/10            dbg]
 * 
 * 	Removed include of kern/processor.h.  We only need the type
 * 	declaration from kern/kern_types.h.
 * 	[93/04/09            dbg]
 * 
 * 	Converted time_values to time_specs.
 * 	[93/01/28            dbg]
 * 
...
struct task {
	...
	/* boolean_t */	assign_wait:1;	/* waiting for may_assign */

	/* User-visible scheduling information */
	int		user_stop_count;
					/* outstanding stops */
	sched_policy_t	sched_policy;	/* default scheduling policy and */
	sched_param_data_t
			sched_data;	/* scheduling data for new threads */
	natural_t	sched_data_count;
	...
	struct ipc_space *itk_space;	/* port space for task */
...
#define task_ref_lock(task)	simple_lock(&(task)->ref_lock)
#define task_ref_unlock(task)	simple_unlock(&(task)->ref_lock)
...
extern kern_return_t	task_set_default_policy(
		task_t		task,
...
extern void		consider_task_collect(void);	/* garbage collection */
