 ...
 * HISTORY
 * $Log:	exception.c,v $
 * Revision 2.18  93/12/07  13:54:02  dbg
 * 	Use 'MACH_MSG_TYPE_INTEGER_T' for type of integer_t.
 * 	[93/11/19            dbg]
 * 
 * Revision 2.17  93/11/17  17:09:45  dbg
 ...
 * 	[93/03/06            af]
 * 
 * Revision 2.15  93/01/14  17:34:09  danner
 * 	64bit cleanup.
 * 	[92/12/01            af]
 * 
...
no_return exception(
	...
	integer_t	code,
	integer_t	subcode);
no_return exception_try_task(
	integer_t	_exception,
	integer_t	code,
	integer_t	subcode);
no_return exception_no_server(void);
...
kern_return_t exception_parse_reply(
	ipc_kmsg_t	kmsg);

no_return exception_raise_continue(void);
no_return exception_raise_continue_slow(
	mach_msg_return_t	mr,
	ipc_kmsg_t		kmsg,
	mach_port_seqno_t	seqno);
no_return exception_raise_continue_fast(
...
extern void thread_kdb_return(void);
	...
	integer_t	subcode)
{
	...
	register struct mach_exception *exc =
			(struct mach_exception *) &kmsg->ikm_header;
	union ipc_kmsg_return reply_kmsg_ret;
	...
	if (!ip_active(reply_port)) {
		ip_unlock(reply_port);
		exception_raise_continue_slow(MACH_RCV_PORT_DIED,
					      IKM_NULL,
					      /*dummy*/0);
		/*NOTREACHED*/
	}
	...
	mr = ipc_mqueue_receive(reply_mqueue, MACH_MSG_OPTION_NONE,
				...
				&reply_kmsg_ret, &reply_seqno);
	/* reply_mqueue is unlocked */

	exception_raise_continue_slow(mr, reply_kmsg_ret.kmsg, reply_seqno);
	/*NOTREACHED*/
    }
}
...
exception_raise_continue(void)
{
	...
	exception_raise_continue_slow(mr, kmsg_ret.kmsg, seqno);
	/*NOTREACHED*/
}
