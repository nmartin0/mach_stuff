 ...
 * HISTORY
 * $Log:	ipc_kobject.c,v $
 * Revision 2.16  93/11/17  17:10:30  dbg
 * 	Added kern_reply_kmsg_zone, for quick allocation of kernel reply
 * 	messages.
 * 	[93/03/30            dbg]
 * 
...
/*
 *	Size of kernel reply messages.
 *	Value is historical.
 */
#define	KERNEL_REPLY_KMSG_SIZE		(8192)

int	kern_reply_kmsg_size = KERNEL_REPLY_KMSG_SIZE;
int	kern_reply_kmsg_max  = 4096;
/*
 *	Zone for kernel reply messages.
 */
zone_t	kernel_reply_kmsg_zone;

boolean_t
ipc_kobject_notify(
	mach_msg_header_t *request_header,
	mach_msg_header_t *reply_header);	/* forward */

/*
 *	Initialize zone.
 */
void kernel_reply_kmsg_init(void)
{
	kernel_reply_kmsg_zone = zinit(
			kern_reply_kmsg_size,
			kern_reply_kmsg_size * kern_reply_kmsg_max,
			kern_reply_kmsg_size,
			FALSE,
			"kernel reply messages");
}
...
#undef	OutP
	 ...
	 */
    {
	extern mig_routine_t
		mach_server_routine(mach_msg_header_t *),
		mach_port_server_routine(mach_msg_header_t *),
		mach_host_server_routine(mach_msg_header_t *),
		device_server_routine(mach_msg_header_t *),
		device_pager_server_routine(mach_msg_header_t *),
		mach4_server_routine(mach_msg_header_t *);
#if	MACH_DEBUG
	extern mig_routine_t
		mach_debug_server_routine(mach_msg_header_t *);
#endif
#if	NORMA_TASK
	extern mig_routine_t
		mach_norma_server_routine(mach_msg_header_t *),
		norma_internal_server_routine(mach_msg_header_t *);
#endif
#if	NORMA_VM
	extern mig_routine_t
		proxy_server_routine(mach_msg_header_t *);
#endif

#if	MACH_MACHINE_ROUTINES
	extern mig_routine_t
		MACHINE_SERVER_ROUTINE(mach_msg_header_t *);
#endif
	 ...
	 */
	dest = (ipc_port_t) request->ikm_header.msgh_remote_port;
	...
	if ((kr == KERN_SUCCESS) || (kr == MIG_NO_REPLY)) {
		 ...
		 *	which is needed in the reply message.  The
		 *	destination port right has already been destroyed.
		 */

		request->ikm_header.msgh_remote_port = MACH_PORT_NULL;
	...
	mach_msg_header_t *reply_header)
{
	...
	switch (ip_kotype(port)) {
		...
		case IKOT_TIMER:
		return timer_notify(request_header);
