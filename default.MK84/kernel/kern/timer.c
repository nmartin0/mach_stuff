 ...
 * HISTORY
 * $Log:	timer.c,v $
 * Revision 2.9  93/11/17  17:32:33  dbg
...
/*
 *	timer_read reads the value of a timer into a time_spec_t.  If the
...
timer_read(
	timer_t		timer,
	register time_spec_t *tv)
{
	timer_save_data_t	temp;

	timer_grab(timer,&temp);

#ifdef	TIMER_TO_TIMESPEC
	TIMER_TO_TIMESPEC(&temp);	/* convert to seconds/nanos */
#endif

	/*
	 *	Normalize the result
	 */
	tv->seconds = temp.high + temp.low/1000000000;
	tv->nanoseconds = temp.low%1000000000;
	...
	time_spec_t	*system_time_p)
{
	...
	user_time_p->seconds = temp.high + temp.low/1000000000;
	user_time_p->nanoseconds = temp.low % 1000000000;
	...
	TIMER_TO_TIMESPEC(&temp);	/* convert to seconds/nanos */
#endif
	system_time_p->seconds = temp.high + temp.low/1000000000;
	system_time_p->nanoseconds = temp.low % 1000000000;
}
	...
	timer_save_t		save)
{
	...
	result = TIMER_HIGH_TO_USAGE(new_save.high - save->high) +
		 TIMER_LOW_TO_USAGE(new_save.low - save->low);
