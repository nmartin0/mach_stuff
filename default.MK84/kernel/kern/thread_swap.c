 ...
 * HISTORY
 * $Log:	thread_swap.c,v $
 * Revision 2.13  93/11/17  17:31:37  dbg
 * 	Break up thread lock.
 * 	[93/05/26            dbg]
 * 
 * 	Added AST_KERNEL_CHECK to swapper thread.  Added ANSI
 * 	function prototypes.  Declare non-returning functions
 * 	as returning 'no_return'.
 * 	[93/05/04            dbg]
 * 
 ...
 */
no_return swapin_thread_continue(void)
{
	for (;;) {
		register thread_t thread;
		spl_t s;
