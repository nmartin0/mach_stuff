 ...
 * HISTORY
 * $Log:	ast.h,v $
 * Revision 2.12  93/11/17  17:06:28  dbg
 * 	Import machine/machspl.h for splsched.
 * 	[93/06/11            dbg]
 * 
 * 	Added AST_TIMER for timeouts.  Defined AST_KERNEL to be
 * 	the set of ASTs for periodic kernel operations.  Added
 * 	AST_KERNEL_CHECK to check for kernel ASTs.  Added ANSI
 * 	function prototypes.
 * 	[93/01/13            dbg]
 * 
...
#define AST_HALT	0x1	/* thread */
#define AST_TERMINATE	0x2	/* thread */
#define AST_BLOCK	0x4	/* scheduling */
#define AST_NETWORK	0x8	/* kernel */
#define AST_NETIPC	0x10	/* kernel */
#define	AST_TIMER	0x20	/* kernel */
...
/*
 * Kernel ASTs are preserved across context switches.  They
 * may be taken at clean points in long-running kernel routines.
 */
#define	AST_KERNEL	(AST_NETWORK | AST_NETIPC | AST_TIMER)

/*
 *	ASTs are set and checked only on the same CPU.
 */
...
/*
 * Call at splsched to take ASTs for kernel and user.
 * Returns at spl0.
 */
extern void ast_taken(void);

/*
 * Call at splsched to take ASTs for periodic kernel activities.
 * Returns at spl0.
 */
extern void ast_kernel_taken(void);

/*
 * Call at interrupt time to check for conditions
 * that may require a context-switch (AST_BLOCK),
 * or for new per-thread ASTs for the current thread.
 *
 * "Thread" is always the current thread.
 * "End_quantum" is to make this routine usable
 * as a 'clock_sched' routine (see kern/sched_policy.h).
 */
extern void ast_check(thread_t thread, boolean_t end_quantum);

/*
 * Macro to check for kernel activity ASTs and call them.
 * Comes in two versions: one to be used at spl0, and
 * one to be used at splsched.
 */
#define	AST_KERNEL_CHECK(mycpu) \
MACRO_BEGIN								\
	if (need_ast[mycpu] & AST_KERNEL) {				\
	    (void) splsched();						\
	    ast_kernel_taken();						\
	}								\
MACRO_END

#define	AST_KERNEL_CHECK_HIGH(mycpu) \
MACRO_BEGIN								\
	if (need_ast[mycpu] & AST_KERNEL) {				\
	    ast_kernel_taken();						\
	    (void) splsched();						\
