 ...
 * HISTORY
 * $Log:	db_break.c,v $
 * Revision 2.18  93/11/17  16:20:04  dbg
 * 	Added ANSI function prototypes.
 * 	[93/10/08            dbg]
 * 
 * Revision 2.17  93/05/17  10:26:24  rvb
 * 	Type casts, etc to quiet gcc 2.3.3 warnings
 * 
 * Revision 2.16  93/01/14  17:24:29  danner
 * 	64bit cleanup.
 * 	[92/11/30            af]
 * 
 * Revision 2.15  92/08/03  17:30:33  jfriedl
 * 	removed silly prototypes
 * 	[92/08/02            jfriedl]
 * 
 * Revision 2.14  92/05/21  17:06:24  jfriedl
 * 	Removed unused variable from db_delete_cmd().
 * 	Added declaration for arg 'count' of db_add_thread_breakpoint().
 * 	[92/05/18            jfriedl]
 * 
 * Revision 2.13  92/05/04  11:23:49  danner
	...
	boolean_t is_task)
{
	...
	db_printf("\n");
	(void) db_delete_thread_breakpoint(bkpt, task_thd);
}
	...
	boolean_t	task_bpt)
{
	...
	if (!db_add_thread_breakpoint(bkpt, task_thd, count, task_bpt)) {
	    if (alloc_bkpt)
	...
	db_addr_t	addr)
{
	...
	for (bkpt = db_breakpoint_list; bkpt != 0; bkpt = bkpt->link) {
	    ...
	    if ((bkpt->flags & BKPT_USR_GLOBAL) == 0 &&
		  DB_PHYS_EQ(task, (vm_offset_t)addr,
			     bkpt->task, (vm_offset_t)bkpt->address))
		return TRUE;
	}
	return FALSE;
}
	...
	db_addr_t	addr)
{
	...
	bkpt->threads = 0;
	if (!db_add_thread_breakpoint(bkpt, 0, 1, FALSE)) {
	    if (bkpt)
		db_breakpoint_free(bkpt);
	    db_printf("Too many thread_breakpoints.\n");
	    return 0;
	...
	db_breakpoint_t	bkpt)
{
	...
	(void) db_delete_thread_breakpoint(bkpt, 0);
	db_breakpoint_free(bkpt);
}
