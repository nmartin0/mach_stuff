 ...
 * HISTORY
 * $Log:	db_print.c,v $
 * Revision 2.20  93/11/17  16:24:19  dbg
 * 	Changes for new scheduler:
 * 	  task_priority is gone
 * 	  use policy-specific function to print policy name and priority
 * 	[93/05/11            dbg]
 * 
 * 	Added include of kern/processor.h.
 * 	[93/04/10            dbg]
 * 
	...
	int	 flag)
{
	if (flag & OPTION_USER) {
		...
		if (flag & OPTION_THREAD_TITLE) {
		    ...
		    db_printf("      SUS POL PRI CONTINUE,WAIT_FUNC\n");
		}
		db_printf("%s%3d%c %0*X %s %0*X %0*X %3d ",
		    indent, thread_id,
		...
		THREAD_DB_PRINT(thread);	/* pol pri */
	...
	int	flag)
{
	...
	if (flag & OPTION_USER) {
		...
		db_printf(" ID: TASK     MAP      THD SUS %s",
			  DB_TASK_NAME_TITLE);
		if ((flag & OPTION_LONG) == 0)
		    db_printf("  THREADS");
		db_printf("\n");
	    }
	    db_printf("%3d: %0*X %0*X %3d %3d ",
...
db_port_iterate(
	thread_t thread,
	void (*func)(int, ipc_port_t, unsigned int, int) )
{
	ipc_entry_t entry;
	int index;
	int n = 0;
