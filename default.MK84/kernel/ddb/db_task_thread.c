 ...
 * Revision 2.6  93/11/17  16:25:42  dbg
...
db_check_thread_address_valid(
	db_addr_t	address)
{
	if (db_lookup_thread((thread_t) address) < 0) {
	...
	int		position)
{
	...
	*threadp = THREAD_NULL;
	if (db_expression(&value)) {
	    if (!db_check_thread_address_valid(value)) {
		db_flush_lex();
		return FALSE;
	    }
	...
	db_var_aux_param_t ap)
{
	...
	if (thread != THREAD_NULL &&
	    !db_check_thread_address_valid((db_expr_t)thread))
	{
	    db_error(0);
	    /* NOTREACHED */
	}
