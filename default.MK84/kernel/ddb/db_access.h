 ...
 * HISTORY
 * $Log:	db_access.h,v $
 * Revision 2.7  93/11/17  16:19:15  dbg
 * 	Added low-level access routines: db_read_bytes, db_write_bytes.
 * 	[93/10/13            dbg]
 * 
...
/*
 *	Read/write a word (or less) in the current task.
 */
...
/*
 *	Read/write a word (or less) in the specified task.
 */
...
/*
 *	Read/write arbitrary arrays of bytes.
 */
extern void	 db_read_bytes(	db_addr_t addr,
				int	size,
				char *	data,
				task_t	task);

extern void	 db_write_bytes(db_addr_t addr,
				int	size,
				char *	data,
				task_t	task);
