 ...
 * HISTORY
 * $Log:	db_sym.c,v $
 * Revision 2.18  93/11/17  16:25:12  dbg
...
db_sym_t	db_lookup(char *symstr);	/* forward */
	...
	char *symstr)
{
	...
	for (i = symtab_start; i < symtab_end; i++) {
		if ((sp = X_db_lookup(&db_symtabs[i], symstr)) != 0) {
  ...
  for (sp = &db_symtabs[0], i = 0; i < db_nsymtab;  sp++, i++) 
	  ...
	  (vm_map_t)sp->map_pointer == map_for_val) 
	{
	  ...
	  if ((vm_map_t) sp->map_pointer != VM_MAP_NULL && 
		...
		* and 
		* old is VM_MAP_NULL or old in is matching map
		* but is further away 
		*/
	      db_last_symtab = sp;
	      diff = newdiff;
