 ...
 * HISTORY
 * $Log:	db_command.c,v $
 * Revision 2.23  93/11/17  16:20:49  dbg
	...
	struct db_command	*cmd_table)
{
	...
	if (t == tEOL || t == tSEMI_COLON) {
		...
		switch (result) {
		    case CMD_NONE:
			if (db_exec_macro(db_tok_string))
			    return;
...
void db_machine_commands_install(
