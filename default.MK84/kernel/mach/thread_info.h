 ...
 * HISTORY
 * $Log:	thread_info.h,v $
 * Revision 2.6  93/11/17  17:47:38  dbg
 * 	Added THREAD_POLICY_INFO flavor.
 * 	[93/05/12            dbg]
 * 
...
/*
 *	Basic information for thread.
 *	The base and current scheduling priority are
 *	valid only if the thread is running the
 ...
 *	The contents are valid only for the timesharing
 *	and fixed priority policies.
 */
...
/*
 *	Thread scheduling policy information.
 *	For all policies, this returns the policy name
 *	and whether the thread is running with priority
 *	depression.  If enough space is allocated, this
 *	structure is followed by policy-specific scheduling
 *	policy information.
 */
#define	THREAD_POLICY_INFO	3

struct thread_policy_info {
	integer_t	policy;		/* scheduling policy */
	integer_t	depressed;	/* TRUE if currently depressed */
};

typedef	struct thread_policy_info	thread_policy_info_data_t;
typedef	struct thread_policy_info	*thread_policy_info_t;
#define	THREAD_POLICY_INFO_COUNT \
		(sizeof(thread_policy_info_data_t) / sizeof(integer_t))
