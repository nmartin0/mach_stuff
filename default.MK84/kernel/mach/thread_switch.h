 ...
 * Revision 2.6  93/11/17  17:47:58  dbg
...
#define	SWITCH_OPTION_NONE	0	/* no change to current thread */
#define SWITCH_OPTION_DEPRESS	1	/* drop to depressed priority for
					   specified time and restore
					   current priority when awakened,
					   or when no other threads can
					   run */
#define SWITCH_OPTION_WAIT	2	/* sleep for specified time */
#define	SWITCH_OPTION_IDLE	3	/* drop to depressed priority for
					   specified time, and restore
					   current priority when time
					   expires.  If there are no other
					   threads to run, run at depressed
					   priority. */

#define valid_switch_option(opt)	((0 <= (opt)) && ((opt) <= 3))
