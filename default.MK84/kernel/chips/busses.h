 ...
 */
struct bus_driver {
	int	(*probe)(vm_offset_t, 
			 struct bus_ctlr *);	/* see if the driver is there */
	int	(*slave)(          
			 struct bus_device *,
			 vm_offset_t);	        /* see if any slave is there */	
	void	(*attach)(struct bus_device *);	/* setup driver after probe */
...
extern boolean_t configure_bus_master(char*, vm_offset_t, vm_offset_t, int, char* );
extern boolean_t configure_bus_device(char*, vm_offset_t, vm_offset_t, int, char* );
#endif	/* KERNEL */
