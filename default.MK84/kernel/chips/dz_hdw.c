 ...
 * HISTORY
 * $Log:	dz_hdw.c,v $
 * Revision 2.20  93/11/17  16:11:15  dbg
 * 	Import kern/time_out.h for hz, timeout, untimeout.
 * 	[93/06/10            dbg]
 * 
...
boolean_t		dz7085_uses_modem_control = FALSE;
						/* patch this with adb */
...
void dz7085_set_modem_control(
	dz7085_softc_t	sc,
	boolean_t	on);

/*
 *	Set up console vectors to point to DZ.
 */
void set_dz_address(
	int		unit,
	vm_offset_t	regs,
	...
	register int		line)
{
	...
	if (tp->t_ispeed == 0) {
		(void) (*console_mctl)(tp->t_dev, TM_HUP, DMSET);
						/* hang up line */
		return;
	}
/* 19200/38400 here */
...
void dz7085_scan()
{
	...
	for (i = 0; i < NDZ_; i++) {
		...
		check_car(&tp[0], 1);
		check_car(&tp[1], 1);
		check_car(&tp[2], msr & DZ_MSR_CD2);
		check_car(&tp[3], msr & DZ_MSR_CD3);
...
/*printf("%s %x->%x\n", "ET Phone RI", temp & RING, msr & RING);*/
			check_ring(&tp[2], msr & DZ_MSR_RI2, temp & DZ_MSR_RI2);
			check_ring(&tp[3], msr & DZ_MSR_RI3, temp & DZ_MSR_RI3);
		}
#undef RING
		/* see if we must do flow-control */
		if ((msr ^ temp) & DZ_MSR_CTS2) {
			tty_cts(&tp[2], msr & DZ_MSR_CTS2);
		}
		if ((msr ^ temp) & DZ_MSR_CTS3) {
			tty_cts(&tp[3], msr & DZ_MSR_CTS3);
		}
	}
	timeout(dz7085_scan, 0, 2*hz);
}

static void dz7085_hup(
	register struct tty *tp)
{
	...
	int	oring)
{
	...
	if (ring) {
		...
		timeout(dz7085_hup, tp, dz7085_ring_timeout*hz);
	}
}
#endif	/* NDZ_ > 0 */
