 ...
 * HISTORY
 * $Log:	nw_mk.c,v $
 * Revision 2.3  93/12/23  17:00:37  dbg
 * 	Converted to new timers.
 * 	[93/12/23            dbg]
 * 
...
void h_initialize() {
  ...
  if (!h_initialized) {
    ...
    nw_fast_timer.te_fcn = mk_fast_sweep;
    nw_fast_timer.te_param = NULL;
    nw_fast_timer.te_flags = TELT_UNSET;
    nw_fast_timer.te_clock = sys_clock;
    nw_slow_timer.te_fcn = mk_slow_sweep;
    nw_slow_timer.te_param = NULL;
    nw_slow_timer.te_flags = TELT_UNSET;
    nw_slow_timer.te_clock = sys_clock;
#if PRODUCTION
    {
	...
	(void) timer_elt_enqueue(&nw_slow_timer, interval, FALSE);
    }
#endif
    h_initialized = TRUE;
  }
...
nw_buffer_t mk_receive(nw_ep ep, int time_out) {
  ...
  if (ep >= MAX_EP || (pv = hect[ep].pv) == NULL) {
	...
	if (time_out != -1) {
	  ...
	  if (!(current_thread()->timer.te_flags & TELT_SET))
...
#ifdef PRODUCTION
  if (!(nw_fast_timer.flags & TELT_SET) {
      time_spec_t	interval;
      interval.seconds = 0;
      interval.nanoseconds = 10 * 1000000;	/* 10 milliseconds */
      (void) timer_elt_enqueue(&nw_fast_timer, interval, FALSE);
  }
#endif
}
...
void h_fast_timer_reset() {
  ...
  timer_elt_remove(&nw_fast_timer);
}
