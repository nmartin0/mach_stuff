...
found:
...
#ifdef	CONFIG_OSFMACH3
	kr = default_pager_object_create(osfmach3_get_default_pager_port(),
					 &mem_obj, size);
	if (kr != KERN_SUCCESS) {
		shm_segs[id] = (struct shmid_ds *) IPC_UNUSED;
		wake_up(&shm_lock);
	...
	shp->shm_pages = (unsigned long *) mem_obj;	/* XXX */
	i = 0;
...
static void killseg (int id)
{
	...
	kr = mach_port_deallocate(mach_task_self(),
				  (mach_port_t) shp->shm_pages);
	if (kr != KERN_SUCCESS) {
		MACH3_DEBUG(1, kr, ("killseg: mach_port_deallocate(0x%x)",
				    (mach_port_t) shp->shm_pages));
	}
	i = 0;
#else	/* CONFIG_OSFMACH3 */
