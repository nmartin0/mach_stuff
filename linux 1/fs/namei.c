...
static inline int get_max_filename(unsigned long address)
{
	...
	if (current->osfmach3.task->mach_aware) {
		 ...
		 * so our memory map might be out of date. Interact with
		 * Mach to determine if the memory is valid.
		 */
		return user_memory_get_max_filename(address);
	}
#endif	/* CONFIG_OSFMACH3 */
...
int getname(const char * filename, char **result)
{
...
#ifdef	CONFIG_OSFMACH3
	{
		...
		if (!(page = __get_free_page(GFP_KERNEL)))
			return -ENOMEM;
		*result = (char *) page;
		len = copystr_fromfs((char *) page, filename, i);
		if (len < i) {
			return 0;
		}
	}
