...
#define HFS_HDR_FNAME	3   /* real name */
#define HFS_HDR_COMNT	4   /* comment */
#define HFS_HDR_BWICN	5   /* bw icon info */
...
#define HFS_HDR_AFPINFO 14  /* AFP file info */
#define HFS_HDR_DIRID   15  /* directory id */

/* these are to deal with unixisms */
#define HFS_HDR_UNIXI   0xACC00000  /* user id */
#define HFS_HDR_MAX	20
...
static inline int copy_from_user(void *to, const void *from, const size_t s)
{
	size_t i;
	i = verify_area(VERIFY_READ, from, s);
	memcpy_fromfs(to, from, s - i);
	return i;
}

static inline int copy_to_user(void *to, const void *from, const size_t s)
{
	size_t i;
	i = verify_area(VERIFY_WRITE, to, s);
	memcpy_tofs(to, from, s - i);
	return i;
}
#endif
