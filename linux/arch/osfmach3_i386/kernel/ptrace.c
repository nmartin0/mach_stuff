...
static inline int get_stack_long(struct task_struct *task, int offset)
{
	...
	if (offset + MAGICNUMBER >= sizeof (struct pt_regs)) {
		/* not a register */
		printk("get_stack_long: can't access offset 0x%x on stack\n",
		       offset);
		return -1;
	}
	stack = (unsigned char *) task->osfmach3.thread->regs_ptr;
	stack += offset + MAGICNUMBER;
	...
	unsigned long data)
{
	...
	if (offset + MAGICNUMBER >= sizeof (struct pt_regs)) {
		/* not a register */
		printk("put_stack_long: can't access offset 0x%x on stack\n",
		       offset);
		return -EINVAL;
	}
	stack = (unsigned char *) task->osfmach3.thread->regs_ptr;
